...
@pytest.mark.parametrize("expected_host, expected_port", [
    ...
    ('smtp.gmail.com', 587),  # Edge case: no changes in attributes
])
...
def test_send_notification_smtp_exception(mock_smtp, email_service):
    result = email_service.send_notification("user@example.com", "Subject", "Body")
...
@pytest.mark.parametrize("invalid_url", [
    ('https://invalid.url/fcm/send'),  # Error case: invalid URL
])
...
@pytest.mark.parametrize("edge_case_url", [
    ('https://fcm.googleapis.com/fcm/send/'),  # Edge case: trailing slash
    ('https://fcm.googleapis.com/fcm/send?'),  # Edge case: trailing question mark
])

def test_push_service_init_edge_cases(edge_case_url):
    push_service = PushService()
    assert push_service.api_url != edge_case_url
