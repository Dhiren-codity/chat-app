...
def main():
    ...
    # DISABLED AUTO-FIX RETRIES - Go straight to nuclear option
    # max_retries=0 means skip LLM fix attempts, go directly to nuclear option
    auto_fixer = TestAutoFixer(max_retries=0)

    all_success = True

    print(f"🚀 Starting test cleanup for {len(test_files)} test file(s)...")
    print(f"⚠️  AUTO-FIX RETRIES DISABLED - Using nuclear option only")
