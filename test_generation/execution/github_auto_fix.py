...
def main():
    ...
    # max_retries=0 means: Skip LLM fix, go straight to nuclear option
    # This preserves passing tests instead of breaking them with LLM fixes
    auto_fixer = TestAutoFixer(max_retries=0)

    all_success = True

    print(f"🚀 Starting test cleanup for {len(test_files)} test file(s)...")
    print(f"🔧 AUTO-FIX STRATEGY: Nuclear option ONLY (skip LLM fix)")
    print(f"📊 Reason: LLM fixes often break passing tests. Nuclear option preserves them.")
