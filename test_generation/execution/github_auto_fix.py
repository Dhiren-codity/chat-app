...
def main():
    ...
    # max_retries=0 means: Skip LLM fix entirely, go straight to nuclear option
    auto_fixer = TestAutoFixer(max_retries=0)

    all_success = True

    print(f"🚀 Starting test cleanup for {len(test_files)} test file(s)...")
    print(f"🔧 AUTO-FIX STRATEGY: Nuclear option (keep only passing tests)")
    print(f"📊 Skipping LLM fix attempts - going straight to reliable nuclear option")
