                    ...
                    if result.returncode == 0:
                        result = subprocess.run(
                            [
                                ...
                                "commit",
                                "-m",
                                f"Auto-fix: Clean up {test_file} - keep only passing tests",
                            ],
                            capture_output=True,
                            text=True,
                            cwd=".",
                        ...
                        if result.returncode == 0:
                            print(f"✅ Committed fixed {test_file}")
                        else:
                            print(f"⚠️ Failed to commit {test_file}: {result.stderr}")
                    else:
                        print(f"⚠️ Failed to add {test_file} to git: {result.stderr}")
                except Exception as e:
                    print(f"⚠️ Error committing {test_file}: {e}")
            else:
