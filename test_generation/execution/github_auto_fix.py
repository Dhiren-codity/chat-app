...
def main():
    ...
    # max_retries=0 means: Skip LLM fixes, go straight to nuclear option
    auto_fixer = TestAutoFixer(max_retries=0)

    all_success = True

    print(f"🚀 Starting test cleanup for {len(test_files)} test file(s)...")
    print(f"🔧 AUTO-FIX STRATEGY: Nuclear option only (remove failing tests)")
    print(f"📊 This will keep passing tests and remove only failing ones")
    ...
    for test_file in test_files:
        ...
        try:
            ...
            # CRITICAL: Repair syntax errors BEFORE attempting auto-fix
            # This fixes empty blocks, bracket mismatches, orphaned decorators, etc.
            if language == "python":
                from test_generation.execution.syntax_repair import SyntaxRepairer
                syntax_repairer = SyntaxRepairer()
                success, test_code = syntax_repairer.repair_file(test_code, max_passes=3)
                if success:
                    print(f"  ✅ Syntax errors repaired successfully")
                else:
                    print(f"  ⚠️ Syntax repair made progress but code still has errors")
