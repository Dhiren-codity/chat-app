                ...
                for i, line in enumerate(lines):
                    ...
                    if pass_match:
                        ...
                        if test_name not in error_info["passing"]:
                            ...
                            logger.info(f"  ✅ Found passing test: {test_name} (from: {test_path})")
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                logger.info(f"   Passing test names: {initial_passing_tests[:20]}")
                if len(initial_passing_tests) > 20:
                    logger.info(f"   ... and {len(initial_passing_tests) - 20} more")
                logger.info(f"🔍 Test functions found in file: {test_functions[:20]}")
                if len(test_functions) > 20:
                    logger.info(f"   ... and {len(test_functions) - 20} more")

                # SIMPLIFIED MATCHING: Just check if test name is in passing list
                passing_tests = []
                failing_tests = []

                for t in test_functions:
                    # Extract bare test name (without class, without params)
                    # "TestClass::test_method[param]" -> "test_method"
                    test_name = t.split("::")[-1].split("[")[0]

                    # Simple substring match - if ANY passing test contains this name, keep it
                    matched = False
                    for passing in initial_passing_tests:
                        # Remove params from passing test too
                        passing_name = passing.split("[")[0]

                        # Simple equality check
                        if test_name == passing_name:
                            matched = True
                            passing_tests.append(t)
                            logger.info(f"  ✅ KEEP: {t} (matches: {passing})")
                            break

                    if not matched:
                        failing_tests.append(t)
                        logger.info(f"  ❌ REMOVE: {t} (not in passing list)")

                logger.info(
                    f"📊 RESULT: Keeping {len(passing_tests)}/{len(test_functions)} test functions"
                )
                logger.info(
                    f"   Expected ~{len(initial_passing_tests)} passing"
                )
            else:
