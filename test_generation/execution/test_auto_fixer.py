...
"""
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                logger.info("⚠️ SKIPPING individual test runs - trusting initial results")
    ...
    def _ensure_pytest_import(self, test_code: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if not in_docstring and not docstring_ended:
                ...
                if (stripped.startswith('"""') or stripped.startswith("'''")) and line_indent == 0:
                    in_docstring = True
    ...
    def _remove_hallucinated_imports(self, test_code: str, context: Optional[str] = None) -> str:
        """
            ...
            if stripped == "@pytest.fixture" and i + 1 < len(lines):
                ...
                if fixture_match:
                    ...
                    if is_problematic:
                        ...
                        # Skip from i to j-1
                        skip_until_line = j - 1
                        logger.info(f"      Removing lines {i + 1} to {skip_until_line + 1}")
                        i += 1
                        continue
                    else:
        ...
        for i, line in enumerate(lines):
            ...
            if not seen_test_code:
                if (stripped.startswith("def test_") or
                    stripped.startswith("class Test") or
                    (stripped.startswith("def ") and not stripped.startswith("def test_") and "@pytest.fixture" not in lines[max(0, i-1)].strip())):
                    seen_test_code = True

            # If we've seen test code and encounter an import line
            if seen_test_code and (stripped.startswith("import ") or stripped.startswith("from ")):
                if not in_import_block:
                    in_import_block = True
                    import_block_start = i
                    logger.info(f"   🔧 Found duplicate import block starting at line {i + 1}")
                ...
                if not stripped:
                    ...
                    # Import block ended
                    in_import_block = False
                    logger.info(f"      Removed import block lines {import_block_start + 1} to {i}")
...
"""
    ...
    def auto_fix_test(
        ...
        if self.max_retries > 0:
            ...
            logger.info("🔍 Removing problematic fixtures and duplicate imports (nuclear mode)")
            test_code = self._remove_hallucinated_imports(test_code)
        ...
        if self.max_retries == 0:
            ...
            has_collection_errors = any(
                err.get("type") in ["SyntaxError", "IndentationError", "CollectionError", "ImportError", "ModuleNotFoundError"]
                for err in error_info.get("errors", [])
            )

            if has_collection_errors:
                logger.warning("⚠️ COLLECTION ERRORS DETECTED - tests cannot be collected/imported!")
                logger.info("   Nuclear option will attempt to remove problematic tests")
                if passing_tests:
                    logger.info(f"   Found {len(passing_tests)} passing tests to keep")
                else:
                    logger.warning("   No passing tests found - will try to create clean file")
            ...
            if passing_tests and has_test_functions:
                ...
                fix_history.append("Nuclear option: No passing tests - keeping original code")
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt >= self.max_retries:
                ...
                logger.info(
                    f"   • Collection errors: {has_collection_errors}"
                )
                logger.info(
