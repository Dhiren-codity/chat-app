...
try:
    from .syntax_repair import SyntaxRepairer
except ImportError:
    # Fallback for when running as a script (not a package)
    from test_generation.execution.syntax_repair import SyntaxRepairer
...
class TestAutoFixer:
    ...
    def __init__(self, llm_client=None, max_retries: int = 3):
        ...
        self.syntax_repairer = SyntaxRepairer()
    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            if (
                ...
                # Match inline status (from -vv output) - FIXED: Handle percentage, brackets, and lambda
                # Pattern 1: Full path format: test_file.py::test_name PASSED [10%]
                pass_pattern = re.compile(
                    r"(\S+::[\w\[\],<>-]+)\s+PASSED\s*(?:\[\s*\d+%\s*\])?"
                )
                fail_inline_pattern = re.compile(
                    r"(\S+::[\w\[\],<>-]+)\s+(FAILED|ERROR)\s*(?:\[\s*\d+%\s*\])?"
                )
                ...
                # Pattern 2: Simple format without path: test_name PASSED [10%]
                simple_pass_pattern = re.compile(
                    r"^\s*(test_\w+(?:\[[\w\[\],<>-]+\])?)\s+PASSED\s*(?:\[\s*\d+%\s*\])?"
                )
                simple_fail_pattern = re.compile(
                    r"^\s*(test_\w+(?:\[[\w\[\],<>-]+\])?)\s+(FAILED|ERROR)\s*(?:\[\s*\d+%\s*\])?"
                )

                for i, line in enumerate(lines):
                    # Extract passing tests - try both patterns
                    ...
                    if pass_match:
                        ...
                        # Try simple pattern for lines like "test_name PASSED"
                        simple_pass_match = simple_pass_pattern.search(line)
                        if simple_pass_match:
                            test_name = simple_pass_match.group(1)
                            if test_name not in error_info["passing"]:
                                error_info["passing"].append(test_name)
                                logger.info(
                                    f"✅ Found passing test (simple): {test_name}"
                                )
                    ...
                    if fail_inline_match:
                        ...
                        # Try simple pattern for lines like "test_name FAILED"
                        simple_fail_match = simple_fail_pattern.search(line)
                        if simple_fail_match:
                            test_name = simple_fail_match.group(1)
                            status = simple_fail_match.group(2)
                            error_info["failures"].append(
                                {
                                    "test_name": line.strip(),
                                    "error_type": status,
                            ...
                            if fail_summary_match:
                                status, test_path, error_msg = (
                                    fail_summary_match.groups()
                                )
                                error_info["failures"].append(
                                    {
                                        "test_name": line.strip(),
        ...
        # FALLBACK: If no individual passing tests found, try to parse pytest summary line
        # Format: "===== 29 failed, 58 passed, 78 errors in 1.42s ====="
        if not error_info["passing"] and language == "python":
            summary_pattern = re.compile(
                r"=+\s*(?:(\d+)\s+failed)?[,\s]*(?:(\d+)\s+passed)?[,\s]*(?:(\d+)\s+error)?.*in\s+[\d.]+s\s*=+"
            )
            for line in lines:
                ...
                if match:
                    ...
                    error_count = int(match.group(3)) if match.group(3) else 0

                    logger.warning(
                        "⚠️ No individual test results found, but pytest summary shows:"
                    )
                    logger.warning(
                    ...
                    if passed_count > 0:
                        logger.warning(
                            f"⚠️ CRITICAL: {passed_count} tests passed but we couldn't extract their names!"
                        )
                        logger.warning(
                            "   This might be due to collection errors or output format issues"
                        )
                        logger.warning(
                            "   Recommendation: Fix collection errors first, then re-run auto-fix"
                        )
                    break

        # CRITICAL: Log the extracted passing tests for debugging
        if error_info["passing"]:
            logger.info(f"🎯 EXTRACTED {len(error_info['passing'])} PASSING TESTS:")
            for i, test in enumerate(error_info["passing"][:10]):  # Show first 10
                logger.info(f"   {i + 1}. {test}")
            if len(error_info["passing"]) > 10:
                logger.info(f"   ... and {len(error_info['passing']) - 10} more")
        else:
            logger.warning(
                "⚠️ NO PASSING TESTS EXTRACTED - This will cause all tests to be removed!"
            )
            logger.warning(
                f"   Raw output contains 'PASSED': {'PASSED' in combined_output}"
            )
            logger.warning(
                f"   Raw output contains 'FAILED': {'FAILED' in combined_output}"
            )
            logger.warning(
                f"   Raw output contains 'ERROR': {'ERROR' in combined_output}"
            )
            # Show a sample of the output for debugging
            logger.warning("   Sample output (first 500 chars):")
            sample = combined_output[:500].replace("\n", "\n   ")
...
"""
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                logger.info(f"🔍 Test functions found in file: {len(test_functions)}")
                logger.info(f"   First 10 passing tests: {initial_passing_tests[:10]}")
                if len(initial_passing_tests) > 10:
                    logger.info(f"   ... and {len(initial_passing_tests) - 10} more")
                ...
                logger.info(
                    "⚠️ SKIPPING individual test runs - trusting initial results"
                )
            ...
            if not passing_tests:
                ...
                # CRITICAL FIX: If we have initial_passing_tests, don't create minimal file
                # Instead, try to preserve the original tests and just fix the issues
                if initial_passing_tests and len(initial_passing_tests) > 0:
                    logger.warning(
                        f"⚠️ BUT we had {len(initial_passing_tests)} passing tests initially!"
                    )
                    logger.warning(
                        "⚠️ The issue is likely that individual test runs fail due to missing context"
                    )
                    logger.warning(
                        "⚠️ Instead of removing all tests, let's try to fix the context issues"
                    )

                    # Return the original test code with fixes applied instead of minimal file
                    logger.info(
                        "🔧 Attempting to fix context issues in original tests..."
                    )
                    fixed_code = self._apply_context_fixes(test_code)
                    if fixed_code:
                        logger.info(
                            "✅ Applied context fixes to preserve original tests"
                        )
                        return fixed_code
                    else:
                        logger.warning(
                            "⚠️ Context fixes failed, but preserving original tests anyway"
                        )
                        return test_code
    ...
    def _ensure_pytest_import(self, test_code: str) -> str:
        """
        Ensure pytest import is always present in test code.
        This is critical to prevent NameError: name 'pytest' is not defined.

        Args:
            test_code: Test code that may be missing pytest import

        Returns:
            Test code with pytest import guaranteed
        """
        lines = test_code.split("\n")

        # Check if pytest import exists
        has_pytest = any(
            "import pytest" in line and not line.strip().startswith("#")
            for line in lines
        )

        if has_pytest:
            return test_code  # Already has it

        logger.info("   ⚠️ pytest import missing - adding it")

        # Find where to insert (after module docstring if present, before other code)
        insert_index = 0
        in_docstring = False
        docstring_char = None
        ...
        for i, line in enumerate(lines):
            ...
            # Track module-level docstring (must be first statement)
            if not in_docstring and not docstring_ended:
                ...
                if (
                    ...
                    # First non-comment, non-docstring statement
                    insert_index = i
                    break
            elif in_docstring:
                ...
                # After docstring, found first code
                insert_index = i
                break

        # Insert pytest import at the correct location
        lines.insert(insert_index, "import pytest")

        # Add blank line after import if next line isn't blank
    ...
    def _remove_hallucinated_imports(
        ...
        """
        Remove common hallucinated imports that cause ImportError.

        Targets imports that LLMs commonly generate but don't exist:
        - test_sample, test_module, sample_module
        - auth_module (unless it actually exists)
        - your_module, module (placeholders)
        - flask.app (Flask internal, not user app)

        Args:
            test_code: Test code that may have hallucinated imports
            context: Optional context (not used in this implementation)

        Returns:
            Test code with hallucinated imports removed
        """
        lines = test_code.split("\n")
        result = []

        hallucinated_patterns = [
            r"from\s+test_sample\s+import",
            r"from\s+test_module\s+import",
            r"from\s+sample_module\s+import",
            r"from\s+auth_module\s+import",
            r"from\s+your_module\s+import",
            r"from\s+module\s+import",
            r"import\s+test_sample",
            r"import\s+test_module",
            r"import\s+sample_module",
            r"import\s+auth_module",
            r"import\s+your_module",
            r"from\s+flask\.app\s+import",  # Flask internal module
            r"import\s+flask\.app",
        ]
        ...
        for line in lines:
            ...
            for pattern in hallucinated_patterns:
                if re.search(pattern, stripped):
                    ...
                    logger.info(f"   🔧 Removing hallucinated import: {stripped[:60]}")
                    break
    ...
    def _remove_problematic_fixtures(self, test_code: str) -> str:
        """
        Remove problematic fixtures that cause collection errors.

        Specifically targets:
        1. Client fixtures inside classes (have 'self' parameter)
        2. Client fixtures that shadow imports (cause AttributeError)
        3. Duplicate fixtures (keep only first occurrence)
        4. CRITICAL: app() fixtures that shadow 'from app import app'

        Args:
            test_code: Test code that may have problematic fixtures

        Returns:
            Test code with problematic fixtures removed
        """
        lines = test_code.split("\n")
        result = []
        skip_until_line = -1
        seen_fixtures = set()  # Track which fixtures we've kept
        ...
        while i < len(lines):
            ...
            if stripped == "@pytest.fixture" and i + 1 < len(lines):
                next_line_stripped = lines[i + 1].strip()

                # Extract fixture name
                fixture_match = re.match(r"def (\w+)\(", next_line_stripped)
                if fixture_match:
                    fixture_name = fixture_match.group(1)

                    # Check if this is a problematic fixture
                    is_problematic = False
                    reason = ""

                    # Check 1: Is it inside a class? (indented)
                    line_indent = len(line) - len(line.lstrip())
                    if line_indent > 0:
                        is_problematic = True
                        reason = "fixture inside class"

                    # Check 2: Does it have 'self' parameter? (inside class)
                    if "self" in next_line_stripped:
                        is_problematic = True
                        reason = "fixture with 'self'"

                    # Check 3: Is it a duplicate fixture?
                    if fixture_name in seen_fixtures:
                        is_problematic = True
                        reason = f"duplicate {fixture_name}() fixture"

                    # Check 4: CRITICAL - is it an app() fixture that shadows 'from app import app'?
                    if fixture_name == "app" and "from app import app" in test_code:
                        is_problematic = True
                        reason = "app() fixture shadows import"

                    if is_problematic:
                        logger.info(f"   🔧 Removing {reason} at line {i + 1}")

                        # Find the end of the fixture
                        # CRITICAL: Use decorator indentation, not def indentation
                        decorator_indent = len(line) - len(line.lstrip())
                        j = i + 2  # Start after 'def fixture():'
                        while j < len(lines):
                            j_line = lines[j]
                            ...
                            # Exit when we find a non-empty line at same or less indentation as decorator
                            # This correctly handles nested functions inside the fixture
                            if j_indent <= decorator_indent:
                                break
                            j += 1
                        ...
                        skip_until_line = j - 1
                        logger.info(
                            f"      Removing lines {i + 1} to {skip_until_line + 1}"
                        )
                        i += 1
                        continue
                    else:
                        # This is a good fixture - track it
    ...
    def _remove_duplicate_import_blocks(self, test_code: str) -> str:
        """
        Remove duplicate import blocks that appear in the middle of the file.

        This happens when multiple test files are concatenated together.
        We keep the imports at the top and remove import blocks that appear
        after test functions or classes have started.

        Args:
            test_code: Test code with potentially duplicate import blocks

        Returns:
            Test code with duplicate import blocks removed
        """
        lines = test_code.split("\n")
        result = []

        # Track when we've seen actual test code (not just imports/fixtures)
        seen_test_code = False
        ...
        for i, line in enumerate(lines):
            ...
            # Check if this is test code (test functions, test classes, etc.)
            if not seen_test_code:
                if (
                    ...
                    or (
                        ...
                        and "@pytest.fixture" not in lines[max(0, i - 1)].strip()
                    )
                ):
                    seen_test_code = True

            # If we've seen test code and encounter an import line
            if seen_test_code and (
                ...
                if not in_import_block:
                    ...
                    logger.info(
                        f"   🔧 Found duplicate import block starting at line {i + 1}"
                    )
                # Skip this import line
                continue
            elif in_import_block:
                # We were in an import block, but this line isn't an import
                # Check if it's just an empty line (still part of the block)
                if not stripped:
                    continue  # Skip empty lines in import block
                else:
                    # Import block ended
                    in_import_block = False
                    logger.info(
                        f"      Removed import block lines {import_block_start + 1} to {i}"
                    )
    ...
    def _fix_file_structure(self, test_code: str) -> str:
        ...
        """
        # First, remove hallucinated imports
        test_code = self._remove_hallucinated_imports(test_code)

        # Then, remove problematic fixtures
        test_code = self._remove_problematic_fixtures(test_code)

        # Clean up any orphaned brackets left by fixture removal
        repairer = SyntaxRepairer()
        success, test_code = repairer.repair_file(test_code)
        if success:
            logger.info("   ✅ Syntax validated and cleaned after fixture removal")
        else:
            logger.warning("   ⚠️ Syntax repair failed after fixture removal")
        ...
        # CRITICAL: Only detect MODULE-LEVEL docstrings (indentation 0)
        # Ignore docstrings inside functions/classes
        header_lines = []
        content_lines = []
        in_header = False
        ...
        for line in cleaned_lines:
            ...
            # Only treat as file header if it's a docstring at indentation 0
            if (
                stripped.startswith('"""') or stripped.startswith("'''")
            ) and line_indent == 0:
                if not in_header:
        ...
        # CRITICAL: Remove empty test classes that may have been created
        # by fixture removal or other cleanup operations
        result_code = "\n".join(result_lines)
        result_code = self._remove_empty_test_classes(result_code)
    ...
    def _apply_context_fixes(self, test_code: str) -> str:
        """
        Apply context fixes to preserve original tests instead of removing them.
        This addresses the issue where individual test runs fail due to missing context.
        """
        logger.info("🔧 Applying context fixes to preserve original tests...")

        # Apply the same fixes we have in the main auto_fix_test method
        fixed_code = test_code

        # Fix dependency errors (mocker fixture)
        fixed_code = self._fix_dependency_errors(fixed_code)

        # Fix Flask errors (app context)
        fixed_code = self._fix_flask_errors(fixed_code)

        # Fix SQLAlchemy mocking errors
        fixed_code = self._fix_sqlalchemy_mocking_errors(fixed_code)

        # Fix syntax errors
        fixed_code = self._fix_syntax_errors(fixed_code)

        # Fix import errors
        fixed_code = self._fix_import_errors(fixed_code)

        logger.info("✅ Applied context fixes to preserve original tests")
        return fixed_code
...
"""
    ...
    def auto_fix_test(
        ...
        # CRITICAL: Ensure pytest import is present (minimal fix)
        logger.info("🔍 Ensuring pytest import is present")
        test_code = self._ensure_pytest_import(test_code)

        # CRITICAL: Only do aggressive fixes if we're doing LLM retries
        # For nuclear option (max_retries=0), keep fixes minimal
        if self.max_retries > 0:
            # CRITICAL: Fix file structure to prevent collection errors
            # This reorganizes imports and removes problematic fixtures
            logger.info("🔍 Fixing file structure (imports, fixtures, empty classes)")
            test_code = self._fix_file_structure(test_code)
        else:
            # Nuclear option mode: Only remove problematic fixtures, don't reorganize
            logger.info(
                "🔍 Removing problematic fixtures and duplicate imports (nuclear mode)"
            )
            test_code = self._remove_hallucinated_imports(test_code)
            test_code = self._remove_duplicate_import_blocks(test_code)
            test_code = self._remove_problematic_fixtures(test_code)
        ...
        if self.max_retries == 0:
            ...
            # Check for collection errors (syntax/import errors)
            ...
            # CRITICAL FIX: Try syntax error fixing before nuclear option
            if has_collection_errors:
                logger.warning(
                    "⚠️ COLLECTION ERRORS DETECTED - attempting syntax fixes before nuclear option"
                )

                # Use the new AST-based syntax repairer
                success_repair, fixed_code = self.syntax_repairer.repair_file(
                    current_code
                )
                # Use the repaired code even if repair didn't fully succeed
                # It may have made partial progress (e.g., removing orphaned brackets)
                if fixed_code != current_code:
                    logger.info("🔧 Applied syntax repairs (partial or full), testing again...")
                    Path(test_file_path).write_text(fixed_code, encoding="utf-8")
                    current_code = fixed_code  # Update for next steps
                    success, stdout, stderr = self.run_tests_and_capture_errors(
                        test_file_path, language
                    )
                    if success:
                        logger.info("✅ Syntax repairs resolved all issues!")
                        return True, fixed_code, ["Syntax errors fixed"]
                    else:
                        # Update error info with new results
            ...
            if has_collection_errors:
                logger.warning(
                    "⚠️ COLLECTION ERRORS DETECTED - tests cannot be collected/imported!"
                )
                logger.info(
                    "   Nuclear option will attempt to remove problematic tests"
                )
                if passing_tests:
                    logger.info(f"   Found {len(passing_tests)} passing tests to keep")
                else:
                    logger.warning(
                        "   No passing tests found - will try to create clean file"
                    )

            logger.info(
                f"📊 Test results: {len(passing_tests)} passing, "
                f"{len(error_info.get('failures', []))} failing, "
                f"collection_errors: {has_collection_errors}"
            )
            ...
            if passing_tests and has_test_functions:
                ...
                logger.warning("⚠️ No passing tests found, skipping nuclear option")
                logger.warning("   Returning original code as-is (no placeholders)")
                fix_history.append(
                    "Nuclear option: No passing tests - keeping original code"
                )
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt >= self.max_retries:
                ...
                if has_collection_errors:
                    ...
                    if passing_tests:
                        logger.info(
                            f"   Found {len(passing_tests)} passing tests to keep"
                        )
                    else:
                        logger.warning(
                            "   No passing tests found - will try to create clean file"
                ...
                logger.info(f"   • Collection errors: {has_collection_errors}")
                logger.info(
                    "   • Target result: Keep passing tests, remove failing ones"
                )
                ...
                if cleaned_code and cleaned_code != current_code:
                    ...
                    if remaining_tests > 0:
                        ...
                        logger.warning("   Keeping original code (no placeholders)")
                        Path(test_file_path).write_text(current_code, encoding="utf-8")
                        fix_history.append(
                            f"Attempt {attempt}: Nuclear option removed all tests - keeping original code"
                        )
                        return True, current_code, fix_history
                else:
                    if has_test_functions:
                        ...
                        Path(test_file_path).write_text(current_code, encoding="utf-8")
                        fix_history.append(
                            f"Attempt {attempt}: Nuclear option failed - keeping original code"
                        )
                        return True, current_code, fix_history
                    else:
                        logger.info(
    ...
    def _fix_syntax_errors(self, test_code: str, error_info: dict = None) -> str:
        """
        Fix common syntax errors in test code.

        Args:
            test_code: Test code with syntax errors
            error_info: Error information from pytest (optional)
        ...
        while i < len(lines):
            ...
            # Fix return statements at module level
            if line.strip().startswith(
                "return "
            ) and not self._is_inside_function_definition(line, lines):
                logger.debug(f"Removing module-level return: {line.strip()}")
                i += 1
                continue

            # Fix decorators without function bodies
            if line.strip().startswith("@") and not self._is_followed_by_function(
                lines, i
            ):
                logger.debug(f"Removing orphaned decorator: {line.strip()}")
                i += 1
                continue

            # Fix empty function bodies
            if line.strip().startswith("def ") and self._has_empty_function_body(
                lines, i
            ):
                fixed_lines.append(line)
                i += 1
                ...
                logger.debug(f"Added pass to empty function: {line.strip()}")
            else:
                fixed_lines.append(line)
    ...
    def _is_inside_function_definition(self, line: str, all_lines: list) -> bool:
        """Check if a line is inside a function definition."""
        line_index = all_lines.index(line)
        indent_level = len(line) - len(line.lstrip())

        # Look backwards for function definition with less indentation
        for i in range(line_index - 1, -1, -1):
            prev_line = all_lines[i]
            if (
                prev_line.strip().startswith("def ")
                and len(prev_line) - len(prev_line.lstrip()) < indent_level
            ):
                return True
        return False

    def _is_followed_by_function(self, lines: list, decorator_index: int) -> bool:
        """Check if a decorator is followed by a function definition."""
        for i in range(decorator_index + 1, min(decorator_index + 3, len(lines))):
            if lines[i].strip().startswith("def "):
                return True
        return False

    def _has_empty_function_body(self, lines: list, function_index: int) -> bool:
        """Check if a function has an empty body."""
        ...
        for i in range(function_index + 1, len(lines)):
            line = lines[i]
            if line.strip() == "":
                continue
            current_indent = len(line) - len(line.lstrip())
