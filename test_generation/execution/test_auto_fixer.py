...
class TestAutoFixer:
    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            # Extract test failures with detailed error messages
            if "FAILED" in combined_output or "ERROR" in combined_output:
                ...
                for i, line in enumerate(lines):
                    ...
                    if match:
                        status, test_path, error_msg = match.groups()
...
"""
    ...
    def _fix_flask_fixtures(
        ...
        # CRITICAL FIX: Detect and fix broken "def app():" fixtures that shadow the app variable
        # This pattern causes: AttributeError: 'FixtureFunctionDefinition' object has no attribute 'config'
        broken_app_fixture_pattern = re.compile(
            r'(@pytest\.fixture\s+)?def app\(\):\s*(?:""".*?"""\s*)?app\.config',
            re.MULTILINE | re.DOTALL,
        )
        if broken_app_fixture_pattern.search(test_code):
            logger.warning(
                "  ⚠️ Detected broken 'def app():' fixture that shadows app variable!"
            )
            logger.info("  🔧 Fixing by adding import with alias...")
            ...
            if parent_module and parent_module != "." and module_file.exists():
                module_import = (
                    f"from {parent_module}.{module_file.stem} import app as flask_app"
                )
            else:
                module_import = "from app import app as flask_app"

            # Replace the broken fixture with a fixed one
            # Pattern: def app(): followed by app.config
            # Fix: Add import and use flask_app
            fixed_code = test_code

            # Find all broken app fixtures
            lines = test_code.split("\n")
            in_app_fixture = False
            fixed_lines = []

            for i, line in enumerate(lines):
                if line.strip() == "def app():" or (
                    line.strip().startswith("def app(") and line.strip().endswith("):")
                ):
                    in_app_fixture = True
                    fixed_lines.append(line)
                    # Add import on next line
                    continue

                if in_app_fixture and "app.config" in line:
                    # This is the broken line - add import before it
                    indent = len(line) - len(line.lstrip())
                    fixed_lines.append(" " * indent + module_import)
                    # Replace app. with flask_app.
                    fixed_lines.append(line.replace("app.", "flask_app."))
                    in_app_fixture = False
                elif in_app_fixture and line.strip().startswith('"""'):
                    # Docstring in fixture
                    fixed_lines.append(line)
                elif in_app_fixture and "return app" in line:
                    # Return statement
                    fixed_lines.append(line.replace("return app", "return flask_app"))
                    in_app_fixture = False
                else:
                    fixed_lines.append(line)

            test_code = "\n".join(fixed_lines)
            logger.info("  ✅ Fixed broken app fixture!")
        ...
        try:
            ...
            if not app_import:
                ...
                # Try to find if the module uses Flask
                if parent_module and parent_module != ".":
                    ...
                    if module_file.exists():
                        ...
                        logger.info(
                            f"  ℹ️ Inferred app import from structure: {app_import}"
                        )
                    else:
                        app_import = "from app import app"
                        logger.info(f"  ℹ️ Using default app import: {app_import}")
                else:
                    app_import = "from app import app"
            ...
            if should_reconstruct:
                ...
                # 3. Add essential Flask fixtures if not already present
                ...
                # Add app fixture if missing
                if not has_app_fixture and app_var_name == "app":
                    final_lines.append("@pytest.fixture")
                    final_lines.append("def app():")
                    final_lines.append('    """Create application for testing."""')
                    # Import with alias to avoid shadowing
                    final_lines.append(
                        f"    from {module_name} import app as flask_app"
                    )
                    final_lines.append("    flask_app.config['TESTING'] = True")
                    final_lines.append("    return flask_app")
                    final_lines.append("")

                # Add client fixture if missing
                if not has_client_fixture:
                    final_lines.append("@pytest.fixture")
                    final_lines.append("def client(app):")
                ...
                # 4. Add moved fixtures at module level with proper Flask context
                    ...
                    for fline in fixture_group:
                        if fline.strip().startswith("def "):
                            ...
                            if "def client" in func_def:
                                # Ensure it has app parameter
                                if "(app)" not in func_def and "()" in func_def:
                                ...
                                final_lines.append(f"    return {app_var_name}")
                            else:
                ...
                # 4. Add the rest of the file (classes, tests) with client/app param fixes
                in_imports = True
                for line in fixed_lines:
                    if in_imports and (
                        line.strip().startswith(("import ", "from "))
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            # DON'T remove imports before testing!
            # The fallback tests that PASS don't have bad imports
            # Removing imports would make them fail
            # We'll remove bad imports AFTER we know which tests to keep
            logger.info(f"📋 Testing with original code (not removing imports yet)...")

            # Keep the original test_code AS-IS for testing
            # Write it back to the file in case it was modified earlier
            ...
            logger.info(
                f"📋 Found {len(test_functions)} test functions, testing each individually..."
            )
            ...
            # NOW remove bad imports from the final result
            # This ensures the kept tests don't have placeholder imports
            logger.info(f"🧹 Cleaning up bad imports from final result...")
            cleaned_code = self._remove_bad_imports_from_code(cleaned_code)
    ...
    def auto_fix_test(
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt >= self.max_retries:
                ...
                logger.warning(
                    "⚠️ Trying NUCLEAR OPTION as last resort: testing each function individually"
                )
                logger.warning(
        ...
        # If we get here, all attempts failed - try absolute last resort
        logger.warning(
