            ...
            if (
                ...
                for i, line in enumerate(lines):
                    ...
                    if pass_match:
                        ...
                        # Preserve Class::method format, remove only file path
                        # file.py::TestClass::test_method -> TestClass::test_method
                        # file.py::test_method -> test_method
                        ...
                        # Safety: Only add if test_name is non-empty
                        # (regex already ensures it matches pytest test format)
...
"""
    ...
    def _remove_orphaned_decorators(self, test_code: str) -> str:
        """Remove decorators that aren't followed by a function/class definition."""
        import re
        lines = test_code.split("\n")
        result_lines = []
        i = 0
        ...
        while i < len(lines):
            ...
            if stripped.startswith('@'):
                # Look ahead to find what follows
                j = i + 1
                has_following_definition = False
                ...
                while j < len(lines):
                    ...
                    if next_stripped.startswith('@'):
                        j += 1
                        continue
                    # Check if followed by def or class
                    if next_stripped.startswith(('def ', 'class ')):
                        has_following_definition = True
                    break

                # If decorator not followed by def/class, skip it (orphaned)
                if not has_following_definition:
                    logger.info(f"  🗑️ Removing orphaned decorator at line {i+1}: {stripped[:50]}")
                    i += 1
                    continue
    ...
    def _fix_module_level_model_instantiation(self, test_code: str, language: str) -> tuple[str, bool]:
        """Fix module-level model instantiation errors (e.g., MessageReaction) by replacing with Mock."""
        if language != "python":
            return (test_code, False)

        import re
        model_pattern = re.compile(r'\b([A-Z][a-zA-Z0-9_]*)\s*\(', re.MULTILINE)
        in_function_or_class = False
        indent_level = 0
        was_modified = False

        # Check if Mock is imported
        mock_import_pattern = re.compile(r'\bfrom\s+unittest\.mock\s+import\s+.*\bMock\b')
                ...
                if matches:
                    ...
                    for match in reversed(matches):
                        ...
                        if model_name in ('Mock', 'MagicMock', 'patch', 'TestCase', 'Dict', 'List',
                                         ...
                                         'NotImplementedError', 'IndexError', 'None', 'True',
                                         'False', 'str', 'int', 'float', 'bool', 'bytes',
                                         'Path', 'datetime'):
                            continue
        ...
        if was_modified:
            logger.info("✅ Fixed module-level model instantiation errors")
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            # CRITICAL: Fix module-level model instantiation errors (e.g., MessageReaction)
            # These prevent test collection before any tests can run
            logger.info(f"🧹 Fixing module-level model instantiation errors...")
            test_code, was_modified = self._fix_module_level_model_instantiation(test_code, language)
            if was_modified:
                logger.info("   ✅ Replaced model instantiations with Mock objects")
            ...
            for line in lines:
                ...
                # Check for test class (pytest collects classes with "Test" in name or containing test methods)
                # Match patterns: Test*, *Test, *Tests, *TestCase
                class_match = re.match(r"^\s*class\s+(\w+)", line)
                if class_match:
                    class_name = class_match.group(1)
                    # Accept any class that might contain tests (be permissive)
                    # We'll validate during matching - if pytest found it, it's valid
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                for t in test_functions:
                    ...
                    file_test_name = t.split("[")[0]  # Remove params from test function

                    # Check if this test (or any parametrized version of it) passed
                    matched = False
                    for passing in initial_passing_tests:
                        # Safety: Skip None or empty values
                        if not passing:
                            continue

                        # passing now looks like: "TestClass::test_method[param]" or "test_function[param]"
                        # (file path already removed by extract_error_details)

                        # Remove parametrization brackets from passing test
                        passing_base = passing.split("[")[0]

                        # Direct comparison since both have same format now
                ...
                # SANITY CHECK: Only trigger if matching is clearly broken
                # The ONLY clear indicator of a matching bug is: we have passing tests and test functions, but matched ZERO
                # Note: Due to parametrization, 50 passing tests might come from 3 functions, so low ratios are OK
                # We only care about the absolute failure case: matched nothing when we should have
                is_likely_matching_bug = (
                    len(initial_passing_tests) > 0 and
                    len(test_functions) > 0 and
                    len(passing_tests) == 0
                )

                if is_likely_matching_bug:
                ...
                # No initial_passing_tests provided - can't use nuclear option optimization
                # CRITICAL: DO NOT run tests individually! Tests that pass as a group often fail individually
                # due to shared fixtures, Flask app context, database state, etc.
                logger.warning("⚠️ No initial passing tests provided - cannot use nuclear option")
                logger.warning(
                    "⚠️ Nuclear option requires knowing which tests passed from the initial run"
                )
                logger.warning(
                    "⚠️ Returning None - caller should try other fix strategies (LLM, etc.)"
                )
                return None

            # Summary
            logger.info(
                f"📊 FINAL RESULT: Keeping {len(passing_tests)}/{len(test_functions)} test functions"
            )
            logger.info(f"   Expected to keep ~{len(initial_passing_tests)} based on initial run")

            if not passing_tests:
                logger.warning("⚠️ No passing tests matched - returning minimal file!")
                logger.warning(
                    f"⚠️ DEBUG: initial_passing_tests had {len(initial_passing_tests)} tests"
                )
                logger.warning(
            ...
            for test_name in failing_tests:
                ...
                if func_name:
                    failing_function_names.add(func_name)
            ...
            # CRITICAL: Clean up after test removal to handle orphaned decorators
            # Removing tests can leave parametrize decorators that attach to wrong functions
            logger.info("🧹 Post-removal cleanup: removing orphaned decorators...")
            cleaned_code = self._remove_orphaned_decorators(cleaned_code)

            # Fix any model instantiations that might now be at module level after removal
            logger.info("🧹 Post-removal cleanup: fixing any new module-level instantiations...")
            cleaned_code, _ = self._fix_module_level_model_instantiation(cleaned_code, language)
            ...
            if attempt >= self.max_retries:
                ...
                if cleaned_code and cleaned_code != current_code:
                    ...
                    if success:
                        ...
                        logger.warning(
                            f"   This means the passing tests have dependencies on the removed tests"
                        )
                        logger.warning(
                            f"   or there are other errors in the test code (imports, fixtures, etc.)"
                        )
                else:
