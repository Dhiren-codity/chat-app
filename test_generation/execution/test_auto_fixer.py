...
"""
    ...
    def keep_only_passing_tests(self, test_file_path: str, language: str = "python") -> Optional[str]:
        ...
        try:
            ...
            if not passing_tests:
                logger.warning("⚠️ No tests passed! Creating minimal valid file...")
                return self._create_minimal_valid_file(test_code)
    ...
    def _create_minimal_valid_file(self, test_code: str) -> str:
        ...
        ALSO removes bad placeholder imports to prevent collection errors.
        """
        import re

        # FIRST: Remove bad imports
        logger.info("🧹 Removing bad imports from minimal file...")
        test_code = self._remove_bad_imports_from_code(test_code)

        # THEN: Remove test functions
        lines = test_code.split('\n')
        cleaned_lines = []
...
"""
    ...
    def auto_fix_test(
        ...
        flask_fix_applied = False  # Track if Flask fix was applied to skip nuclear option
            ...
            if attempt == 2 and error_info['failures'] and language == "python":
                ...
                if is_flask_context_error:
                    ...
                    if fixed_code != current_code:
                        ...
                        flask_fix_applied = True  # Mark that we applied Flask fix
                        continue

            # AGGRESSIVE FALLBACK: After attempt 2, keep only passing tests
            # This matches the reference Go implementation behavior
            # IMPORTANT: Skip at attempt 2 if Flask fix was already applied (give it a chance first)
            # But apply at attempt 3 if Flask fix didn't work
            should_apply_nuclear = (
                (attempt == 2 and not flask_fix_applied) or  # Normal: apply at attempt 2
                (attempt == 3 and flask_fix_applied)  # If Flask fix didn't work, apply at attempt 3
            )

            if should_apply_nuclear and (error_info['errors'] or error_info['failures']):
                logger.warning("⚠️ Attempt {} failed. Trying aggressive fallback: testing each function individually".format(attempt))
