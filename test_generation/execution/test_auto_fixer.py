            ...
            if (
                ...
                for i, line in enumerate(lines):
                    ...
                    if pass_match:
                        ...
                        # Keep FULL test identifier with class name for matching
                        # Format: file.py::Class::test_name or file.py::test_name
...
"""
    ...
    def keep_only_passing_tests(
        ...
        if language == "python":
            return self._keep_only_passing_tests_python(
                test_file_path, initial_passing_tests, failing_test_bases
            )
        elif language == "go":
    ...
    def _keep_only_passing_tests_python(
        ...
        # Initialize failing_test_bases if not provided
        if failing_test_bases is None:
            failing_test_bases = set()
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # CRITICAL: For parametrized tests, ALL variants must pass or we remove the entire test
                passing_tests = []
                failing_tests = []

                # First, build a map of test base names to all their passing AND failing variants
                test_variants = {}  # {base_name: {'passing': set(), 'failing': set()}}

                for passing in initial_passing_tests:
                    # Extract test name after file path
                    if "::" in passing:
                        parts = passing.split("::")
                        if len(parts) >= 2:
                    ...
                    # Get base name (without parametrization)
                    base_name = test_name.split("[")[0]

                    if base_name not in test_variants:
                        test_variants[base_name] = {'passing': set(), 'failing': set()}

                    # Store the full variant name (with params if any)
                    test_variants[base_name]['passing'].add(test_name)

                # Now check each test function
                for t in test_functions:
                    # Extract full test name with class (if present)
                    # t looks like: "TestClass::test_method" or "test_function"
                    file_test_name = t.split("[")[0]  # Remove params

                    # CRITICAL: Check if this test has ANY failing variants
                    if file_test_name in failing_test_bases:
                        # This test has at least one failing variant - remove it entirely
                        failing_tests.append(t)
                        logger.info(f"  ❌ REMOVE: {t} (has failing variant)")
                    elif file_test_name in test_variants:
                        # This test has passing variants and NO failing variants
                        passing_tests.append(t)
                        logger.info(f"  ✅ KEEP: {t} (all variants pass)")
                    else:
                        # No passing variants found
                        failing_tests.append(t)
                        logger.info(f"  ❌ REMOVE: {t} (no passing variants)")
...
"""
    ...
    def auto_fix_test(
        ...
        if self.max_retries == 0:
            ...
            passing_tests = error_info.get("passing", [])
            failing_tests_list = error_info.get("failures", [])

            logger.info(
                f"📊 Test results: {len(passing_tests)} passing, "
                f"{len(failing_tests_list)} failing"
            )
            ...
            if passing_tests:
                ...
                # CRITICAL: For parametrized tests, we need to know about ALL variants
                # Build a set of failing test base names
                failing_test_bases = set()
                logger.info(f"🔍 Processing {len(failing_tests_list)} failures...")
                for failure in failing_tests_list:
                    logger.info(f"  Failure type: {type(failure)}, content: {failure}")
                    if isinstance(failure, dict) and 'test_name' in failure:
                        test_name = failure['test_name']
                        logger.info(f"  Raw test_name: {test_name}")
                        # Extract just the test path (file::class::method or file::method)
                        # Example: "FAILED routes/reactions_test.py::TestClass::test_method[param]"
                        if "::" in test_name:
                            # Remove "FAILED " prefix if present
                            test_name = test_name.replace("FAILED ", "").strip()
                            # Also remove trailing status (FAILED, ERROR, etc)
                            if " FAILED" in test_name:
                                test_name = test_name.split(" FAILED")[0].strip()
                            if " ERROR" in test_name:
                                test_name = test_name.split(" ERROR")[0].strip()

                            logger.info(f"  Cleaned test_name: {test_name}")
                            # Split by :: and get everything after file path
                            parts = test_name.split("::")
                            if len(parts) >= 2:
                                # Get class::method or just method
                                base_with_class = "::".join(parts[1:])
                                # Remove parametrization
                                base_name = base_with_class.split("[")[0]
                                failing_test_bases.add(base_name)
                                logger.info(f"  Added to failing bases: {base_name}")

                logger.info(f"🔍 Final failing test base names: {failing_test_bases}")

                cleaned_code = self.keep_only_passing_tests(
                    test_file_path, language, initial_passing_tests=passing_tests,
                    failing_test_bases=failing_test_bases
                )
