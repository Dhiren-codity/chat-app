...
"""
    ...
    def _fix_duplicate_parametrization(self, test_code: str) -> str:
        ...
        Strategy:
        1. Find test functions with multiple parametrize decorators
        2. Check if any parameter names are duplicated
        3. Keep only the first occurrence, remove duplicates
        ...
        while i < len(lines):
            ...
            if stripped.startswith("def test"):
                ...
                # Collect all decorators above this function
                while j >= 0:
                    ...
                    if "@pytest.mark.parametrize" in decorator:
                        ...
                        if match:
                            ...
                            if param_name in seen_params:
                                # Duplicate found!
                                logger.warning(f"  🗑️ Removing duplicate parametrize '{param_name}'")
                                duplicate_indices.append(decorator_lines[idx])
                            else:
                                seen_params.add(param_name)

                # Add all lines except duplicates
                for line_idx in range(len(fixed_lines), i):
                    if line_idx not in duplicate_indices:
