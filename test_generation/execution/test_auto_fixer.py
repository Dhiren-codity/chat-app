            ...
            if attempt == 1 and language == "python":
                ...
                if is_dependency_error:
                    ...
                    if "ModuleNotFoundError:" in combined_output:
                        ...
                        # Handle real missing dependencies (mock them)
                        if real_missing_deps:
                            logger.warning(
                                f"⚠️ Real missing dependencies detected: {real_missing_deps}\n"
                                "   Adding sys.modules mocking to allow tests to run..."
                            )
                            current_code = self._add_sys_modules_mocking(current_code, real_missing_deps)

                        # If we made any changes, write and re-run tests
                        if fixable_bad_modules or real_missing_deps:
                            ...
                            if success:
                                ...
                                if real_missing_deps:
                                    fix_msg.append(f"mocked missing deps: {real_missing_deps}")
