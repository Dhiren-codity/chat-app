...
"""
    ...
    def _remove_problematic_fixtures(self, test_code: str) -> str:
        ...
        while i < len(lines):
            ...
            # Detect client fixture
            if stripped == "@pytest.fixture" and i + 1 < len(lines):
                next_line_stripped = lines[i + 1].strip()
                # Check for client fixture
                if next_line_stripped.startswith("def client("):
                    # Found a client fixture - check if it's problematic
                    is_problematic = False
                    ...
                    if line_indent > 0:
                        is_problematic = True
                        logger.info(f"   🔧 Removing client fixture inside class at line {i + 1}")

                    # Check 2: Does it have 'self' parameter? (inside class)
                    if "self" in next_line_stripped:
                        is_problematic = True
                        logger.info(f"   🔧 Removing client fixture with 'self' at line {i + 1}")
                    ...
                    if is_problematic:
                        ...
                        # Skip from i to j-1
                        skip_until_line = j - 1
                        logger.info(f"      Skipping lines {i + 1} to {skip_until_line + 1}")
                        i += 1
                        continue
...
"""
    ...
    def auto_fix_test(
        ...
        if self.max_retries > 0:
            ...
            logger.info("🔍 Removing problematic fixtures only (nuclear mode)")
            test_code = self._remove_hallucinated_imports(test_code)
            test_code = self._remove_problematic_fixtures(test_code)
