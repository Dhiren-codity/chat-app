...
"""
        ...
        try:
            ...
            try:
                ...
                while lines and lines[-1].strip().startswith("@"):
                    logger.warning(f"   Removing orphaned decorator at end: {lines[-1].strip()}")
                    lines.pop()
                cleaned_code = "\n".join(lines) + "\n"
                ...
                for i, line in enumerate(lines):
                    ...
                    if stripped.startswith("@") and not line[0].isspace():
                        ...
                        if not has_function_after:
                            logger.warning(f"   Removing orphaned decorator: {stripped}")
                            continue
                    fixed_lines.append(line)
                cleaned_code = "\n".join(fixed_lines) + "\n"

                # Fix 4: Add 'pass' to empty function bodies
                lines = cleaned_code.split("\n")
                fixed_lines = []
                i = 0
                while i < len(lines):
                    ...
                    if stripped.startswith("def test"):
                        ...
                        if not has_body:
                            logger.warning(f"   Adding 'pass' to empty function: {stripped[:60]}")
                ...
                # Fix 5: Remove duplicate @pytest.mark.parametrize decorators
                lines = cleaned_code.split("\n")
                fixed_lines = []
                seen_params = {}
                ...
                for i, line in enumerate(lines):
                    ...
                    if stripped.startswith("def test"):
                        ...
                        seen_params = {}
                        fixed_lines.append(line)
                    elif stripped.startswith("@pytest.mark.parametrize"):
                        if current_function or (i + 1 < len(lines) and lines[i + 1].strip().startswith("def test")):
                            match = re.search(r'@pytest\.mark\.parametrize\(\s*["\'](\w+)', stripped)
                            if match:
                                param_name = match.group(1)

                                if param_name in seen_params:
                                    logger.warning(f"   Removing duplicate parametrize for '{param_name}' on {current_function or 'next function'}")
                                    continue
                                else:
                        ...
                        if stripped.startswith("class ") or (stripped and not stripped.startswith("@") and not line[0].isspace()):
                            seen_params = {}
                            current_function = None
                        fixed_lines.append(line)
                ...
                try:
                    ...
                    logger.warning("   Simple fixes didn't resolve all issues - returning code anyway")
            ...
            remaining_test_count = len(re.findall(r"^\s*def test\w+", cleaned_code, re.MULTILINE))
            if remaining_test_count == 0:
                logger.warning("⚠️ No test functions remain after cleanup")
                logger.warning("   This is unusual - check if matching logic worked correctly")
            else:
    ...
    def _fix_file_structure(self, test_code: str) -> str:
        ...
        for line in cleaned_lines:
            ...
            if (stripped.startswith('"""') or stripped.startswith("'''")) and not found_code:
