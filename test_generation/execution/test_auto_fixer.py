...
"""
    ...
    def _remove_bad_imports_from_code(self, test_code: str, context: Optional[str] = None) -> str:
        """
            ...
            if (stripped.startswith("from ") or stripped.startswith("import ")) and not stripped.startswith("#"):
                ...
                if import_name:
                    ...
                    if is_real:
                        ...
                        logger.info(f"  🗑️ Removing hallucinated import: {stripped[:100]}")
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            try:
                ...
                logger.warning("⚠️ Falling back to minimal valid file due to syntax error")
                return self._create_minimal_valid_file(test_code)
    ...
    def _create_minimal_valid_file(self, test_code: str) -> str:
        ...
        """
        logger.info("🧹 Creating minimal test file (essential imports + placeholder only)...")
        logger.info(f"   Original code had {len(test_code)} characters - discarding to avoid hallucinated imports")
        ...
        if self.max_retries == 0:
            ...
            if passing_tests and has_test_functions:
                ...
                cleaned_code = self.keep_only_passing_tests(
                    test_file_path, language, initial_passing_tests=passing_tests, context=context
                )
