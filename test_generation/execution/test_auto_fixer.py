...
"""
    ...
    def auto_fix_test(
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            # CHECK FOR EARLY NUCLEAR OPTION: If coverage is good (>50%), skip LLM fixes and go straight to nuclear option
            ...
            if total_tests > 0:
                ...
                # If we have >50% pass rate, trigger nuclear option immediately (skip LLM attempts)
                if pass_rate > 0.5 and len(passing_tests) > 0:
                    logger.warning(
                        f"⚡ EARLY NUCLEAR OPTION: {pass_rate*100:.1f}% tests passing ({len(passing_tests)}/{total_tests})"
                    )
                    logger.warning(
                        "   Skipping LLM fixes - keeping passing tests and removing failing ones"
                    )
                    ...
                    if cleaned_code and cleaned_code != current_code:
                        ...
                        logger.info("✅ Created cleaned version with passing tests only - testing now")
                        Path(test_file_path).write_text(current_code, encoding="utf-8")
                        success, stdout, stderr = self.run_tests_and_capture_errors(
                            test_file_path, language
                        )
                        if success:
                            logger.info(f"✅ Early nuclear option succeeded! Kept {len(passing_tests)} passing tests")
                            fix_history.append(f"Attempt {attempt}: Early nuclear option SUCCESS (kept {len(passing_tests)} tests)")
                            return True, current_code, fix_history
                        else:
                            logger.warning("❌ Early nuclear option still has errors - will continue with normal flow")
                    else:
                        logger.warning("⚠️ Early nuclear option failed or didn't change code")
            ...
            if attempt >= self.max_retries:
                ...
                passing_tests = error_info.get("passing", [])
                if passing_tests:
                    logger.info(f"✅ {len(passing_tests)} tests passed: {passing_tests}")

                cleaned_code = self.keep_only_passing_tests(test_file_path, language, initial_passing_tests=passing_tests)

                if cleaned_code and cleaned_code != current_code:
                    current_code = cleaned_code
                    logger.info("✅ Created cleaned version - testing now")
                    Path(test_file_path).write_text(current_code, encoding="utf-8")
                    success, stdout, stderr = self.run_tests_and_capture_errors(
                        test_file_path, language
                    )
                    if success:
                        ...
                        logger.warning("❌ Nuclear option still has errors")
                else:
