    ...
    def _keep_only_passing_tests_python(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # CRITICAL: If we matched 0 tests but had passing tests initially,
                # it's likely a matching logic bug, not that all tests failed
                # Better to keep all tests than delete everything
                if len(passing_tests) == 0 and len(initial_passing_tests) > 0:
                    ...
                    logger.warning(f"⚠️ SAFEGUARD: Keeping ALL {len(test_functions)} tests to avoid data loss")
                    # Keep all test functions as passing (safer than deleting everything)
                    passing_tests = test_functions.copy()
                    failing_tests = []
            else:
