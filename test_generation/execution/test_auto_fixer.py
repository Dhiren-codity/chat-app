    ...
    def auto_fix_test(
        ...
        if self.max_retries == 0:
            ...
            if has_collection_errors:
                ...
                if success_repair and fixed_code != current_code:
                    logger.info("🔧 Applied AST-based syntax fixes, testing again...")
                    Path(test_file_path).write_text(fixed_code, encoding="utf-8")
                    success, stdout, stderr = self.run_tests_and_capture_errors(
                        test_file_path, language
                    )
                    if success:
                        logger.info("✅ AST-based syntax fixes resolved all issues!")
                        return True, fixed_code, ["AST-based syntax errors fixed"]
                    else:
