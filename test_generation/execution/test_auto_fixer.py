...
"""
    ...
    def _create_basic_go_test_file(self, test_code: str, test_file_path: str = None) -> str:
        ...
        The test file may have invalid package names like "package go-services/file.go"
        """
        ...
        # CRITICAL FIX: Extract package name from SOURCE file, not test file
        # The test file may have invalid package name like "package go-services/message_validator_test.go"
        package_name = "main"  # Default fallback

        if test_file_path:
            # Derive source file path from test file path
            # message_validator_test.go -> message_validator.go
            test_path = Path(test_file_path)
                        ...
                        if stripped.startswith("package "):
                            ...
                            if "/" not in potential_pkg and " " not in potential_pkg and potential_pkg:
                                if potential_pkg.replace("_", "").isalnum():
                                    ...
                                    logger.info(f"   ✅ Extracted package name from source: {package_name}")
                                    break
                            break
                except Exception as e:
