    ...
    def _keep_only_passing_tests_python(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # NO SANITY CHECK - let nuclear option do its job
                # If we matched 0 tests, warn but continue to the cleanup logic below
                # Don't short-circuit here - let the code proceed to actually remove failing tests
                if len(passing_tests) == 0:
                    logger.warning(f"⚠️ No passing tests matched from initial run")
                    logger.warning(f"⚠️ Will proceed with test cleanup anyway")
            else:
