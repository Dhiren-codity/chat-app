...
"""
    ...
    def _create_minimal_valid_file(self, test_code: str) -> str:
        """
        Create a minimal valid test file with a skip marker instead of placeholder tests.
        This marks the test file as needing manual review rather than creating fake passing tests.

        Args:
            test_code: Original test code (kept for signature compatibility, not used to avoid hallucinated imports)
        """
        logger.info(
            "🧹 Creating minimal test file with skip marker (all tests failed)..."
        )
        logger.info(
            f"   Original code had {len(test_code)} characters - all tests failed validation"
        )

        # Create a single skip test instead of fake passing tests
        skip_lines = []

        # Add only pytest import (essential for test discovery)
        skip_lines.append("import pytest")
        skip_lines.append("")

        # Add a skip test instead of placeholder
        skip_lines.append("@pytest.mark.skip(reason='All original tests failed during auto-fix - needs manual review')")
        skip_lines.append("def test_needs_manual_review():")
        skip_lines.append('    """')
        skip_lines.append("    All tests in this file failed during automated fixing.")
        skip_lines.append("    Possible reasons:")
        skip_lines.append("    - Collection errors (import/syntax issues)")
        skip_lines.append("    - All tests had assertion failures")
        skip_lines.append("    - Tests used non-existent imports")
        skip_lines.append('    """')
        skip_lines.append("    pass")
    ...
    def auto_fix_test(
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt >= self.max_retries:
                ...
                # CRITICAL: If no passing tests, mark file with skip instead of fake passing tests
                logger.info("🔧 Creating skip marker for failed tests...")
                skip_code = '''"""
...
'''
                ...
                current_code = skip_code
                Path(test_file_path).write_text(current_code, encoding="utf-8")
                logger.info("✅ Skip marker created (not fake passing tests)")
                logger.info("   File marked for manual review")
                fix_history.append(
                    f"Attempt {attempt}: Nuclear option - marked for manual review (all original tests failed)"
                )
                return True, current_code, fix_history
