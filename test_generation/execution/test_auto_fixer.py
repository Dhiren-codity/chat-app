    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # SANITY CHECK: Only warn if we're keeping very few tests (less than 10% of original)
                # This allows the nuclear option to work while still catching major issues
                if len(passing_tests) < max(1, len(initial_passing_tests) * 0.1):
                    logger.warning(
                        f"⚠️ SANITY CHECK: Only keeping {len(passing_tests)} tests out of {len(initial_passing_tests)}"
                    )
                    logger.warning(
                        f"⚠️ This might indicate a parametrized test matching issue, but proceeding anyway..."
                    )
                    # Don't return here - let the nuclear option proceed
                else:
                    logger.info(
                        f"✅ SANITY CHECK PASSED: Keeping {len(passing_tests)}/{len(initial_passing_tests)} tests"
                    )
            else:
