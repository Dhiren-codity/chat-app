    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            if (
                ...
                # SIMPLIFIED: Also match tests without :: (short format)
                pass_pattern = re.compile(r"([\w:.\/\[\],-]+)\s+PASSED")
                fail_inline_pattern = re.compile(r"([\w:.\/\[\],-]+)\s+(FAILED|ERROR)")
                ...
                for i, line in enumerate(lines):
                    ...
                    if fail_inline_match:
                        ...
                        logger.info(f"  ❌ Found failing test: {test_name} (from: {test_path})")
                        ...
                        if fail_summary_match:
                            ...
                            # Special case: Collection errors with "Failed: In test_name:" pattern
                            # Example: "ERROR file.py - Failed: In test_func: function uses no argument 'param'"
                            collection_error_match = re.search(r'Failed:\s+In\s+(test_\w+):', line)
                            if collection_error_match:
                                test_name = collection_error_match.group(1)
                                logger.info(f"  ❌ Found failing test (collection error): {test_name}")
                                error_info["failures"].append(
                                    {
                                        "test_name": test_name,
                                        "error_type": "CollectionError",
                                ...
                                # Fallback - try to extract test name from line
                                test_name = line.strip()
                                if "::" in line:
                                    parts = line.split("::")
                                    if len(parts) >= 2:
                                        # Extract test name, remove status words
                                        test_name = parts[-1].split()[0].strip()
                                logger.info(f"  ❌ Found failing test (fallback): {test_name}")
                                error_info["failures"].append(
                                    {
                                        "test_name": test_name,
                                        "context": "\n".join(
...
"""
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # Build a set of passing test base names and parametrizations
                passing_base_names = set()
                passing_with_params = {}  # {base_name: [param1, param2, ...]}

                for passing in initial_passing_tests:
                    base_name = passing.split("[")[0]
                    passing_base_names.add(base_name)
                    if "[" in passing:
                        # This is a parametrized test
                        if base_name not in passing_with_params:
                            passing_with_params[base_name] = []
                        passing_with_params[base_name].append(passing)
                ...
                for t in test_functions:
                    ...
                    # Check if this function is parametrized in the test results
                    if file_func_name in passing_with_params:
                        # This is a parametrized test - we know which params pass/fail
                        # For nuclear option, if ANY param fails, remove entire function
                        # (More precise param removal would require AST manipulation)
                        logger.warning(
                            f"  ⚠️ Parametrized test {file_func_name} has {len(passing_with_params[file_func_name])} passing cases"
                        )
                        logger.warning(
                            "     For nuclear option safety, we remove entire function if ANY param fails"
                        )
                        # Check if ALL parametrizations passed by comparing against test count
                        # If any failed, this function won't be in passing list for all params
                        matched = file_func_name in passing_base_names
                    else:
                        # Non-parametrized test - simple match
                        matched = file_func_name in passing_base_names

                    if matched:
                        passing_tests.append(t)
                        logger.info(f"  ✅ KEEP: {t}")
                    else:
                        failing_tests.append(t)
                        logger.info(f"  ❌ REMOVE: {t} (failed or not in passing list)")
            ...
            # Extract just the function names (without Class:: prefix and without [params])
            failing_function_names = set()
            for test_name in failing_tests:
                # Extract base function name: "TestClass::test_foo[param]" -> "test_foo"
                base_name = test_name
                if "::" in base_name:
                    base_name = base_name.split("::")[-1]
                if "[" in base_name:
                    base_name = base_name.split("[")[0]
                failing_function_names.add(base_name)

            logger.info(f"🗑️ Removing functions (base names): {failing_function_names}")
    ...
    def _fix_duplicate_parametrization(self, test_code: str, error_output: str = None) -> str:
        ...
        3. Fallback: Parse pytest error message to extract duplicate param name

        Args:
            test_code: Test code with duplicate parametrization
            error_output: Optional pytest error output for fallback parsing

        Returns:
            Fixed test code with duplicates removed
        """
        logger.info("🔧 Fixing duplicate parametrization decorators...")
        logger.info(f"   Test code length: {len(test_code)} chars, {len(test_code.split(chr(10)))} lines")

        # Save test code to temp file for debugging if fix fails
        debug_file = tempfile.NamedTemporaryFile(mode='w', suffix='_debug_dup_param.py', delete=False, encoding='utf-8')
        debug_file.write(test_code)
        debug_file.close()
        logger.info(f"   Debug: Saved test code to {debug_file.name}")
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith("def test"):
                test_func_name = stripped.split("(")[0].replace("def ", "")
                logger.info(f"  🔍 Scanning decorators for {test_func_name} at line {i + 1}")
                ...
                while j >= 0 and j >= i - 50:
                    ...
                    if decorator_line.startswith("@"):
                        ...
                        full_decorator = decorator_line
                        start_line_idx = j

                        # If decorator doesn't end with ), it's multi-line
                        if not decorator_line.endswith(")"):
                            k = j + 1
                            while k < i and k < j + 20:
                                next_line = lines[k].strip()
                                if next_line:
                                    full_decorator += " " + next_line
                                    if next_line.endswith(")"):
                                        break
                                k += 1

                        decorators.insert(0, full_decorator)
                        decorator_line_indices.insert(0, start_line_idx)
                        logger.info(f"    Found decorator at line {start_line_idx + 1}: {full_decorator[:80]}")
                        j -= 1
                ...
                if not decorators:
                    logger.info(f"    No decorators found for {test_func_name}")
                    continue

                # Check for duplicate parametrize decorators
                seen_params = set()
                duplicate_found_for_this_func = False
                ...
                for idx, decorator in enumerate(decorators):
                    if "@pytest.mark.parametrize" in decorator:
                        ...
                        if match:
                            ...
                            logger.info(f"      Parametrize decorator: {param_name}")
                            logger.debug(f"        Full decorator: {decorator}")

                            if param_name in seen_params:
                                # Duplicate found! Mark this line and any continuations for removal
                                line_idx = decorator_line_indices[idx]
                                logger.warning(
                                    f"  🗑️ Line {line_idx + 1}: DUPLICATE parametrize '{param_name}' on {test_func_name}"
                                )
                                lines_to_skip.add(line_idx)
                                duplicate_found_for_this_func = True

                                # Handle multi-line decorators (lines that don't end with ))
                                # Scan forward from decorator line to find all continuation lines
                                check_idx = line_idx
                                while check_idx < len(lines) and check_idx < line_idx + 20:
                                    check_line = lines[check_idx]
                                    stripped_check = check_line.rstrip()

                                    # If line doesn't end with ), it might continue
                                    if "@pytest.mark.parametrize" in check_line:
                                        # This is the start line, check if it ends properly
                                        if not stripped_check.endswith(")"):
                                            # Multi-line decorator - mark next lines
                                            next_idx = check_idx + 1
                                            while next_idx < len(lines):
                                                next_line = lines[next_idx].rstrip()
                                                lines_to_skip.add(next_idx)
                                                if next_line.endswith(")"):
                                                    break
                                                next_idx += 1
                                        break
                            ...
                            logger.warning(f"      Could not extract param name from: {decorator[:80]}")

                if duplicate_found_for_this_func:
                    logger.info(f"    ✅ Found and marked duplicates for {test_func_name}")
        ...
        if lines_to_skip:
            ...
            logger.warning("   No duplicate parametrization found via decorator scanning")

            # FALLBACK: Try to parse pytest error message to extract duplicate param name
            if error_output:
                logger.info("   Attempting fallback: parsing pytest error message...")
                # Look for: "duplicate parametrization of 'group_name'"
                dup_match = re.search(r"duplicate parametrization of ['\"]([^'\"]+)['\"]", error_output, re.IGNORECASE)
                if dup_match:
                    dup_param_name = dup_match.group(1)
                    logger.info(f"   Found duplicate param name from error: '{dup_param_name}'")

                    # IMPROVED: Scan for @pytest.mark.parametrize and check next N lines for param name
                    # This handles multi-line decorators where decorator and param are on different lines

                    # EXTENSIVE DEBUG LOGGING
                    logger.info("   📊 Fallback scan stats:")
                    logger.info(f"      Total lines in file: {len(lines)}")
                    logger.info(f"      Searching for param name: '{dup_param_name}'")
                    logger.info(f"      Looking for patterns: \"{dup_param_name}\" or '{dup_param_name}'")

                    # PASS 1: Find all decorator blocks with this param name
                    decorator_blocks_to_remove = set()  # Set of line indices to skip
                    ...
                    while i < len(lines):
                        ...
                        # Found a @pytest.mark.parametrize decorator
                        if '@pytest.mark.parametrize' in lines[i]:
                            decorators_found += 1
                            logger.info(f"   🔍 Found @pytest.mark.parametrize at line {i+1}")
                            logger.info(f"      Line content: {lines[i][:100]}")

                            # Scan this line and next 5 lines for the param name
                            param_found = False
                            decorator_end_line = i

                            # Build full decorator by scanning until closing paren
                            logger.info(f"      Scanning next 10 lines for param '{dup_param_name}':")
                            for j in range(i, min(i + 10, len(lines))):
                                line_text = lines[j]
                                logger.info(f"        Line {j+1}: {line_text[:100]}")

                                # Check if param name is in this line
                                has_double_quote = f'"{dup_param_name}"' in line_text
                                has_single_quote = f"'{dup_param_name}'" in line_text
                                if has_double_quote or has_single_quote:
                                    param_found = True
                                    logger.info(f"        ✅ FOUND param '{dup_param_name}' at line {j+1} (double={has_double_quote}, single={has_single_quote})")

                                # Find decorator end (line with closing paren)
                                if ')' in line_text:
                                    decorator_end_line = j
                                    logger.info(f"        End of decorator at line {j+1}")
                                    break
                            ...
                            if param_found:
                                if first_occurrence_line is None:
                                    ...
                                    logger.info(f"   Keeping first occurrence at lines {i+1}-{decorator_end_line+1}")
                                else:
                                    # This is a duplicate - mark for removal
                                    logger.warning(f"   Marking duplicate for removal at lines {i+1}-{decorator_end_line+1}")
                                    for k in range(i, decorator_end_line + 1):
                                        decorator_blocks_to_remove.add(k)

                                # Skip past this decorator
                                i = decorator_end_line + 1
                                continue
                    ...
                    logger.info(f"      Total @pytest.mark.parametrize decorators found: {decorators_found}")
                    logger.info(f"      Decorators with param '{dup_param_name}': {1 if first_occurrence_line is not None else 0}")
                    logger.info(f"      Lines marked for removal: {len(decorator_blocks_to_remove)}")

                    # PASS 2: Build result excluding marked lines
                    if decorator_blocks_to_remove:
                        result_lines = []
                        for i, line in enumerate(lines):
                            if i not in decorator_blocks_to_remove:
                                result_lines.append(line)
                            else:
                                logger.info(f"     Skipping line {i+1}: {line.strip()[:60]}")

                        result = "\n".join(result_lines)
                        logger.info(f"✅ Fixed duplicate parametrization via fallback (removed {len(decorator_blocks_to_remove)} lines with '{dup_param_name}')")
                    elif first_occurrence_line is not None:
                        # Found param but no duplicates
                        logger.info(f"   Found '{dup_param_name}' at line {first_occurrence_line+1} but no duplicates")
                        result = test_code
                    else:
                        logger.warning(f"   Could not find any @pytest.mark.parametrize with '{dup_param_name}'")
                        result = test_code
                else:
                    logger.warning("   Could not extract duplicate param name from error output")
                    result = test_code
            else:
                result = test_code
    ...
    def _scan_and_remove_bad_imports(self, test_code: str) -> str:
        """
        Scan for and remove bad test-related imports (test_sample, test_helpers, etc.)
        that don't exist and cause ModuleNotFoundError.

        This runs BEFORE any tests to prevent collection errors.
        ...
        for line in lines:
            ...
            if stripped.startswith('from ') or stripped.startswith('import '):
                ...
                # CRITICAL: Whitelist legitimate test libraries first
                if module_name:
                    ...
                    # Check if this is a legitimate library (exact match or starts with legitimate.)
                    is_legitimate = (
                    ...
                    if not is_legitimate:
                        # Only check for bad patterns if not a legitimate library
                        is_bad_import = (
        ...
        if removed_imports:
            logger.info(f"✅ Pre-scan removed {len(removed_imports)} bad import(s)")
            return '\n'.join(cleaned_lines)
        else:
            logger.info("  No bad imports found in pre-scan")
            return test_code
    ...
    def _remove_orphaned_decorators(self, test_code: str) -> str:
        """
        Remove orphaned @pytest decorator lines that aren't followed by a test function.
        This can happen after nuclear option removes test functions but leaves decorators.

        Uses parenthesis tracking to handle multi-line decorators properly.

        Args:
            test_code: Test code that may contain orphaned decorators

        Returns:
            Test code with orphaned decorators removed
        """
        lines = test_code.split("\n")
        result_lines = []
        i = 0
        removed_decorator_count = 0

        while i < len(lines):
            line = lines[i]
            stripped = line.strip()

            # Check if this is a @pytest.mark.parametrize decorator (NOT @pytest.fixture!)
            # @pytest.fixture is legitimate and should never be removed
            # Only remove orphaned parametrize decorators that remain after nuclear option
            if stripped.startswith("@pytest.mark.parametrize"):
                decorator_start = i
                j = i
                paren_depth = 0
                decorator_lines_range = []

                # Track parentheses to find complete decorator (including multi-line)
                while j < len(lines):
                    current_line = lines[j]
                    decorator_lines_range.append(j)

                    # Count parentheses on this line
                    paren_depth += current_line.count('(') - current_line.count(')')

                    # If we started with decorator and parens are balanced, decorator is complete
                    if j > i and paren_depth == 0:
                        j += 1
                        break

                    j += 1

                    # Safety: don't scan more than 50 lines for one decorator
                    if j - i > 50:
                        logger.warning(f"  ⚠️ Decorator at line {i+1} spans >50 lines, assuming incomplete")
                        break

                # Now find what follows (skip empty lines, comments, other decorators)
                k = j
                subsequent_decorator_lines = []
                ...
                while k < len(lines):
                    ...
                    if not next_stripped or next_stripped.startswith("#"):
                        # Empty or comment - skip over it
                        subsequent_decorator_lines.append(k)
                        k += 1
                    elif next_stripped.startswith("@"):
                        # Another decorator - will be handled in next iteration
                        break
                    else:
                        # Found non-decorator, non-empty line
                        break

                # Check if followed by a function definition
                if k < len(lines):
                    next_line = lines[k]
                    next_stripped = next_line.strip()

                    if next_stripped.startswith("def "):
                        # Valid decorator followed by function - keep all
                        for idx in decorator_lines_range:
                            result_lines.append(lines[idx])
                        for idx in subsequent_decorator_lines:
                            result_lines.append(lines[idx])
                        result_lines.append(next_line)
                        i = k + 1
                    else:
                        # Orphaned decorator - not followed by function
                        removed_decorator_count += 1
                        logger.warning(f"  🧹 Removing orphaned decorator (lines {decorator_start+1}-{j}): {stripped[:60]}")
                        logger.warning(f"     Not followed by function (next: {next_stripped[:60]})")
                        # Skip ALL decorator lines including data
                        i = j
                else:
                    # Decorator at end of file - orphaned
                    removed_decorator_count += 1
                    logger.warning(f"  🧹 Removing orphaned decorator at EOF (lines {decorator_start+1}-{j}): {stripped[:60]}")
                    i = j
            else:
                # Not a decorator, keep the line
                result_lines.append(line)
                i += 1

        cleaned_code = "\n".join(result_lines)

        if removed_decorator_count > 0:
            logger.info(f"✅ Removed {removed_decorator_count} orphaned decorator block(s)")
...
"""
    ...
    def auto_fix_test(
        ...
        # NOTE: Pre-scan for bad imports now happens in auto_fix_test_file() before this is called
        # This ensures it runs before the initial test execution in GitHub Actions
            ...
            if attempt == 1 and language == "python":
                ...
                if is_dependency_error:
                    ...
                    if "ModuleNotFoundError:" in combined_output:
                        ...
                        # Pattern: starts with "test" or ends with "test", or contains "fixture", "helper", "mock_", "sample"
                        fixable_bad_modules = []
                        for bad_mod in bad_modules:
                            ...
                            is_test_related = (
                                ...
                                bad_mod.endswith('test') or
                                'fixture' in bad_mod.lower() or
                                'helper' in bad_mod.lower() or
                                'mock_' in bad_mod.lower() or
                                'sample' in bad_mod.lower() or
                                bad_mod == 'conftest'
                            )
                            if is_test_related:
                                fixable_bad_modules.append(bad_mod)
                                logger.info(f"  ✓ Identified as test-related fake import: {bad_mod}")
                            else:
                                logger.warning(f"  ✗ Not test-related (likely real dependency): {bad_mod}")

                        if fixable_bad_modules:
                            logger.info(f"🔧 Attempting to remove {len(fixable_bad_modules)} test-related fake import(s): {fixable_bad_modules}")
                            ...
                            for line in lines:
                                ...
                                for bad_mod in fixable_bad_modules:
                                    ...
                                    if (f"from {bad_mod} import" in line.lower() or
                                        f"import {bad_mod}" in line.lower() or
                                        f"from {bad_mod}." in line.lower()):
                                        has_bad_import = True
                                        removed_imports.append(stripped)
                            ...
                            if removed_imports:
                                ...
                                # Add unittest.mock import if not present (common replacement)
                                if 'from unittest.mock import' not in fixed_code:
                                    # Find the last import line to add after it
                                    lines_with_mock = []
                                    added_mock = False
                                    for line in fixed_code.split('\n'):
                                        lines_with_mock.append(line)
                                        if not added_mock and line.strip().startswith(('import ', 'from ')):
                                            # After this import, we'll add mock import
                                            pass
                                        elif not added_mock and line.strip() and not line.strip().startswith('#'):
                                            # First non-import, non-comment line - add mock before it
                                            lines_with_mock.insert(-1, 'from unittest.mock import Mock, MagicMock, patch')
                                            added_mock = True
                                            logger.info("  ➕ Added: from unittest.mock import Mock, MagicMock, patch")

                                    if added_mock:
                                        fixed_code = '\n'.join(lines_with_mock)

                                Path(test_file_path).write_text(fixed_code, encoding='utf-8')
                                logger.info(f"✅ Removed {len(removed_imports)} bad import line(s)")

                                # Re-run tests to see if this fixed the issue
                                logger.info("🔄 Re-running tests after removing bad imports...")
                                success, stdout, stderr = self.run_tests_and_capture_errors(
                                    test_file_path, language
                                )

                                if success:
                                    logger.info("✅ Tests pass after removing bad imports!")
                                    fix_history.append(
                                        f"Attempt {attempt}: Fixed by removing bad imports: {fixable_bad_modules}"
                                    )
                                    return True, fixed_code, fix_history
                                else:
                            ...
                            # Not a fixable bad import - likely a real missing dependency
                            logger.error(
                                f"❌ ModuleNotFoundError for modules: {bad_modules}\n"
                                "   These appear to be real missing dependencies, not test code errors.\n"
                ...
                if cleaned_code and cleaned_code != current_code:
                    ...
                    if remaining_tests > 0:
                        ...
                        if not verify_success:
                            ...
                            # CRITICAL: Only get failures (actual test failures), NOT errors (collection errors)
                            # Collection errors don't have proper test_name fields
                            failing_tests_raw = verify_error_info.get("failures", [])

                            # Filter to only valid test failures (must be dict with test_name field)
                            failing_tests = []
                            for f in failing_tests_raw:
                                if isinstance(f, dict) and "test_name" in f:
                                    test_name = f.get("test_name", "")
                                    # Validate it's a real test name (not error output)
                                    if test_name and isinstance(test_name, str) and ("test" in test_name.lower() or "::" in test_name):
                                        failing_tests.append(f)
                                    else:
                                        logger.warning(f"  ⚠️ Filtered out invalid test_name: {test_name[:100]}")
                                else:
                                    logger.warning(f"  ⚠️ Filtered out non-dict or missing test_name: {str(f)[:100]}")

                            logger.info(
                                f"📊 Verification found: {len(verify_error_info.get('passing', []))} passing, {len(failing_tests)} failing (after filtering)"
                            )
                            ...
                            if failing_tests:
                                ...
                                logger.warning("⚠️ verify_success=False but no valid failing tests extracted!")
                                logger.warning(f"   Raw failures count: {len(failing_tests_raw)}")
                                logger.warning(f"   Stdout (first 500 chars): {verify_stdout[:500]}")
                                logger.warning(f"   Stderr (first 500 chars): {verify_stderr[:500]}")

                            if failing_tests:
                                logger.info(
                                    f"🔄 Directly removing {len(failing_tests)} failing test functions"
                                )

                                # Extract base function names from failing tests
                                # Handle parametrized tests: "test_foo[param]" -> "test_foo"
                                failing_function_names = set()
                                for f in failing_tests:
                                    test_name = f.get("test_name", str(f))
                                    # Remove [params] suffix: "test_foo[param]" -> "test_foo"
                                    base_name = test_name.split("[")[0]
                                    # Remove file path prefix if present: "file.py::test_foo" -> "test_foo"
                                    if "::" in base_name:
                                        base_name = base_name.split("::")[-1]
                                    # Clean up any status indicators: "test_foo FAILED" -> "test_foo"
                                    base_name = base_name.split()[0].strip()

                                    if base_name.startswith("test"):
                                        failing_function_names.add(base_name)
                                        logger.info(f"  🗑️ Will remove: {base_name} (from: {test_name})")
                                    else:
                                        logger.warning(f"  ⚠️ Skipping invalid test name: {test_name}")

                                if failing_function_names:
                                    # Directly remove these functions using AST-based removal
                                    logger.info(f"🗑️ Removing {len(failing_function_names)} function(s): {failing_function_names}")
                                    cleaned_code_2 = self._remove_test_functions(
                                        current_code,
                                        failing_function_names
                                    )

                                    if cleaned_code_2:
                                        logger.info(
                                            f"📝 Removal complete (length: {len(current_code)} -> {len(cleaned_code_2)})"
                                        )

                                        # CRITICAL: Clean up orphaned decorators after function removal
                                        logger.info("🧹 Cleaning up orphaned decorators...")
                                        cleaned_code_2 = self._remove_orphaned_decorators(cleaned_code_2)
                                    else:
                                        logger.error("❌ _remove_test_functions() returned None!")
                                else:
                                    logger.warning("⚠️ No valid function names extracted from failing tests")
                                    cleaned_code_2 = None
...
def auto_fix_test_file(
    ...
    logger.info("🔍 Pre-scanning for bad test imports before initial test run...")
    fixer_temp = TestAutoFixer(llm_client=None, max_retries=0)  # Temporary instance for pre-scan
    test_code = fixer_temp._scan_and_remove_bad_imports(test_code)
    Path(test_file_path).write_text(test_code, encoding="utf-8")
    logger.info("✅ Pre-scan complete, test file updated")
