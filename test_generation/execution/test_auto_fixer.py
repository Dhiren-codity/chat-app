            ...
            if stripped == "@pytest.fixture" and i + 1 < len(lines):
                ...
                if next_line_stripped.startswith("def client("):
                    ...
                    if is_problematic:
                        ...
                        fixture_def_indent = len(lines[i + 1]) - len(lines[i + 1].lstrip())
                        j = i + 2  # Start after 'def client():'
                        while j < len(lines):
                            j_line = lines[j]
                            j_stripped = j_line.strip()
                            j_indent = len(j_line) - len(j_line.lstrip()) if j_stripped else 999

                            # Exit when we find a non-empty line at same or less indentation
                            if j_stripped and j_indent <= fixture_def_indent:
                                break
                            j += 1
