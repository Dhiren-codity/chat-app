...
"""
    ...
    def _add_missing_imports(self, test_code: str) -> str:
        """
        Add missing imports for commonly used functions in test code.
        Similar to python_test_gen._ensure_datetime_import() but for auto-fixing.
        """
        import re

        # Map of function/class patterns to their imports
        common_imports = [
            # datetime functions
            (r'\bdatetime\s*\(', 'from datetime import datetime', r'from\s+datetime\s+import\s+.*\bdatetime\b|^import\s+datetime$'),
            (r'\btimedelta\s*\(', 'from datetime import timedelta', r'from\s+datetime\s+import\s+.*\btimedelta\b'),
            (r'\bdate\s*\(', 'from datetime import date', r'from\s+datetime\s+import\s+.*\bdate\b'),
            # datetime.now() usage
            (r'datetime\.now\(\)', 'from datetime import datetime', r'from\s+datetime\s+import\s+.*\bdatetime\b|^import\s+datetime$'),

            # unittest.mock
            (r'\bMock\s*\(', 'from unittest.mock import Mock', r'from\s+unittest\.mock\s+import\s+.*\bMock\b'),
            (r'\bMagicMock\s*\(', 'from unittest.mock import MagicMock', r'from\s+unittest\.mock\s+import\s+.*\bMagicMock\b'),
            (r'@patch\s*\(|^\s*patch\s*\(', 'from unittest.mock import patch', r'from\s+unittest\.mock\s+import\s+.*\bpatch\b'),
        ]
        ...
        # Find existing imports and import section end
        ...
        # If no imports found, find where to insert (after docstring/comments)
        if import_section_end == 0:
            for idx, line in enumerate(lines):
                stripped = line.strip()
                if stripped and not stripped.startswith(('"""', "'''", '#')) or 'Auto-generated' in stripped:
                    import_section_end = idx
                    break

        # Check each pattern and add missing imports
        imports_to_add = []
        for usage_pattern, import_stmt, check_pattern in common_imports:
            # Check if the pattern is used in the code
            if re.search(usage_pattern, test_code, re.MULTILINE):
                # Check if it's already imported
                already_imported = any(re.search(check_pattern, imp, re.MULTILINE) for imp in existing_imports)

                if not already_imported and import_stmt not in imports_to_add:
        ...
        if imports_to_add:
            ...
            for import_stmt in reversed(imports_to_add):  # Reverse to maintain order
                lines.insert(import_section_end + 1, import_stmt)

            test_code = '\n'.join(lines)
            logger.info(f"  ✅ Added {len(imports_to_add)} missing import(s)")
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            # CRITICAL: Add missing imports (datetime, Mock, etc.) BEFORE testing
            logger.info("🔧 Adding missing imports (datetime, Mock, etc.)...")
            test_code = self._add_missing_imports(test_code)
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt == 1 and language == "python":
                ...
                is_framework_context_error = (
                    ...
                    or "AssertionError" in combined_output and "failures" in combined_output  # CRITICAL: Multiple failures pattern
                    or "def test_" in combined_output and "client.get" not in combined_output  # CRITICAL: Missing test client usage
                    or "NameError: name" in combined_output and "is not defined" in combined_output  # CRITICAL: Missing imports
                    or "ModuleNotFoundError" in combined_output  # CRITICAL: Wrong module imports
                )
            ...
            if attempt >= self.max_retries:
                ...
                if cleaned_code and cleaned_code != current_code:
                    ...
                    # CRITICAL: Always return the cleaned code immediately
                    # Don't test it again - we already know these tests passed individually
                    logger.info(f"✅ Nuclear option complete - returning {remaining_tests} tests that passed")
                    logger.info("   (Tests passed individually, so file should work)")
                    Path(test_file_path).write_text(current_code, encoding="utf-8")
                    fix_history.append(
