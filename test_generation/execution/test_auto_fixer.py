        ...
        try:
            ...
            for line in test_code.split("\n"):
                ...
                if "from flask.app import" in line:
                    ...
                    # Don't use this import, will be replaced
                    continue
            ...
            if not app_import:
                ...
                # CRITICAL: Never import from flask.app (Flask's internal module)
                if parent_module and parent_module != "." and parent_module != "flask":
                    module_file = test_path.parent / test_path.name.replace(
                        "_test.py", ".py"
                    )
                    if module_file.exists():
                        # Additional check: don't create circular or flask.app imports
                        if module_file.stem != "flask" and parent_module != "flask":
                            app_import = (
                            ...
                            logger.info(f"  ℹ️ Using default (avoiding flask.app): {app_import}")
                    else:
                        app_import = "from app import app"
            ...
            if should_reconstruct:
                ...
                # 1. Keep imports (but filter out bad flask.app imports)
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt == 1 and language == "python":
                ...
                is_flask_context_error = (
                    ...
                    or "module 'flask.app' has no attribute 'config'" in combined_output  # CRITICAL: Fix bad flask.app import
                )
