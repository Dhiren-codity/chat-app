    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            if "FAILED" in combined_output or "ERROR" in combined_output or "PASSED" in combined_output:
                ...
                pass_pattern = re.compile(r"PASSED\s+(\S+::[\w\[\],-]+)")
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests:
                logger.info(f"✅ Using {len(initial_passing_tests)} tests that passed: {initial_passing_tests}")
                passing_tests = [t for t in test_functions if t.split("::")[-1] in initial_passing_tests]
