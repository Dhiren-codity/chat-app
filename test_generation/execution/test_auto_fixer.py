    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            if (
                ...
                # Match inline status (from -vv output) - FIXED: More flexible patterns
                pass_pattern = re.compile(r"(\S+::[\w\[\],-]+)\s+PASSED\s*$")
                fail_inline_pattern = re.compile(r"(\S+::[\w\[\],-]+)\s+(FAILED|ERROR)\s*$")
                ...
                # CRITICAL FIX: Also match simple PASSED/FAILED/ERROR lines without file prefix
                simple_pass_pattern = re.compile(r"^\s*(\w+)\s+PASSED\s*$")
                simple_fail_pattern = re.compile(r"^\s*(\w+)\s+(FAILED|ERROR)\s*$")
                ...
                for i, line in enumerate(lines):
                    ...
                    if pass_match:
                        ...
                        if simple_pass_match:
                            ...
                            if test_name not in error_info["passing"]:
                                error_info["passing"].append(test_name)
                                logger.info(f"✅ Found passing test (simple): {test_name}")
        ...
        if error_info["passing"]:
            ...
            logger.warning("⚠️ NO PASSING TESTS EXTRACTED - This will cause all tests to be removed!")
            logger.warning(f"   Raw output contains 'PASSED': {'PASSED' in combined_output}")
            logger.warning(f"   Raw output contains 'FAILED': {'FAILED' in combined_output}")
            logger.warning(f"   Raw output contains 'ERROR': {'ERROR' in combined_output}")
        ...
        try:
            ...
            if not passing_tests:
                ...
                if initial_passing_tests and len(initial_passing_tests) > 0:
                    logger.warning(f"⚠️ BUT we had {len(initial_passing_tests)} passing tests initially!")
                    logger.warning("⚠️ The issue is likely that individual test runs fail due to missing context")
                    logger.warning("⚠️ Instead of removing all tests, let's try to fix the context issues")
                    
                    # Return the original test code with fixes applied instead of minimal file
                    logger.info("🔧 Attempting to fix context issues in original tests...")
                    fixed_code = self._apply_context_fixes(test_code)
                    if fixed_code:
                        logger.info("✅ Applied context fixes to preserve original tests")
                        return fixed_code
                    else:
                        logger.warning("⚠️ Context fixes failed, but preserving original tests anyway")
                        return test_code
