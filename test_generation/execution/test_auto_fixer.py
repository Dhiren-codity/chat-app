...
"""
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                for idx, t in enumerate(test_functions):
                    ...
                    # Extract JUST the function name from file_test_name
                    # file_test_name could be "TestClass::test_foo" or "test_foo"
                    if "::" in file_test_name:
                        file_function_name = file_test_name.split("::")[-1]  # Get "test_foo"
                    else:
                        file_function_name = file_test_name

                    # Now check if THIS FUNCTION NAME exists in passing_base_names
                    if file_function_name in passing_base_names:
                        matched = True
                        if idx < 5:
                            logger.info(f"      → MATCHED (direct: {file_function_name})")
                    else:
                        # Also check full path match
                        if file_test_name in passing_base_names:
                            matched = True
                            if idx < 5:
                                logger.info(f"      → MATCHED (full path: {file_test_name})")

                    if not matched and idx < 5:
                        logger.info(f"      → NOT MATCHED (tried function name '{file_function_name}' and full path '{file_test_name}')")
    ...
    def _fix_go_compilation_errors(self, test_code: str, test_file_path: str = None) -> str:
        ...
        if self.llm_client and source_content:
            ...
            prompt = f"""You are a Go testing expert. Your task is to generate REAL, production-ready unit tests for the following Go source code.

SOURCE CODE FILE: {source_file_path_obj.name if source_file_path_obj else 'unknown'}
```go
{source_content}
```
...
1. ANALYZE THE SOURCE CODE FIRST
   - Identify if functions are standalone functions or struct methods
   - Check the EXACT function signatures (parameters and return types)
   - Note which structs have constructors (New* functions)

2. FUNCTION vs METHOD CALLS - THIS IS CRITICAL
   - If you see: `func FunctionName(params)` → Call it directly: `result := FunctionName(args)`
   - If you see: `func (receiver *Type) MethodName(params)` → Create instance first, then call:
     ```go
     instance := NewType()  // or &Type{{}}
     result := instance.MethodName(args)  // ❌ NOT: MethodName(args)
     ```

3. EXACT FUNCTION SIGNATURES
   - Count parameters EXACTLY as defined in source
   - Match return types EXACTLY (error, bool, string, etc.)
   - If function returns (result, error), test BOTH values
   - If function returns just error, only check error

4. STRUCT INITIALIZATION
   - If source has `func NewType() *Type`, use: `instance := NewType()`
   - If no constructor, use: `instance := &Type{{}}`
   - Set required fields if struct has public fields

5. TEST STRUCTURE
   - Each test function tests ONE method/function
   - Use table-driven tests for multiple scenarios
   - Test success cases AND error cases
   - Use meaningful test data (not "test", "foo", etc.)

6. ASSERTIONS
   - Use testify/assert correctly:
     - `assert.NoError(t, err)` - when expecting no error
     - `assert.Error(t, err)` - when expecting any error
     - `assert.Equal(t, expected, actual)` - for exact matches
     - `assert.Contains(t, str, substring)` - for error message checks
     - `assert.ElementsMatch(t, expected, actual)` - for slices (order doesn't matter)

7. PACKAGE AND IMPORTS
   - Package name MUST be: {package_name}
   - Import exactly what you use:
     ```go
     import (
         "testing"
         "github.com/stretchr/testify/assert"
         // Add others only if needed: "time", "strings", etc.
     )
     ```

8. WHAT NOT TO DO
   - ❌ NO placeholders or TODOs
   - ❌ NO `assert.True(t, true)` dummy tests
   - ❌ NO calling methods as functions: `ValidateContent()` when it's `mv.ValidateContent()`
   - ❌ NO wrong number of parameters
   - ❌ NO ignoring return values
   - ❌ NO testing functions that don't exist in source

EXAMPLE - ANALYZE THIS PATTERN:
...
func NewMessageValidator() *MessageValidator {{
    return &MessageValidator{{MinLength: 1, MaxLength: 1000}}
}}

func (mv *MessageValidator) ValidateContent(content string) error {{
    if len(content) < mv.MinLength {{
...
func TestMessageValidator_ValidateContent(t *testing.T) {{
    mv := NewMessageValidator()  // ✅ Use constructor

    // Test valid content
    err := mv.ValidateContent("Hello world")  // ✅ Call as method
...
func TestMessageValidator_CountWords(t *testing.T) {{
    ...
    for _, tt := range tests {{
        t.Run(tt.name, func(t *testing.T) {{
            result := mv.CountWords(tt.content)  // ✅ Returns int, not error
...
- Analyze the source code structure first
- Identify all exported functions and methods
- Generate comprehensive tests for each
- Use EXACT function signatures from source
- Call methods on instances, not directly
