...
class TestAutoFixer:
    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            if "FAILED" in combined_output or "ERROR" in combined_output or "PASSED" in combined_output:
                import re
...
"""
        ...
        try:
            ...
            for line in test_code.split("\n"):
                ...
                if "from flask.app import" in line:
                    ...
                    logger.warning(f"     This imports Flask's internal module, not your app!")
                    logger.warning(f"     Will be removed and replaced with correct import")
                ...
                if "import app" in line or (
                    ...
                    if "flask.app" in line:
                        logger.warning(f"  ⚠️ Skipping bad flask.app import: {line.strip()}")
                        continue
                    ...
                    if "import" in line:
                        ...
                        if len(parts) > 1:
                            ...
                            if imported and not imported.startswith("("):
                                ...
                                logger.info(f"  ✅ Found valid app import: {app_import}")
                                logger.info(f"     Using app variable name: {app_var_name}")
                    break

            # If no valid app import found, infer from test file path
            if not app_import:
                logger.info("  🔍 No valid app import found - inferring correct import...")
                ...
                module_file = test_path.parent / test_path.name.replace("_test.py", ".py")
                if module_file.exists():
                    # Check if parent is a Python package or just a system directory
                    if parent_module and parent_module != "." and parent_module != "flask" and parent_module not in non_package_dirs:
                        # Parent is a package, use qualified import
                        app_import = f"from {parent_module}.{module_file.stem} import app"
                        logger.info(f"  ✅ Inferred app import from package structure: {app_import}")
                    else:
                        # Parent is not a package (or is tmp/var/etc), use direct import
                        app_import = f"from {module_file.stem} import app"
                        logger.info(f"  ✅ Inferred app import from sibling file: {app_import}")
                else:
                    # No source file found, use default
                    app_import = "from app import app"
                    logger.info(f"  ✅ No source file found, using default: {app_import}")
            ...
            if should_reconstruct:
                ...
                has_app_import = any("import app" in line and "from flask.app" not in line for line in final_lines)
                ...
                if not has_client_fixture:
                    ...
                    final_lines.append("def client():")  # ✅ No 'app' parameter, use imported app directly
                    final_lines.append('    """Flask test client with app context."""')
                    final_lines.append(f"    {app_var_name}.config['TESTING'] = True")
                    final_lines.append(f"    with {app_var_name}.test_client() as client:")
    ...
    def keep_only_passing_tests(
        self, test_file_path: str, language: str = "python", initial_passing_tests: list = None
    ) -> Optional[str]:
        """
        ...
        logger.info(f"   initial_passing_tests provided: {initial_passing_tests is not None}")
        if initial_passing_tests is not None:
            logger.info(f"   initial_passing_tests count: {len(initial_passing_tests)}")
            logger.info(f"   initial_passing_tests (first 10): {initial_passing_tests[:10]}")
        ...
        try:
            ...
            logger.info(
                f"📋 Found {len(test_functions)} test functions in file"
            )
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                logger.info(f"✅ Using {len(initial_passing_tests)} tests that passed initially: {initial_passing_tests}")
                ...
                for t in test_functions:
                    ...
                    test_name = t.split("::")[-1]  # Remove class: TestClass::test_foo -> test_foo
                    base_name = test_name.split("[")[0]  # Remove params: test_foo[x] -> test_foo

                    # Check if this test (or any parametrized version of it) passed
                    matched = False
                    for passing in initial_passing_tests:
                        passing_base = passing.split("[")[0]  # Get base name of passing test
                        if base_name == passing_base:
                            matched = True
                            passing_tests.append(t)
                            logger.info(f"  ✅ KEEP: {t} (matches passing test: {passing})")
                            break
                ...
                logger.info(f"📊 RESULT: Keeping {len(passing_tests)}/{len(test_functions)} test functions")
                logger.info(f"   Expected ~{len(initial_passing_tests)} passing (may differ due to parametrization)")
                ...
                if len(passing_tests) < len(initial_passing_tests) * 0.5:
                    ...
                    logger.error(f"⚠️ Only keeping {len(passing_tests)} tests out of {len(initial_passing_tests)} that passed!")
                    logger.error(f"⚠️ This suggests a parametrized test matching problem!")
                    logger.error(f"⚠️ Refusing to delete passing tests - returning original code unchanged")
            ...
            if not passing_tests:
                ...
                logger.warning(f"⚠️ DEBUG: initial_passing_tests had {len(initial_passing_tests) if initial_passing_tests else 0} tests")
                logger.warning(f"⚠️ DEBUG: test_functions found {len(test_functions)} functions")
                return self._create_minimal_valid_file(test_code)
...
"""
    ...
    def auto_fix_test(
        ...
        if self.max_retries == 0:
            ...
            logger.info(f"📊 Test results: {len(passing_tests)} passing, "
                       f"{len(error_info.get('failures', []))} failing")

            if passing_tests:
                logger.info(f"🚀 Running nuclear option to keep {len(passing_tests)} passing tests...")
                ...
                if cleaned_code and cleaned_code != current_code:
                    fix_history.append(f"Nuclear option: Kept {len(passing_tests)} passing tests")
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if total_tests > 0:
                ...
                logger.info(f"📊 Test coverage: {len(passing_tests)}/{total_tests} passed ({pass_rate*100:.1f}%)")
            ...
            if attempt == 1 and language == "python":
                ...
                is_flask_context_error = (
                    ...
                    or "NameError: name 'app' is not defined" in combined_output  # CRITICAL: Add missing app import
                    or "module 'flask.app' has no attribute" in combined_output  # CRITICAL: Fix bad flask.app import
                    or "cannot import name 'app' from 'flask.app'" in combined_output  # CRITICAL: Fix bad flask.app import (import error)
                )
                ...
                is_dependency_error = (
                    "AttributeError: module 'sqlalchemy' has no attribute '__all__'" in combined_output
                    or "AttributeError: module 'werkzeug' has no attribute '__version__'" in combined_output
                ...
                if is_dependency_error:
                    ...
                    if "module 'sqlalchemy' has no attribute '__all__'" in combined_output:
                        logger.error(
                    ...
                    if "module 'werkzeug' has no attribute '__version__'" in combined_output:
                        logger.error(
            ...
            if attempt >= self.max_retries:
                ...
                passing_tests = error_info.get("passing", [])
                logger.info(f"📊 Nuclear Option Input: {len(passing_tests)} passing tests from error_info")
                if passing_tests:
                    logger.info(f"✅ Passing tests list (first 20): {passing_tests[:20]}")
                ...
                logger.info(f"🔧 Calling keep_only_passing_tests with {len(passing_tests)} passing tests...")
                cleaned_code = self.keep_only_passing_tests(test_file_path, language, initial_passing_tests=passing_tests)

                if cleaned_code and cleaned_code != current_code:
                    current_code = cleaned_code
                    # Count how many test functions remain
                    import re
                    remaining_tests = len(re.findall(r'^\s*def test_\w+', cleaned_code, re.MULTILINE))
                    logger.info(f"✅ Nuclear option created cleaned version with {remaining_tests} test functions")
                    logger.info(f"   Original size: {len(current_code)} chars → Cleaned size: {len(cleaned_code)} chars")
                    ...
                    if success:
                        logger.info(f"✅ Nuclear option succeeded! Final test count: {remaining_tests}")
                        fix_history.append(f"Attempt {attempt}: Nuclear option SUCCESS (kept {remaining_tests} tests)")
                        return True, current_code, fix_history
                    else:
                        logger.warning(f"❌ Nuclear option still has errors after keeping {remaining_tests} tests")
                        logger.warning(f"   This means even the 'passing' tests fail when run individually!")
                else:
            ...
            if self.llm_client:
                ...
                logger.info(f"   Will ask LLM to fix {len(error_info['errors'])} errors and {len(error_info['failures'])} failures")
                ...
                logger.info(f"✅ Attempt {attempt}: Got fixed code from LLM ({len(fixed_code)} chars)")
                logger.info(f"   Code size changed: {len(current_code)} → {len(fixed_code)} chars")
                current_code = fixed_code
            else:
        ...
        if self.llm_client:
            ...
            if 'error_info' in locals():
                ...
                logger.info(f"   No error_info available (max_retries=0 or loop skipped)")
            ...
            try:
                ...
                if 'error_info' not in locals():
                    logger.warning("⚠️ Cannot run fallback: no error_info available (max_retries=0)")
                    return False, current_code, fix_history
                ...
                if fallback_code and fallback_code != current_code:
                    import re
                    original_tests = len(re.findall(r'^\s*def test_\w+', current_code, re.MULTILINE))
                    remaining_tests = len(re.findall(r'^\s*def test_\w+', fallback_code, re.MULTILINE))

                    logger.info(f"✅ LLM removed {original_tests - remaining_tests} tests, {remaining_tests} remain")
                    logger.info(f"   Code size: {len(current_code)} → {len(fallback_code)} chars")
