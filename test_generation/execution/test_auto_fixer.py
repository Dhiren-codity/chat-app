            ...
            try:
                ...
                while i < len(lines):
                    ...
                    if stripped.startswith("def test") and stripped.endswith(":"):
                        ...
                        # Check if function has a body by looking at next non-empty line
                        has_body = False
                        j = i + 1
                        ...
                        while j < len(lines):
                            ...
                            # If next line is indented MORE than function, it's the body
                            if next_indent > func_indent:
                                has_body = True
                                break
                            else:
                                # Next line is at same or lower indent - function is empty
                                break

                        # If no body found, strip parameters and add 'pass'
                        if not has_body:
                            # Strip parameters from empty functions to avoid pytest errors
                            # "def test_foo(param1, param2):" -> "def test_foo():"
                            func_match = re.match(r'^(\s*def\s+test\w+)\s*\([^)]*\)\s*:\s*$', line)
                            if func_match:
                                # Has parameters - strip them
                                func_name_part = func_match.group(1)
                                cleaned_line = func_name_part + "():"
                                logger.warning(f"   Stripping parameters from empty function: {stripped[:60]}")
                                fixed_lines.append(cleaned_line)
                            else:
                                # No parameters or malformed - keep as-is
                                fixed_lines.append(line)

                            logger.warning(f"   Adding 'pass' to empty function: {stripped[:60]}")
                            fixed_lines.append(" " * expected_body_indent + "pass")
                        else:
                            # Has body - keep function definition as-is
                            fixed_lines.append(line)
