...
"""
    ...
    def _keep_only_passing_tests_python(
        ...
        print("\n" + "="*80)
        print("🚀 NUCLEAR OPTION ACTIVATED FOR PYTHON")
        print("="*80)
        print(f"📄 File: {test_file_path}")
        print(f"📊 Initial passing tests: {len(initial_passing_tests) if initial_passing_tests else 0}")
        ...
        try:
            ...
            print("🧹 NUCLEAR OPTION: Starting preprocessing pipeline...")
            print(f"   📄 File: {test_file_path}")
            ...
            print("   ✍️  Writing pre-cleaned code to disk...")
            ...
            print("🧹 NUCLEAR OPTION: Fixing module-level model instantiation errors...")
            logger.info("🧹 Fixing module-level model instantiation errors...")
            test_code, was_modified = self._fix_module_level_model_instantiation(test_code, language)
            if was_modified:
                print(f"   ✅ Modified code to replace model instantiations with Mock")
                logger.info("   ✅ Model instantiations replaced with Mock")
            else:
                print(f"   ℹ️  No model instantiations found to replace")
                logger.info("   ℹ️  No model instantiations found")

            # CRITICAL: Remove bad imports BEFORE testing individual functions
            # This allows tests that don't use the bad imports to pass
            print("🧹 NUCLEAR OPTION: Removing placeholder imports...")
            ...
            print("🧹 NUCLEAR OPTION: Removing mocker fixture usage...")
            logger.info("🧹 Fixing mocker fixture usage (replacing with monkeypatch)...")
            test_code = self._fix_mocker_fixture(test_code)

            # Remove parametrize decorators from fixtures (they cause warnings)
            print("🧹 NUCLEAR OPTION: Removing parametrize from fixtures...")
            logger.info("🧹 Removing parametrize decorators from fixture functions...")
            test_code = self._remove_parametrize_from_fixtures(test_code)

            # CRITICAL: Fix misplaced imports BEFORE testing
            # This fixes IndentationError from orphaned imports at wrong indentation
            print("🧹 NUCLEAR OPTION: Fixing misplaced imports...")
            ...
            print("🧹 NUCLEAR OPTION: Fixing orphaned functions...")
            logger.info("🧹 Fixing orphaned function definitions before testing...")
            test_code = self._fix_orphaned_functions(test_code)

            # Fix orphaned decorators (decorators without functions)
            print("🧹 NUCLEAR OPTION: Fixing orphaned decorators...")
            logger.info("🧹 Fixing orphaned decorators before testing...")
            test_code = self._fix_syntax_errors_after_cleanup(test_code)

            # Remove tests with invalid signatures (causes pytest collection errors)
            print("🧹 NUCLEAR OPTION: Removing tests with invalid signatures...")
            logger.info("🧹 Removing tests with invalid parameter references...")
            test_code = self._remove_tests_with_invalid_signatures(test_code)

            # Write fully cleaned code to file so individual test runs work
            print("   ✍️  Writing ALL fixes to disk before testing...")
            Path(test_file_path).write_text(test_code, encoding="utf-8")
            print(f"   ✅ Preprocessing complete! File updated: {test_file_path}")
            ...
            if not passing_tests:
                print(f"\n⚠️  NO TESTS PASSED INDIVIDUALLY - Creating minimal file")
                print(f"   Initial passing tests: {len(initial_passing_tests) if initial_passing_tests else 0}")
            ...
            # CRITICAL: Re-run cleanup pipeline after test removal
            # Removing tests can create orphaned decorators that attach to fixtures!
            print("🧹 POST-REMOVAL CLEANUP: Re-running preprocessing pipeline...")
            logger.info("🧹 Post-removal cleanup: Running full preprocessing pipeline...")

            # Run cleanup in a loop until no more changes are made
            # This handles cases where one cleanup reveals another issue
            max_cleanup_passes = 3
            for cleanup_pass in range(1, max_cleanup_passes + 1):
                code_before_pass = cleaned_code
                print(f"  🔄 Cleanup pass {cleanup_pass}/{max_cleanup_passes}...")

                # Clean up orphaned decorators first
                logger.info(f"  Pass {cleanup_pass}: Removing orphaned decorators...")
                cleaned_code = self._fix_syntax_errors_after_cleanup(cleaned_code)

                # Remove parametrize from fixtures (critical - test removal can orphan decorators)
                print(f"    Removing parametrize from fixtures...")
                logger.info(f"  Pass {cleanup_pass}: Removing parametrize from fixtures...")
                cleaned_code = self._remove_parametrize_from_fixtures(cleaned_code)

                # Remove tests with invalid signatures (new tests might be exposed)
                print(f"    Removing tests with invalid signatures...")
                logger.info(f"  Pass {cleanup_pass}: Removing tests with invalid signatures...")
                cleaned_code = self._remove_tests_with_invalid_signatures(cleaned_code)

                # Final orphaned decorator cleanup for this pass
                logger.info(f"  Pass {cleanup_pass}: Final orphaned decorator cleanup...")
                cleaned_code = self._fix_syntax_errors_after_cleanup(cleaned_code)

                # Check if any changes were made
                if cleaned_code == code_before_pass:
                    print(f"  ✅ No changes in pass {cleanup_pass} - cleanup complete!")
                    logger.info(f"  ✅ Cleanup converged after {cleanup_pass} pass(es)")
                    break
                else:
                    print(f"  🔄 Pass {cleanup_pass} made changes - running another pass...")
                    logger.info(f"  🔄 Pass {cleanup_pass} made changes")
            else:
                print(f"  ⚠️  Reached max cleanup passes ({max_cleanup_passes})")
                logger.warning(f"  ⚠️  Reached max cleanup passes ({max_cleanup_passes})")
            ...
            try:
                ...
                try:
                    ...
                    print("\n⚠️  NUCLEAR OPTION: Returning minimal file (syntax errors couldn't be fixed)")
                    return self._create_minimal_valid_file(test_code)

            print(f"\n✅ NUCLEAR OPTION COMPLETE: Returning cleaned code ({len(cleaned_code)} chars)")
            # Check if MessageReaction in final code
            if 'MessageReaction(' in cleaned_code:
                print(f"   ⚠️  WARNING: Cleaned code STILL contains MessageReaction(")
            else:
                print(f"   ✓ No MessageReaction in cleaned code")
            return cleaned_code

        except Exception as e:
            print(f"\n❌ NUCLEAR OPTION ERROR: {e}")
    ...
    def _remove_parametrize_from_fixtures(self, test_code: str) -> str:
        """
        Remove parametrize decorators from fixture functions.
        Pytest doesn't support parametrize on fixtures - it causes warnings and doesn't work.

        Args:
            test_code: Test code that may have parametrize on fixtures

        Returns:
            Test code with parametrize removed from fixtures
        """
        import re

        print(f"  🔍 Scanning {len(test_code.split(chr(10)))} lines for parametrize on fixtures...")

        lines = test_code.split("\n")
        result_lines = []
        i = 0
        total_parametrize_found = 0
        total_removed = 0
        ...
        while i < len(lines):
            ...
            if stripped.startswith('@pytest.mark.parametrize'):
                total_parametrize_found += 1
                print(f"    Line {i+1}: Found parametrize decorator")
                # Find the end of this parametrize decorator (handles multi-line)
                parametrize_start = i
                j = i
                paren_depth = line.count('(') - line.count(')')
                bracket_depth = line.count('[') - line.count(']')

                while j < len(lines) - 1 and (paren_depth > 0 or bracket_depth > 0):
                    j += 1
                    paren_depth += lines[j].count('(') - lines[j].count(')')
                    bracket_depth += lines[j].count('[') - lines[j].count(']')

                parametrize_end = j  # Last line of parametrize decorator

                # Now look ahead to find what it decorates
                k = parametrize_end + 1
                ...
                # Check if @pytest.fixture appears BEFORE or AFTER parametrize (both are possible)
                is_fixture = False
                current_indent = len(lines[parametrize_start]) - len(lines[parametrize_start].lstrip())

                # Check BEFORE parametrize (look back through decorators at same indentation)
                m = parametrize_start - 1
                while m >= 0:
                    line_text = lines[m].strip()
                    line_indent = len(lines[m]) - len(lines[m].lstrip())

                    # Stop if we hit code at lower indentation (different function/class)
                    if line_text and line_indent < current_indent:
                        break

                    # Only check decorators at same indentation level
                    if line_indent == current_indent and line_text:
                        # Check for @pytest.fixture or @fixture (exact or with parens, but not @fixture_something)
                        if '@pytest.fixture' in line_text:
                            is_fixture = True
                            print(f"      Found @pytest.fixture BEFORE at line {m+1}")
                            logger.info(f"  🔍 Found @pytest.fixture at line {m+1} (before parametrize at {parametrize_start+1})")
                            break
                        elif line_text == '@fixture' or line_text.startswith('@fixture('):
                            if 'import fixture' not in lines[m]:  # Not an import line
                                is_fixture = True
                                print(f"      Found @fixture BEFORE at line {m+1}")
                                logger.info(f"  🔍 Found @fixture at line {m+1} (before parametrize at {parametrize_start+1})")
                                break

                        # Stop if we hit a function/class definition or non-decorator
                        if not line_text.startswith('@') and not line_text.startswith('#'):
                            break
                    m -= 1

                # Check AFTER parametrize (between parametrize and function)
                if not is_fixture:
                    for m in range(parametrize_start, k):
                        line_text = lines[m].strip()
                        line_indent = len(lines[m]) - len(lines[m].lstrip())

                        # Only check lines at same indentation
                        if line_indent == current_indent and line_text:
                            if '@pytest.fixture' in line_text:
                                is_fixture = True
                                print(f"      Found @pytest.fixture AFTER at line {m+1}")
                                logger.info(f"  🔍 Found @pytest.fixture at line {m+1} (after parametrize at {parametrize_start+1})")
                                break
                            elif line_text == '@fixture' or line_text.startswith('@fixture('):
                                if 'import fixture' not in lines[m]:
                                    is_fixture = True
                                    print(f"      Found @fixture AFTER at line {m+1}")
                                    logger.info(f"  🔍 Found @fixture at line {m+1} (after parametrize at {parametrize_start+1})")
                                    break

                if is_fixture:
                    # Skip all lines from parametrize_start to parametrize_end (inclusive)
                    total_removed += 1
                    logger.info(f"  🗑️ Removing parametrize decorator from fixture (lines {parametrize_start+1}-{parametrize_end+1})")
                    print(f"      REMOVING parametrize from fixture (lines {parametrize_start+1}-{parametrize_end+1})")
                    i = parametrize_end + 1  # Jump past all parametrize lines
                    continue  # Don't append these lines
                else:
                    print(f"      Not on a fixture - keeping parametrize")
        ...
        # Summary
        result_code = "\n".join(result_lines)
        print(f"  📊 Parametrize scan: Found {total_parametrize_found} decorators, removed {total_removed} from fixtures")
        if total_removed > 0:
            logger.info(f"  ✅ Removed {total_removed} parametrize decorators from fixtures")
            # Verify the result doesn't have "@pytest.mark.parametrize" on fixtures
            if '@pytest.mark.parametrize' in result_code:
                remaining = result_code.count('@pytest.mark.parametrize')
                print(f"    ℹ️  Note: {remaining} parametrize decorators remain (on regular tests)")

        return result_code

    def _remove_tests_with_invalid_signatures(self, test_code: str) -> str:
        """
        Remove test functions that reference parameters they don't have in their signature.
        This causes pytest collection errors like "function uses no argument 'x'".

        Args:
            test_code: Test code that may have invalid function signatures

        Returns:
            Test code with invalid tests removed
        """
        import re
        import ast

        print(f"  🔍 Scanning for tests with invalid signatures...")
        ...
        # Extract all fixtures defined in this file
        defined_fixtures = set()
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                # Check if function has @pytest.fixture or @fixture decorator
        ...
        # Extract all imported names (top-level imports only for simplicity)
        imported_names = set()
        ...
        # Common pytest fixtures (built-in and common plugins)
        known_fixtures = {
            'request', 'monkeypatch', 'tmp_path', 'capsys', 'capfd', 'mocker',
            'client', 'app', 'db', 'session', 'tmpdir', 'caplog', 'pytestconfig',
            'record_property', 'record_testsuite_property', 'recwarn', 'capfdbinary',
            'capsysbinary', 'tmp_path_factory', 'tmpdir_factory'
        }

        # Python builtins and common test utilities
        known_names = {
            'True', 'False', 'None', 'len', 'str', 'int', 'float', 'list', 'dict',
            'set', 'tuple', 'range', 'enumerate', 'zip', 'map', 'filter', 'print',
            'type', 'isinstance', 'issubclass', 'Exception', 'ValueError', 'TypeError',
            'Mock', 'MagicMock', 'patch', 'call', 'ANY', 'PropertyMock', 'mock_open',
            'assert', 'pytest', 'TestCase', 'asyncio', 'await', 'async'
        }

        # Combine all known safe names
        # NOTE: We do NOT include defined_fixtures here because fixtures MUST be declared
        # in function parameters to be used. Just because a fixture exists in the file
        # doesn't mean tests can use it without declaring it in their signature.
        safe_names = known_fixtures | imported_names | known_names
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name.startswith('test_'):
                ...
                # Extract parametrize arguments from decorators
                ...
                for decorator in node.decorator_list:
                    if isinstance(decorator, ast.Call):
                        ...
                        if is_parametrize and len(decorator.args) >= 1:
                            # First argument is the parameter names (string or list of strings)
                            first_arg = decorator.args[0]
                            if isinstance(first_arg, ast.Constant) and isinstance(first_arg.value, str):
                                # Split by comma and strip whitespace
                                param_names = [p.strip() for p in first_arg.value.split(',')]
                                parametrize_params.update(param_names)

                # Get names that are LOADED (used) but not STORED (defined locally)
                loaded_names = set()
                stored_names = set()
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.Name):
                        if isinstance(child.ctx, ast.Load):
                            ...
                            stored_names.add(child.id)

                # Names that are used but not defined locally
                undefined_names = loaded_names - stored_names

                # Check if any undefined name is NOT in params, parametrize, or safe names
                for name in undefined_names:
                    if (name not in params and
                        name not in parametrize_params and
                        name not in safe_names):
                        # Check if it looks like a variable (not a property/method call)
                        # Variables typically have underscores or are lowercase multi-char
                        if '_' in name or (name.islower() and len(name) > 1):
                            # This is likely a missing parameter/fixture
                            tests_removed += 1
                            logger.info(f"  🗑️ Removing test '{node.name}' (uses '{name}' not in params/fixtures/imports)")
                            print(f"  🗑️ Removing test '{node.name}' (uses '{name}' not in signature)")

                            # Mark function lines for removal
                            start_line = node.lineno - 1  # Convert to 0-indexed
                            end_line = node.end_lineno

                            # Also remove decorators above the function
                            deco_start = start_line
                            while deco_start > 0 and lines[deco_start - 1].strip().startswith('@'):
                                deco_start -= 1

                            for line_idx in range(deco_start, end_line):
                                lines_to_remove.add(line_idx)
                            break  # Found an issue, remove this function

        # Summary
        print(f"    Found {tests_analyzed} test functions, removing {tests_removed} with invalid signatures")
        if tests_removed > 0:
            logger.info(f"  ✅ Removed {tests_removed} tests with invalid signatures")
            print(f"    ℹ️  Removed {len(lines_to_remove)} total lines")

        # Build result without removed lines
        result_lines = [line for idx, line in enumerate(lines) if idx not in lines_to_remove]
        return "\n".join(result_lines)
...
"""
    ...
    def _fix_module_level_model_instantiation(self, test_code: str, language: str) -> tuple[str, bool]:
        ...
        # Pattern to detect model instantiation with keyword OR positional arguments
        # Matches: ModelName(...) where ModelName starts with uppercase
        # Examples: MessageReaction(1, 2, 3), User(id=1, name="test")
        # The pattern matches the opening paren to detect instantiation attempts
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith(('def ', 'class ')):
                ...
                print(f"      🔍 Line {i+1}: ENTER scope (def/class at indent={current_indent})")
                fixed_lines.append(line)
                continue

            # Exit function/class scope when we hit module-level code again
            # Key insight: ANY line at module-level indentation means we've exited the function
            # This includes: blank lines, decorators, comments, or any other code
            if in_function_or_class and current_indent <= indent_level:
                print(f"      🔍 Line {i+1}: EXIT scope (indent={current_indent} <= {indent_level}, line='{stripped[:40] if stripped else 'BLANK'}')")
                in_function_or_class = False
            ...
            if not in_function_or_class:
                ...
                if matches:
                    ...
                    for match in reversed(matches):
                        ...
                        # Skip common Python built-ins, exceptions, and test utilities
                        if model_name in ('Mock', 'MagicMock', 'patch', 'TestCase', 'Dict', 'List',
                                         ...
                                         'NameError', 'RuntimeError', 'NotImplementedError', 'IndexError',
                                         'None', 'True', 'False', 'str', 'int', 'float', 'bool', 'bytes',
                                         'Path', 'datetime'):
                            continue
                    ...
                    if models_replaced:
                        ...
                        was_modified = True
                        print(f"      🔧 Line {i+1}: Replaced {', '.join(set(models_replaced))} with Mock")
                        print(f"         Before: {stripped[:80]}")
        ...
        if was_modified and not has_mock_import:
            ...
            for i, line in enumerate(lines):
                ...
                if i == last_import_index and not import_added:
                    ...
                    import_added = True
                    print(f"      ✅ Added 'from unittest.mock import Mock' after line {i+1}")
                    logger.info("✅ Added 'from unittest.mock import Mock' import")

            fixed_code = "\n".join(final_lines)

        if was_modified:
            print("   ✅ Model instantiation fix complete - replaced models with Mock objects")
            logger.info("✅ Fixed module-level model instantiation errors by replacing with Mock objects")
        else:
            print("   ℹ️  No changes made - no model instantiations found at module level")
