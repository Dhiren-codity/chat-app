        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                if len(passing_tests) < len(initial_passing_tests) * 0.5:
                    logger.warning(f"⚠️ WARNING: Only keeping {len(passing_tests)} tests out of {len(initial_passing_tests)} that passed!")
                    logger.warning(f"⚠️ This suggests a matching problem - refusing to remove passing tests!")
                    logger.warning(f"⚠️ Returning original code unchanged to prevent data loss")
                    return test_code
            else:
