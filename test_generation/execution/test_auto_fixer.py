...
"""
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            try:
                ...
                # Fix 3: Remove lines that are just whitespace followed by decorator
                # (These cause "expected an indented block" errors)
                fixed_lines = []
                for i, line in enumerate(lines):
                    stripped = line.strip()
                    # If this is a decorator at module level (no indentation)
                    # and previous line was empty, and no function definition follows
                    if stripped.startswith("@") and not line[0].isspace():
                        # Look ahead to see if there's a function definition
                        has_function_after = False
                        for j in range(i + 1, min(i + 5, len(lines))):
                            if lines[j].strip().startswith("def "):
                                has_function_after = True
                                break
                        if not has_function_after:
                            logger.warning(
                                f"   Removing orphaned decorator: {stripped}"
                            )
                            continue
                    fixed_lines.append(line)
                cleaned_code = "\n".join(fixed_lines) + "\n"
            ...
            # CRITICAL: Apply additional structural fixes
            logger.info("🔧 Applying structural fixes...")

            # Fix empty function bodies (add 'pass' statements)
            cleaned_code = self._fix_empty_function_bodies(cleaned_code)

            # Remove orphaned parametrize decorators
            cleaned_code = self._remove_orphaned_parametrize(cleaned_code)

            # Final AST-based validation
            try:
                compile(cleaned_code, "<string>", "exec")
                logger.info("✅ Final structural validation passed")
            except SyntaxError as e:
                logger.warning(f"⚠️ Final validation failed: {e}")
                logger.warning(f"   Line {e.lineno}: {e.text}")
                # Try one more simple fix for any remaining issues
                lines = cleaned_code.split("\n")
                fixed_lines = []
                for i, line in enumerate(lines):
                    stripped = line.strip()
                    # If this is a function definition with no following content, add pass
                    if re.match(r"^\s*def\s+\w+\s*\(.*\):\s*$", line):
                        ...
                        while j < len(lines):
                            ...
                            if next_indent <= func_indent:
                                ...
                                logger.warning(
                                    f"   Added 'pass' to function at line {i + 1}"
                                )
                                break
                            else:
                                has_body = True
                ...
                cleaned_code = "\n".join(fixed_lines)
                logger.info("✅ Applied emergency syntax fixes")
    ...
    def _remove_orphaned_parametrize(self, test_code: str) -> str:
        """
        Remove @pytest.mark.parametrize decorators that have mismatched parameters.
        This fixes cases where decorators reference parameters not in function signature.

        Args:
            test_code: Test code to clean

        Returns:
            Cleaned test code with orphaned parametrize decorators removed
        """
        lines = test_code.split("\n")
        fixed_lines = []
        i = 0
        ...
        while i < len(lines):
            ...
            if stripped.startswith("@pytest.mark.parametrize"):
                ...
                # Search for the function definition (skip empty lines and comments)
                for j in range(i + 1, min(i + 10, len(lines))):
                    next_line = lines[j]
                    next_stripped = next_line.strip()

                    # Skip empty lines, comments, and other decorators
                    if (
                        not next_stripped
                        or next_stripped.startswith("#")
                        or next_stripped.startswith("@")
                    ...
                    # Check if this is a function definition
                    func_match = re.match(r"^\s*def\s+(\w+)\s*\((.*?)\):", next_line)
                    if func_match:
                        func_line_idx = j
                        func_name = func_match.group(1)
                        ...
                        if params_str.strip():
                            # Parse parameters (handle defaults, type hints, etc.)
                            params = [
                                p.strip().split("=")[0].split(":")[0]
                                for p in params_str.split(",")
                            ]
                            func_params = set(p.strip() for p in params if p.strip())
                        break

                if func_line_idx is None:
                    # No function found after decorator - remove the decorator
                    logger.warning(
                        f"⚠️  Removing orphaned @pytest.mark.parametrize decorator at line {i + 1}"
                    )
                    removed_decorators += 1
                    i += 1
                    continue

                # Extract parameter names from the parametrize decorator
                # Look for the parameter list in the decorator
                decorator_params = set()
                decorator_line = stripped

                # Handle multi-line parametrize decorators
                j = i
                while j < len(lines) and (
                    lines[j].strip().startswith("@pytest.mark.parametrize")
                    or (
                        j > i
                ...
                # Extract parameter names from the decorator
                # Look for the first argument (parameter names)
                try:
                    # Find the opening parenthesis after parametrize
                    start = decorator_line.find("(")
                    if start != -1:
                        # Find the first comma or closing parenthesis
                        paren_count = 0
                        end = start
                        for k, char in enumerate(decorator_line[start:], start):
                            if char == "(":
                                paren_count += 1
                            elif char == ")":
                                paren_count -= 1
                        ...
                        if end > start:
                            param_str = decorator_line[start + 1 : end].strip()
                    ...
                    # If parsing fails, assume decorator is malformed
                    decorator_params = set()

                # Check if decorator parameters match function parameters
                if decorator_params and not decorator_params.issubset(func_params):
                    # Mismatch detected - remove the decorator
                    logger.warning(
                        f"⚠️  Removing @pytest.mark.parametrize decorator for '{func_name}' - parameter mismatch"
                    )
                    logger.warning(f"   Decorator params: {decorator_params}")
                    logger.warning(f"   Function params: {func_params}")
                    removed_decorators += 1
                    i += 1
                    continue

                # Decorator is valid, keep it
                fixed_lines.append(line)
                i += 1
            else:
                # Not a parametrize decorator, keep it
                fixed_lines.append(line)
                i += 1

        if removed_decorators > 0:
            logger.info(
                f"✅ Removed {removed_decorators} orphaned @pytest.mark.parametrize decorator(s)"
            )
