        ...
        if lines_to_skip:
            ...
            if error_output:
                ...
                if dup_match:
                    ...
                    # Simple approach: Remove ALL @pytest.mark.parametrize decorators with this param name
                    ...
                    for i, line in enumerate(lines):
                        if skip_next_n_lines > 0:
                            logger.info(f"     Skipping continuation line {i+1}: {line.strip()[:60]}")
                            skip_next_n_lines -= 1
                            continue

                        stripped = line.strip()
                        # Check if this line contains the duplicate parametrize
                        if '@pytest.mark.parametrize' in line and (f'"{dup_param_name}"' in line or f"'{dup_param_name}'" in line):
                            if not first_occurrence_found:
                                # Keep the first occurrence
                                first_occurrence_found = True
                                result_lines.append(line)
                                logger.info(f"   Keeping first occurrence at line {i+1}: {stripped[:60]}")
                            else:
                                # Remove subsequent occurrences
                                logger.warning(f"   Removing duplicate at line {i+1}: {stripped[:60]}")
                                # Check if multi-line (doesn't end with ))
                                if not stripped.endswith(')'):
                                    ...
                                    while j < len(lines) and not lines[j].strip().endswith(')'):
                                        ...
                                        j += 1
                                    if j < len(lines):
                                        skip_next_n_lines += 1  # Skip the closing line too
                                    logger.info(f"     Will skip {skip_next_n_lines} continuation lines")
                    ...
                    if first_occurrence_found:
                        result = "\n".join(result_lines)
                        logger.info(f"✅ Fixed duplicate parametrization via fallback (removed duplicates of '{dup_param_name}')")
                    else:
