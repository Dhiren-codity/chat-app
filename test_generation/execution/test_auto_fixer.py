...
"""
    ...
    def _remove_hallucinated_imports(self, test_code: str) -> str:
        """Remove hallucinated imports from test code."""
        lines = test_code.split("\n")
        cleaned_lines = []

        # Common hallucinated imports to remove
        ...
        for line in lines:
            # Check if this is a hallucinated import
            is_hallucinated = any(pattern in line for pattern in hallucinated_patterns)

            if is_hallucinated:
                # Replace with TODO comment
                cleaned_lines.append(
                    f"# TODO: {line.strip()} - hallucinated import removed"
                )
            else:
    ...
    def _remove_bad_imports_from_code(self, test_code: str) -> str:
        """
        Remove bad/hallucinated imports from test code.
        Only allows imports of:
        1. Standard library modules (pytest, flask, datetime, etc.)
        2. Relative imports (starting with .)
        3. Modules that actually exist in the codebase
        ...
        # Known standard library and common third-party imports
        known_good_imports = [
            "pytest",
            "unittest",
            "mock",
        ...
        for line in lines:
            ...
            if stripped.startswith("from ") or stripped.startswith("import "):
                # Allow known standard library imports
                if any(
                    f"import {module}" in line or f"from {module}" in line
                    for module in known_good_imports
                ):
                    cleaned_lines.append(line)
                    continue
                ...
                try:
                    ...
                    if module_name:
                        ...
                        if module_name.startswith("."):
                            # Relative imports are OK - they refer to project modules
                            cleaned_lines.append(line)
                            continue

                        # Check if module exists (either in stdlib or installed packages or codebase)
                        try:
                            ...
                            # Module doesn't exist - this is a hallucinated import
                            logger.info(
                                f"  🗑️ Removing hallucinated import (module not found): {stripped[:100]}"
                            )
                            removed_imports.append(stripped)
                    else:
                        # Couldn't parse module name - keep it to be safe
                        cleaned_lines.append(line)

                except Exception:
                    # If validation fails for any reason, keep the import to be safe
                    logger.warning(
                        f"  ⚠️ Failed to validate import, keeping it: {stripped[:80]}"
                    )
                    cleaned_lines.append(line)
            else:
    ...
    def _create_minimal_valid_file(self, test_code: str) -> str:
        ...
        # Extract only imports from the original code
        lines = test_code.split("\n")
        import_lines = []
        ...
        # Create proper placeholder with imports at the top
        placeholder_lines = []

        # Add imports if any
        if import_lines:
            placeholder_lines.extend(import_lines)
...
"""
    ...
    def auto_fix_test(
        ...
        """
        # Remove hallucinated imports first
        logger.info("🔍 Removing hallucinated imports")
