...
class TestAutoFixer:
    ...
    def extract_error_details(
        ...
        if language == "python":
            ...
            java_error_pattern = re.compile(r'\[ERROR\]\s+(.+\.java):\[(\d+),(\d+)\]\s*(.+)')
            simple_error_pattern = re.compile(r'(.+\.java):\[(\d+),(\d+)\]\s*(.+)')

            for line in lines:
                match = java_error_pattern.search(line) or simple_error_pattern.search(line)
                if match:
                    file_path, line_num, col_num, error_msg = match.groups()
                    error_info["errors"].append({
                        "type": "CompilationError",
...
"""
    ...
    def keep_only_passing_tests(
        ...
        if language == "python":
            return self._keep_only_passing_tests_python(test_file_path, initial_passing_tests)
        elif language == "go":
            return self._keep_only_passing_tests_go(test_file_path, initial_passing_tests)
        elif language == "java":
            return self._keep_only_passing_tests_java(test_file_path, initial_passing_tests)
        elif language in ["javascript", "typescript"]:
            return self._keep_only_passing_tests_javascript(test_file_path, initial_passing_tests)
        else:
    ...
    def _keep_only_passing_tests_python(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                logger.info(f"🔍 Test functions found in file: {len(test_functions)} total")
                ...
                for t in test_functions:
                    ...
                    for passing in initial_passing_tests:
                        ...
                        # Match exact name OR if file_test_name ends with passing_base (for nested classes)
                        if (file_test_name == passing_base
                            or file_test_name.endswith("::" + passing_base)
                ...
                if len(passing_tests) > 0:
                    logger.info(f"✅ Successfully matched {len(passing_tests)} passing tests from initial run")
                    logger.info(f"🗑️ Removing {len(failing_tests)} failing tests")
                    logger.info(f"⚡ SKIPPING individual test execution - we already know what passes!")
                    ...
                    logger.info(f"🗑️ Removing functions: {list(failing_function_names)[:10]}")
                ...
                if len(passing_tests) == 0 and len(initial_passing_tests) > 0:
                    logger.warning(f"⚠️ MATCHING FAILED: 0 matches from {len(initial_passing_tests)} passing tests")
                    logger.warning(f"⚠️ This is likely a bug in test name matching logic")
    ...
    def _keep_only_passing_tests_java(
        ...
        try:
            ...
            test_methods = []
            for match in re.finditer(r'@Test\s+(?:public\s+)?void\s+(\w+)\s*\(', test_code):
            ...
            while build_dir != build_dir.parent:
                if (build_dir / 'pom.xml').exists() or (build_dir / 'build.gradle').exists():
                    break
            ...
            for test_method in test_methods:
                ...
                try:
                    ...
                    if result.returncode != 0:
                        # Try Gradle
                        result = subprocess.run(
                            ['gradle', 'test', '--tests', f'{full_class_name}.{test_method}'],
                            cwd=build_dir,
                            capture_output=True,
                            text=True,
                            timeout=60
    ...
    def _keep_only_passing_tests_javascript(
        ...
        try:
            ...
            for match in re.finditer(r"(?:test|it)\s*\(\s*['\"]([^'\"]+)['\"]", test_code):
            ...
            for test_name in test_names:
                ...
                try:
                    ...
                    try:
                        relative_path = str(Path(test_file_path).relative_to(project_dir))
                    except ValueError:
                    ...
                    result = subprocess.run(
                        ['npm', 'test', '--', relative_path, '--testNamePattern', test_name],
                        cwd=project_dir,
                        capture_output=True,
                        text=True,
                        timeout=30
            ...
            for line in lines:
                ...
                if match:
                    ...
                    if current_test in passing_tests:
                        ...
                        if brace_count == 0 and paren_count == 0 and ('}' in line or ')' in line):
                            in_test_block = False
                    else:
                        in_test_block = False
                elif in_test_block:
