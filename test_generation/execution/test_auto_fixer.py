...
4. For ModuleNotFoundError:
   - Check if test file is in a subdirectory (e.g., routes/test_*.py)
   - If test is in routes/ and importing from parent directory:
     * ADD: import sys, os at the top
     * ADD: sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
     * THEN: from app import app (or other parent modules)
   - If importing from same directory: from module_name import function (no package prefix)
   - Example: In routes/test_reactions.py:
     * Same dir: from reactions import require_auth  ✅
     * Parent dir: sys.path setup + from app import app  ✅
     * WRONG: from routes.reactions import ...  ❌
...
"""
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                for t in test_functions:
                    ...
                    if test_name in initial_passing_tests:
                        ...
                        logger.info(
                            f"  ❌ No match: {t} -> {test_name} (not in initial passing list)"
                        )
    ...
    def _create_minimal_valid_file(self, test_code: str) -> str:
        ...
        # THEN: Keep ONLY imports and sys.path setup
        lines = test_code.split("\n")
        cleaned_lines = []
        in_code_block = False
        in_sys_path_setup = False
        ...
        for line in lines:
            ...
            if stripped.startswith("import ") or stripped.startswith("from "):
                cleaned_lines.append(line)
                in_sys_path_setup = False
            # Keep sys.path manipulation lines (critical for subdirectory tests)
            elif "sys.path" in line and not in_code_block:
                cleaned_lines.append(line)
                in_sys_path_setup = True
            # Keep os.path or Path lines right after sys.path setup
            elif in_sys_path_setup and any(
                x in line for x in ["os.path", "os.dirname", "__file__", "abspath"]
            ):
                cleaned_lines.append(line)
            # Keep empty lines and comments before we hit code
            elif (not stripped or stripped.startswith("#")) and not in_code_block:
                cleaned_lines.append(line)
                in_sys_path_setup = False
            # Stop at first test/fixture/class definition
            elif stripped and any(
                stripped.startswith(x)
                for x in ["def test_", "@pytest", "class ", "def "]
            ):
                in_code_block = True
            elif stripped:
                in_code_block = True
