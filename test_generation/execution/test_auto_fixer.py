...
"""
    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # SANITY CHECK: Only block if NO tests matched AND we had passing tests
                # This indicates a real bug, not just fewer tests than expected
                if len(passing_tests) == 0 and len(initial_passing_tests) > 0:
                    logger.warning(f"⚠️ WARNING: No tests matched out of {len(initial_passing_tests)} passing tests!")
                    logger.warning("⚠️ This suggests a bug in the matching logic")
                    logger.warning("⚠️ Will proceed to test each function individually as fallback")
                    # Don't return - let it fall through to individual testing
            else:
    ...
    def _remove_test_functions(
        ...
        # CRITICAL: Validate syntax and fix any issues
        cleaned_code = self._validate_and_fix_syntax(cleaned_code)
    ...
    def _validate_and_fix_syntax(self, test_code: str) -> str:
        """
        Validate Python syntax and fix common issues after function removal.

        Args:
            test_code: Code that might have syntax errors
        ...
        try:
            ...
            logger.warning(f"⚠️ Syntax error after function removal: {e}")
            logger.warning(f"   Line {e.lineno}: {e.text if e.text else 'N/A'}")
        ...
        while i < len(lines):
            ...
            # Check for empty class definition (class Foo: followed by nothing)
            if re.match(r"^(\s*)class \w+.*:\s*$", line):
                indent = len(line) - len(line.lstrip())
                # Look ahead to see if there's anything in the class
                j = i + 1
                has_body = False
                ...
                while j < len(lines):
                    ...
                    # If back to same or lower indent, class is empty
                ...
                if not has_body:
                    # Add pass to empty class
                    fixed_lines.append(line)
                    fixed_lines.append(" " * (indent + 4) + "pass")
                    logger.info(f"  ✅ Added 'pass' to empty class at line {i+1}")
                    i += 1
                    continue

            # Check for empty function definition
            if re.match(r"^(\s*)def \w+.*:\s*$", line):
                ...
                if not has_body:
                    # Add pass to empty function
                    fixed_lines.append(line)
                    fixed_lines.append(" " * (indent + 4) + "pass")
                    logger.info(f"  ✅ Added 'pass' to empty function at line {i+1}")
                    i += 1
                    continue
        ...
        try:
            ...
            logger.error(f"❌ Still has syntax errors after fixes: {e}")
            logger.error(f"   Line {e.lineno}: {e.text if e.text else 'N/A'}")
            # Return the fixed version anyway - it's better than the original
            return fixed_code
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt >= self.max_retries:
                ...
                if cleaned_code and cleaned_code != current_code:
                    ...
                    # CRITICAL: Return immediately - don't re-test the cleaned file
                    # The nuclear option already tested each function individually
                    # Re-testing can fail due to fixture/context issues even though individual tests passed
                    logger.info(f"✅ Nuclear option complete - returning {remaining_tests} passing tests")
                    logger.info("   (Each test passed individually, so file should work)")
                    Path(test_file_path).write_text(current_code, encoding="utf-8")
                    fix_history.append(
