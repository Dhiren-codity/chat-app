    ...
    def keep_only_passing_tests(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # Match test functions to initial passing tests
                passing_tests = []
                for t in test_functions:
                    test_name = t.split("::")[-1]
                    if test_name in initial_passing_tests:
                        passing_tests.append(t)
                        logger.info(f"  ✅ Matched: {t} -> {test_name}")
                    else:
                        logger.info(f"  ❌ No match: {t} -> {test_name} (not in initial passing list)")

                failing_tests = [t for t in test_functions if t not in passing_tests]
                logger.info(f"📊 {len(passing_tests)} passed, {len(failing_tests)} failed")
            else:
