...
"""
    ...
    def _remove_test_functions(
        ...
        # CRITICAL: Clean up orphaned imports at wrong indentation
        cleaned_code = self._fix_misplaced_imports(cleaned_code)

        # CRITICAL: Final safety check - ensure no classes without bodies
        cleaned_code = self._ensure_valid_class_bodies(cleaned_code)
                ...
                while j < len(lines):
                    ...
                    if not in_docstring:
                        if next_stripped.startswith('"""') or next_stripped.startswith("'''"):
                            docstring_quote = '"""' if next_stripped.startswith('"""') else "'''"
                            # Check if docstring closes on same line
                            if next_stripped.count(docstring_quote) >= 2 or next_stripped.endswith(docstring_quote) and len(next_stripped) > 3:
                        ...
                        # Inside docstring, look for closing quote
                        if docstring_quote in next_stripped:
                            in_docstring = False
                            docstring_quote = None
                        j += 1
                        continue
