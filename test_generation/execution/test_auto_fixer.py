...
"""
    ...
    def auto_fix_test(
        ...
        if self.max_retries == 0:
            ...
            # CRITICAL: Check for collection errors (syntax/import errors)
            # These prevent tests from running at all - nuclear option will only make it worse
            ...
            if has_collection_errors:
                logger.error("❌ COLLECTION ERRORS DETECTED - tests cannot be collected/imported!")
                logger.error("   Nuclear option SKIPPED - would only create more syntax errors")
                logger.error("   Keeping original code as-is")
                logger.error("   These errors must be fixed in test generation, not auto-fixer")
                fix_history.append("Collection errors detected - keeping original code")
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt >= self.max_retries:
                ...
                if has_collection_errors:
                    ...
                    logger.error("   These errors must be fixed in test generation, not auto-fixer")
                    Path(test_file_path).write_text(current_code, encoding="utf-8")
                    fix_history.append(f"Attempt {attempt}: Collection errors - keeping original code")
                    return True, current_code, fix_history
