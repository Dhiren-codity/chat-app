    ...
    def _keep_only_passing_tests_python(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # Even if we match 0 passing tests, it's better to create a minimal file
                # than to keep broken tests that fail
                if len(passing_tests) == 0:
                    logger.warning(f"⚠️ No passing tests matched - creating minimal test file")
                    return self._create_minimal_valid_file(test_code)
            else:
