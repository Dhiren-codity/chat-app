    ...
    def _keep_only_passing_tests_python(
        ...
        try:
            ...
            if initial_passing_tests is not None and len(initial_passing_tests) > 0:
                ...
                # CRITICAL FIX: Only apply safeguard if we matched 0 tests
                # If we matched some tests (even if less than expected), proceed with cleanup
                if len(passing_tests) == 0 and len(initial_passing_tests) > 0:
                    ...
                    # Fall back to individual testing rather than keeping all or deleting all
                    # Set to empty lists to trigger individual testing below
                    initial_passing_tests = None

            # Only do individual testing if we don't have initial passing tests
            # or if matching failed (initial_passing_tests was set to None above)
