    ...
    def _remove_parametrize_from_fixtures(self, test_code: str) -> str:
        ...
        while i < len(lines):
            ...
            if stripped.startswith('@pytest.mark.parametrize'):
                ...
                # Check BEFORE parametrize (look back through decorators)
                for m in range(max(0, parametrize_start - 10), parametrize_start):
                    line_text = lines[m].strip()
                    if '@pytest.fixture' in line_text or (line_text == '@fixture' and 'import fixture' not in lines[m]):
                        is_fixture = True
                        break
                    # Stop looking back if we hit a function definition or non-decorator line
                ...
                if not is_fixture:
                    for m in range(parametrize_start, k):
                        if '@pytest.fixture' in lines[m] or '@fixture' in lines[m]:
                            is_fixture = True
                            break
    ...
    def _remove_tests_with_invalid_signatures(self, test_code: str) -> str:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name.startswith('test_'):
                ...
                # Get all names referenced in function body
                referenced_names = set()
                for child in ast.walk(node):
                    if isinstance(child, ast.Name):
                        referenced_names.add(child.id)

                # Check for references to undefined parameters
                # Common pytest fixtures that are OK to reference
                known_fixtures = {'request', 'monkeypatch', 'tmp_path', 'capsys', 'capfd', 'mocker'}

                for name in referenced_names:
                    # If name is used but not in params and not a known fixture
                    if name not in params and name not in known_fixtures:
                        # Check if it looks like it should be a parameter (lowercase, not builtin)
                        if name.islower() and name not in ('self', 'cls') and '_' in name:
                            # This might be a missing parameter
                            logger.info(f"  🗑️ Removing test '{node.name}' (references '{name}' which is not in signature)")

                            # Mark function lines for removal
                            start_line = node.lineno - 1  # Convert to 0-indexed
                            end_line = node.end_lineno  # Already 1 past end in 0-indexed
