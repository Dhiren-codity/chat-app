...
"""
                    ...
                    while i < len(lines):
                        ...
                        if '@pytest.mark.parametrize' in lines[i]:
                            ...
                            for j in range(i, min(i + 10, len(lines))):
                                ...
                                if f'"{dup_param_name}"' in line_text or f"'{dup_param_name}'" in line_text:
                                    param_found = True
    ...
    def _remove_orphaned_decorators(self, test_code: str) -> str:
        ...
        while i < len(lines):
            ...
            # Check if this is a pytest decorator
            if stripped.startswith("@pytest.") or stripped.startswith("@patch") or stripped.startswith("@mock"):
                # Look ahead to find what follows this decorator
                j = i + 1
                decorator_lines = [i]  # Track all decorator/comment/empty lines

                # Collect all consecutive decorators, comments, and empty lines
                while j < len(lines):
                    next_line = lines[j]
                    next_stripped = next_line.strip()

                    if not next_stripped or next_stripped.startswith("#"):
                        # Empty line or comment - part of decorator block
                        decorator_lines.append(j)
                        j += 1
                    elif next_stripped.startswith("@"):
                        # Another decorator - part of decorator block
                        decorator_lines.append(j)
                        j += 1
                    else:
                        # Found something else - check if it's a function
                        break

                # Check if the next non-decorator line is a test function
                if j < len(lines):
                    next_line = lines[j]
                    next_stripped = next_line.strip()

                    if next_stripped.startswith("def test") or next_stripped.startswith("def "):
                        # Valid decorator followed by function - keep all
                        for idx in decorator_lines:
                            result_lines.append(lines[idx])
                        result_lines.append(next_line)
                        i = j + 1
                    else:
                        # Orphaned decorator - not followed by function
                        logger.warning(f"  🧹 Removing orphaned decorator at line {i+1}: {stripped[:60]}")
                        logger.warning(f"     Not followed by function (next line: {next_stripped[:60]})")
                        # Skip all decorator lines
                        i = j
                else:
                    # Decorator at end of file with no function - orphaned
                    logger.warning(f"  🧹 Removing orphaned decorator at end of file (line {i+1}): {stripped[:60]}")
                    i = j
            else:
        ...
        # Count how many decorators were removed
        original_decorator_count = sum(1 for line in lines if line.strip().startswith("@pytest."))
        cleaned_decorator_count = sum(1 for line in result_lines if line.strip().startswith("@pytest."))
        removed_count = original_decorator_count - cleaned_decorator_count

        if removed_count > 0:
            logger.info(f"✅ Removed {removed_count} orphaned @pytest decorator(s)")
