...
@pytest.mark.parametrize("password, expected", [
    ...
    ("NoSpecial1", {'valid': False, 'messages': ['Password must contain at least one special character']}),
...
@pytest.mark.parametrize("profile, expected", [
    # Happy path
    ({"username": "validuser", "email": "user@example.com", "password": "strongpassword"}, {'valid': True, 'errors': {}}),
    # Error case: invalid email
    ({"username": "validuser", "email": "userexample.com", "password": "strongpassword"}, {'valid': False, 'errors': {'email': 'Invalid email format'}}),
    # Edge case: missing username
    ({"email": "user@example.com", "password": "strongpassword"}, {'valid': False, 'errors': {'username': 'Invalid username format'}}),
...
@pytest.mark.parametrize("bio, max_length, expected", [
    ...
    ("   ", 500, ""),  # Edge case: whitespace only
    ("A" * 600, 500, "A" * 500),  # Edge case: bio longer than max_length
    ("Bio with null byte\x00", 500, "Bio with null byte"),  # Error case: null byte
])
