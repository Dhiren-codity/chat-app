...
@pytest.mark.parametrize("password, expected", [
    ...
    ("short", {'valid': False, 'messages': [
        'Password must be at least 8 characters',
        'Password must contain at least one uppercase letter',
        'Password must contain at least one digit',
        'Password must contain at least one special character'
    ]}),
    ("NoDigitsOrSpecials", {'valid': False, 'messages': [
        'Password must contain at least one digit',
        'Password must contain at least one special character'
    ]}),
    ("Valid1Password!", {'valid': True, 'messages': []}),
    ("noupper1!", {'valid': False, 'messages': [
        'Password must contain at least one uppercase letter'
    ]}),
])
...
@pytest.mark.parametrize("profile, expected", [
    # Happy path
    ({"username": "validUser", "email": "user@example.com", "password": "strongPass123"},
     {"valid": True, "errors": {}}),
    # Error case: Invalid username
    ({"username": "us", "email": "user@example.com", "password": "strongPass123"},
     {"valid": False, "errors": {"username": "Invalid username format"}}),
    # Error case: Invalid email
    ({"username": "validUser", "email": "userexample.com", "password": "strongPass123"},
     {"valid": False, "errors": {"email": "Invalid email format"}}),
    # Edge case: Missing password
    ({"username": "validUser", "email": "user@example.com"},
     {"valid": True, "errors": {}}),
    # Edge case: Invalid password
    ({"username": "validUser", "email": "user@example.com", "password": "short"},
     {"valid": False, "errors": {"password": "Password too short"}}),
])
...
@pytest.mark.parametrize("bio, max_length, expected", [
    ...
    ("   Surrounded by spaces   ", 500, "Surrounded by spaces"),  # Edge case: leading/trailing spaces
    ("A" * 600, 500, "A" * 500),  # Edge case: bio longer than max_length
    ("Bio with null byte \x00 in it", 500, "Bio with null byte  in it"),  # Error case: null byte
])
