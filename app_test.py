...
class TestStartTyping:
    ...
    def test_start_typing_missing_data(self, json_data, expected_status, client):
...
class TestCreateGroup:
    ...
    def test_create_group_invalid_creator_id(self, mock_create_group, client):
        response = client.post('/api/groups/create', json={
            'name': 'Test Group',
        ...
        assert response.status_code == 400  # Assuming the function should return 400 for invalid creator_id
...
class TestRenameGroup:
    ...
    @pytest.mark.parametrize("group_id,new_name,user_id,expected_status,expected_response", [
        ...
        (1, '', 1, 400, {'success': False, 'message': 'New name cannot be empty'}),
        (1, 'New Group Name', None, 400, {'success': False, 'message': 'User ID is required'}),
    ])
    @patch('app.group_manager.update_group_name')
...
class TestCustomizeGroupTitle:
    ...
    def test_customize_group_title_success(self, mock_customize_room_title, client):
        mock_customize_room_title.return_value = {'success': True, 'message': 'Title updated'}
        
        response = client.post('/api/groups/1/customize', json={'custom_title': 'New Title', 'user_id': 1})
        
        assert response.status_code == 200
        assert response.get_json() == {'success': True, 'message': 'Title updated'}
    ...
    def test_customize_group_title_invalid_user(self, mock_customize_room_title, client):
        mock_customize_room_title.return_value = {'success': False, 'message': 'Invalid user'}
        
        response = client.post('/api/groups/1/customize', json={'custom_title': 'New Title', 'user_id': 999})
        
        assert response.status_code == 200
        assert response.get_json() == {'success': False, 'message': 'Invalid user'}
        mock_customize_room_title.assert_called_once_with(1, 999, 'New Title')
...
class TestUpdateDisplayName:
    ...
    def test_update_display_name_various_inputs(self, mock_update_display_name, client, user_id, display_name, expected_status):
        mock_update_display_name.return_value = {'display_name': display_name} if display_name else None
