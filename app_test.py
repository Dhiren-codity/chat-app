...
class TestCreateGroup:
    ...
    @patch('app.group_manager')
    @pytest.mark.parametrize("creator_id, member_ids, expected_status", [
        (None, [2, 3], 400),  # Missing creator_id
        (1, None, 200),       # Missing member_ids, should default to empty list
        (1, [], 200),         # Empty member_ids
    ])

    def test_create_group_various_inputs(self, mock_group_manager, client, creator_id, member_ids, expected_status):
        # Setup mock
        mock_group_manager.create_group.return_value = {'id': 1, 'name': 'Test Group'}
        ...
        if expected_status == 200:
            mock_group_manager.create_group.assert_called_once_with('Test Group', creator_id, member_ids or [])
        else:
            mock_group_manager.create_group.assert_not_called()
