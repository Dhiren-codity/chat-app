...
def test_get_typing_users_invalid_room_id(mock_get_typing_users, client):
    ...
    assert 'Invalid room_id' in response.get_data(as_text=True)
...
def test_customize_group_title_success(mock_customize_room_title, client):
    # Setup mock response
...
def test_customize_group_title_missing_custom_title(mock_customize_room_title, client):
...
def test_customize_group_title_missing_user_id(mock_customize_room_title, client):
...
def test_customize_group_title_internal_error(mock_customize_room_title, client):
    # Setup mock to raise an exception
    mock_customize_room_title.side_effect = Exception("Internal Error")
    # Make request
    response = client.post('/api/groups/1/customize', json={'custom_title': 'New Title', 'user_id': 1})
    # Assert
...
def test_mark_message_read_various_user_ids(mock_message_status_manager, client, user_id, expected_status):
    # Setup mock
