...
def test_stop_typing_invalid_json(mock_typing_indicator, client):
    ...
    # Assert the response
    assert response.status_code == 400  # Assuming 400 for invalid JSON
    mock_typing_indicator.user_stopped_typing.assert_not_called()
...
def test_rename_group_success(mock_update_group_name, client):
    ...
    # Assert
    assert response.status_code == 200
    assert response.get_json() == {'success': True, 'message': 'Group name updated'}
...
def test_rename_group_update_failure(mock_update_group_name, client):
    ...
    mock_update_group_name.return_value = {'success': False, 'message': 'Update failed'}
    # Make request
    response = client.post('/api/groups/1/rename', json={'new_name': 'New Group Name', 'user_id': 1})
    # Assert
    assert response.status_code == 200
    assert response.get_json() == {'success': False, 'message': 'Update failed'}
    mock_update_group_name.assert_called_once_with(1, 'New Group Name', 1)
