...
def test_get_messages_happy_path(mock_query, client):
    ...
    assert data[0]['created_at'] == "2023-10-01T12:00:00"
@patch('app.Message.query')
...
def test_update_status_missing_user_id(mock_status_manager, client):
    ...
    # Assert the response
    assert response.status_code == 400  # Assuming 400 for missing user_id
...
def test_update_status_missing_is_online(mock_status_manager, client):
    ...
    # Assert the response
    assert response.status_code == 400  # Assuming 400 for missing is_online
    mock_status_manager.update_user_status.assert_not_called()
@patch('app.status_manager')
...
def test_update_status_invalid_user_id(mock_status_manager, client):
    ...
    # Assert the response
    assert response.status_code == 400  # Assuming 400 for invalid user_id
    mock_status_manager.update_user_status.assert_not_called()
@patch('app.status_manager')
...
def test_update_status_invalid_is_online(mock_status_manager, client):
    ...
    # Assert the response
    assert response.status_code == 400  # Assuming 400 for invalid is_online
    mock_status_manager.update_user_status.assert_not_called()
...
def test_stop_typing_invalid_json(mock_typing_indicator, client):
    # Make a POST request with invalid JSON
    response = client.post('/api/typing/stop', data='not a json')
    # Assert the response
    assert response.status_code == 400  # Assuming 400 for bad request
    mock_typing_indicator.user_stopped_typing.assert_not_called()
...
def test_create_group_invalid_creator_id(mock_group_manager, client):
