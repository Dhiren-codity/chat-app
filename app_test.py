...
class TestUpdateStatus:
    ...
    def test_update_status_success(self, mock_status_manager, client):
        # Mock the update_user_status method
...
def test_mark_message_read_missing_user_id(mock_message_status_manager, client):
    # Make request without user_id
    response = client.post('/api/messages/1/read', json={})
    # Assert
    assert response.status_code == 400
    assert response.get_json() == {'error': 'Failed to update status'}
    mock_message_status_manager.mark_as_read.assert_not_called()
