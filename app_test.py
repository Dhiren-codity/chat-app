...
class TestGetStatus:
    ...
    @pytest.mark.parametrize("user_id,expected_status", [
        (1, {'status': 'active'}),
        (2, {'status': 'inactive'}),
        (3, None),  # Simulate user not found
    ])
    @patch('app.status_manager')
...
class TestCreateGroup:
    ...
    def test_create_group_invalid_creator_id(self, mock_create_group, client):
        ...
        assert response.status_code == 400  # Assuming the function returns 400 for invalid creator_id
...
class TestRenameGroup:
    ...
    @pytest.mark.parametrize("group_id,new_name,user_id,expected_status,expected_response", [
        (1, 'New Name', 1, 200, {'success': True}),
        (1, '', 1, 200, {'success': False, 'error': 'Invalid name'}),
        (1, 'New Name', None, 200, {'success': False, 'error': 'User ID required'}),
    ])
    @patch('app.group_manager.update_group_name')
...
class TestCustomizeGroupTitle:
    ...
    def test_customize_group_title_success(self, mock_customize_room_title, client):
        ...
        assert response.status_code == 200
        assert response.get_json() == {'success': True}
        mock_customize_room_title.assert_called_once_with(1, 123, 'New Group Title')
    ...
    @pytest.mark.parametrize("group_id, user_id, custom_title, expected_status", [
        (1, 123, 'New Group Title', 200),
        (1, None, 'New Group Title', 400),
        (1, 123, None, 400),
    ])
    @patch('app.group_manager.customize_room_title')
...
class TestUploadProfilePicture:
    ...
    def test_upload_profile_picture_various_inputs(self, mock_upload, client, user_id, image_data, filename, expected_status):
        ...
        response = client.post(f'/api/profile/{user_id}/picture', json={
