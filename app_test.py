...
class TestStopTyping:
    ...
    @pytest.mark.parametrize("payload", [
        {},  # Missing both room_id and user_id
        {'room_id': '123'},  # Missing user_id
        {'user_id': '456'},  # Missing room_id
    ])
    @patch('app.typing_indicator')

    def test_stop_typing_missing_data(self, mock_typing_indicator, client, payload):
        # Make request
    ...
    def test_stop_typing_internal_error(self, mock_typing_indicator, client):
        # Simulate an internal error in the typing_indicator
        mock_typing_indicator.user_stopped_typing.side_effect = Exception("Internal Error")
...
class TestCreateGroup:
    ...
    @pytest.mark.parametrize("json_data, expected_status", [
        ...
        ({'creator_id': 1, 'member_ids': [2, 3]}, 400),
        ({'name': 'Test Group', 'member_ids': [2, 3]}, 400),
    ])
    @patch('app.group_manager.create_group')

    def test_create_group_various_inputs(self, mock_create_group, client, json_data, expected_status):
        mock_create_group.return_value = {'id': 1, 'name': 'Test Group', 'creator_id': 1, 'member_ids': [2, 3]}
        
        response = client.post('/api/groups/create', json=json_data)
...
class TestRenameGroup:
    ...
    def test_rename_group_invalid_user_id(self, mock_update_group_name, client):
        ...
        assert response.status_code == 400
        assert response.get_json() == {'success': False, 'message': 'Invalid user ID'}
    ...
    @pytest.mark.parametrize("group_id,new_name,user_id,expected_status,expected_response", [
        (1, 'New Group Name', 1, 200, {'success': True, 'message': 'Group name updated successfully'}),
        (1, '', 1, 400, {'success': False, 'message': 'Invalid group name'}),
        (1, 'New Group Name', None, 400, {'success': False, 'message': 'Invalid user ID'}),
    ])
    @patch('app.group_manager.update_group_name')
...
class TestCustomizeGroupTitle:
    ...
    def test_customize_group_title_happy_path(self, mock_customize_room_title, client):
    ...
    def test_customize_group_title_invalid_user_id(self, mock_customize_room_title, client):
        mock_customize_room_title.return_value = {'success': False, 'error': 'Invalid user'}
        response = client.post('/api/groups/1/customize', json={'custom_title': 'New Title', 'user_id': 999})
        assert response.status_code == 200
    ...
    def test_customize_group_title_edge_cases(self, mock_customize_room_title, client, group_id, user_id, custom_title, expected_status):
        mock_customize_room_title.return_value = {'success': True}
...
class TestUpdateDisplayName:
    ...
    def test_update_display_name_success(self, mock_profile_manager, client):
        mock_profile_manager.update_display_name.return_value = {'display_name': 'new_name'}
        
        response = client.put('/api/profile/1/display-name', json={'display_name': 'new_name'})
        
        assert response.status_code == 200
        assert response.get_json() == {'display_name': 'new_name'}
