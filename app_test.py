...
class TestGetMessages:
    ...
    def test_get_messages_happy_path(self, mock_message, client):
        ...
        mock_message.query.filter_by.return_value.order_by.return_value.all.return_value = [
            Mock(id=1, user_id=1, content='Hello', created_at=datetime(2023, 10, 1)),
            Mock(id=2, user_id=2, content='Hi', created_at=datetime(2023, 10, 2))
        ]
...
class TestCreateGroup:
    ...
    def test_create_group_missing_name(self, mock_create_group, client):
        ...
        assert response.status_code == 400  # Assuming the function should return 400 for missing name
    ...
    def test_create_group_no_member_ids(self, mock_create_group, client):
        mock_create_group.return_value = {'id': 3, 'name': 'No Members Group', 'creator_id': 1, 'member_ids': []}
        ...
        data = response.get_json()
        assert data['id'] == 3
        assert data['name'] == 'No Members Group'
        assert data['creator_id'] == 1
        assert data['member_ids'] == []
...
class TestRenameGroup:
    ...
    def test_rename_group_failure(self, mock_update_group_name, client):
        mock_update_group_name.return_value = {'success': False, 'message': 'Failed to update group name'}
        
        response = client.post('/api/groups/1/rename', json={'new_name': '', 'user_id': 1})
        
        assert response.status_code == 200
        assert response.get_json() == {'success': False, 'message': 'Failed to update group name'}
        mock_update_group_name.assert_called_once_with(1, '', 1)

    @pytest.mark.parametrize("group_id,new_name,user_id,expected_response", [
        (1, 'New Group Name', 1, {'success': True, 'message': 'Group name updated successfully'}),
        (1, '', 1, {'success': False, 'message': 'Failed to update group name'}),
        (1, 'Another Name', 2, {'success': True, 'message': 'Group name updated successfully'}),
    ])
    @patch('app.group_manager.update_group_name')

    def test_rename_group_various_inputs(self, mock_update_group_name, client, group_id, new_name, user_id, expected_response):
...
class TestCustomizeGroupTitle:
    ...
    @pytest.mark.parametrize("group_id, custom_title, user_id, expected_status", [
        ...
        (1, 'New Title', None, 400),  # Missing user_id
    ])
    @patch('app.group_manager.customize_room_title')
...
class TestGetRoomMessageStatuses:
    ...
    def test_get_room_message_statuses_no_user_id(self, mock_get_statuses, client):
        # Setup mock response
        mock_get_statuses.return_value = [{'message_id': 1, 'status': 'delivered'}]
        ...
        # Assert
        assert response.status_code == 200
        assert response.get_json() == [{'message_id': 1, 'status': 'delivered'}]
...
class TestUploadProfilePicture:
    ...
    @pytest.mark.parametrize("user_id, image_data, filename, expected_status, expected_response", [
        (1, 'fake_image_data', 'profile.jpg', 200, {'success': True}),
        (1, None, 'profile.jpg', 400, {'error': 'Upload failed'}),
        (1, 'fake_image_data', None, 200, {'success': True}),
    ])
    @patch('app.profile_manager.upload_profile_picture')

    def test_various_inputs(self, mock_upload, client, user_id, image_data, filename, expected_status, expected_response):
        mock_upload.return_value = expected_response if image_data else None
...
class TestUpdateBio:
    ...
    def test_update_bio_various_cases(self, mock_update_bio, client, user_id, bio, expected_status, expected_response):
