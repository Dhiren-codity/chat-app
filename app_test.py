...
class TestCustomizeGroupTitle:
    ...
    def test_customize_group_title_happy_path(self, mock_group_manager, client):
...
class TestMarkMessageRead:
    ...
    def test_mark_message_read_various_inputs(self, mock_message_status_manager, client, message_id, user_id, expected_status):
        # Setup mock
        mock_message_status_manager.mark_as_read.return_value = {'status': 'read'} if user_id is not None else None
...
class TestUpdateDisplayName:
    ...
    @pytest.mark.parametrize("user_id, display_name, expected_status, expected_response", [
        (1, 'new_name', 200, {'display_name': 'new_name'}),
