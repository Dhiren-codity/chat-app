...
class TestRenameGroup:
    ...
    @pytest.mark.parametrize("group_id,new_name,user_id,expected_status,expected_success", [
        (1, 'New Group Name', 1, 200, True),
        (1, '', 1, 400, False),
        (1, 'New Group Name', None, 400, False),
    ])
    @patch('app.group_manager.update_group_name')

    def test_rename_group_various_inputs(self, mock_update_group_name, client, group_id, new_name, user_id, expected_status, expected_success):
        mock_update_group_name.return_value = {'success': expected_success, 'message': 'Test message'}
