...
class TestCreateGroup:
    ...
    @pytest.mark.parametrize("payload, expected_status", [
        ({'name': 'Group 1', 'creator_id': 1}, 200),
        ({'name': '', 'creator_id': 1}, 400),
        ({'creator_id': 1}, 400),
        ({'name': 'Group 2'}, 400),
    ])
    @patch('app.group_manager')

    def test_create_group_various_inputs(self, mock_group_manager, client, payload, expected_status):
        if expected_status == 200:
            mock_group_manager.create_group.return_value = {'id': 3, 'name': payload['name']}
