...
def test_send_message_no_user_found(mock_user, mock_db, client):
...
def test_send_message_no_room_users(mock_user, mock_db, client):
    # Mock user query
    mock_user.query.filter_by.return_value.first.return_value = Mock(
        id=1,
        username='testuser'
    )
    # Mock room users to return empty list
    ...
    assert response.get_json() == {'message_id': 123, 'sent': True}
...
def test_mark_notification_read_already_read(mock_db, mock_notification, client):
    # Setup mock notification
