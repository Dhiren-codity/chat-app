...
def test_reaction_manager_init_edge_case_empty_session():
    manager = ReactionManager(db_session=MockDBSession())
    assert isinstance(manager.db, MockDBSession)

def test_reaction_manager_init_edge_case_large_session():
    large_session = MockDBSession()
    manager = ReactionManager(db_session=large_session)
    assert manager.db == large_session
...
def test_get_user_reactions_happy_path(reaction_manager, mock_message_reaction, user_id, message_id, expected):
    mock_message_reaction.query.all.return_value = [
...
def test_get_user_reactions_edge_cases(reaction_manager, mock_message_reaction, user_id, message_id, expected):
    mock_message_reaction.query.all.return_value = []
    result = reaction_manager.get_user_reactions(user_id, message_id)
    assert result == expected
