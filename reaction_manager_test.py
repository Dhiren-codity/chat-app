...
def test_get_reaction_count_happy_path_and_edge_cases(reaction_manager, message_id, mock_count, expected):
    # Mocking the MessageReaction.query.filter_by().count() method
...
@pytest.mark.parametrize("message_id", [
    (None),  # Error case: message_id is None
    ("invalid_id"),  # Error case: message_id is not an integer
])


def test_get_reaction_count_error_cases(reaction_manager, message_id):
    # Mocking the MessageReaction.query.filter_by().count() method
    MessageReaction = MagicMock()
    MessageReaction.query.filter_by.return_value.count.side_effect = ValueError("Invalid message_id")
    with pytest.raises(ValueError):
        reaction_manager.get_reaction_count(message_id)
