...
def test_reaction_manager_init(db_session, expected_db, monkeypatch):
    # Mock the default db
    default_db_mock = Mock()
    monkeypatch.setattr('your_module.db', default_db_mock)
    ...
    if db_session is None:
        assert manager.db == default_db_mock
    else:
...
def test_get_reaction_count_happy_path_and_edge_cases(reaction_manager, message_id, mock_count, expected):
...
def test_get_allowed_emojis_no_modification(reaction_manager):
    ...
    assert reaction_manager.get_allowed_emojis() == original_emojis  # Ensure original is unchanged


def test_get_allowed_emojis_type_error(reaction_manager):
    ReactionManager.ALLOWED_EMOJIS = None  # Error case: ALLOWED_EMOJIS is None
    with pytest.raises(TypeError):
