...
def test_reaction_manager_init_invalid_session():
    with pytest.raises(TypeError):
        ReactionManager(db_session="invalid_session")  # Assuming the implementation raises TypeError for invalid session types
...
def test_remove_reaction(reaction_manager, message_id, user_id, emoji, reaction_exists, expected):
    ...
    with patch('reaction_manager.MessageReaction.query.filter_by', mock_filter_by):
...
def test_get_user_reactions(reaction_manager, mock_query, user_id, message_id, expected_reactions):
