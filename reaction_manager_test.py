...
@pytest.mark.parametrize("message_id, user_id, emoji, expected", [
    (1, 1, ":smile:", {"success": True, "message": "Reaction added"}),
    (1, 1, ":thumbsup:", {"success": True, "message": "Reaction added"}),
])


def test_add_reaction_success(reaction_manager, message_id, user_id, emoji, expected):
    # Mock message existence
    Message.query.get = MagicMock(return_value=MagicMock())
...
@pytest.mark.parametrize("message_id, exception", [
    # Error case: invalid message_id type
    ('invalid_id', TypeError),
])


def test_get_message_reactions_errors(reaction_manager, mock_query, message_id, exception):
...
# Mock query filter_by method
def mock_filter_by(user_id=None, message_id=None):
    if user_id == 1:
        if message_id:
            return [r for r in mock_reactions if r.message_id == message_id]
        return mock_reactions
    return []
MessageReaction.query.filter_by = MagicMock(side_effect=mock_filter_by)
...
def test_get_user_reactions(user_id, message_id, expected):
    manager = ReactionManager()
    result = manager.get_user_reactions(user_id, message_id)
    assert result == expected
