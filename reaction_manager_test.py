...
def test_remove_reaction(reaction_manager, message_id, user_id, emoji, reaction_exists, expected):
    # Mock the query and reaction existence
...
class MockMessageReaction:
    def __init__(self, id, message_id, emoji, created_at):
        self.id = id
        self.message_id = message_id
        self.emoji = emoji
...
def test_get_user_reactions(reaction_manager, user_id, message_id, expected):
    # Mock data
    reactions = [
        MockMessageReaction(1, 101, "👍", MagicMock(isoformat=lambda: "2023-10-01T12:00:00")),
        MockMessageReaction(2, 102, "❤️", MagicMock(isoformat=lambda: "2023-10-02T12:00:00")),
    ]
    # Mock the query
...
@pytest.mark.parametrize("message_id, user_id, emoji, expected", [
    ...
    (1, 1, '🔥', False),  # Error case: emoji not in ALLOWED_EMOJIS
])
