...
def test_get_message_reactions_invalid_message_id(reaction_manager, message_id, reactions):
    ...
    assert result == []
@pytest.mark.parametrize("message_id, reactions, expected", [
    (5, [MessageReaction(5, 1, '🔥'), MessageReaction(5, 1, '🔥')],
    [{'emoji': '🔥', 'count': 2, 'users': [1, 1]}]),
])

def test_get_message_reactions_duplicate_user_reactions(reaction_manager, message_id, reactions, expected):
    MessageReaction.query.filter_by.return_value.all.return_value = reactions
    result = reaction_manager.get_message_reactions(message_id)
    assert result == expected
...
@pytest.mark.parametrize("message_id, user_id, emoji, existing, expected_action", [
    ...
    (1, 1, '👍', True, "removed"),  # Edge case: Reaction found, remove it
])
