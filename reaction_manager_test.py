...
def test_has_user_reacted(message_id, user_id, emoji, expected):
    # Setup
...
def test_get_allowed_emojis_immutable(modify_list):
    manager = ReactionManager()
    emojis = manager.get_allowed_emojis()
    emojis.append('😎')
    assert manager.get_allowed_emojis() == modify_list
