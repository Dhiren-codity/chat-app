...
def test_reaction_manager_init_edge_case_empty_session():
    manager = ReactionManager(db_session=None)
    assert manager.db is not None  # Assuming default db is set

def test_reaction_manager_init_edge_case_invalid_session():
...
@pytest.mark.parametrize("message_id, reactions, expected", [
    ...
    (1, [
        MessageReaction(1, 101, '👍'),
        MessageReaction(1, 102, '❤️'),
        MessageReaction(1, 103, '👍'),
    ], [
        {'emoji': '👍', 'count': 2, 'users': [101, 103]},
        {'emoji': '❤️', 'count': 1, 'users': [102]},
    ]),
    # Edge case: no reactions
    (2, [], []),
    # Edge case: all reactions with the same emoji
    (3, [
        MessageReaction(3, 104, '😂'),
        MessageReaction(3, 105, '😂'),
    ], [
        {'emoji': '😂', 'count': 2, 'users': [104, 105]},
    ]),
])
...
@pytest.mark.parametrize("message_id, reactions", [
    # Error case: invalid message_id (simulating database error)
    (None, None),
])
