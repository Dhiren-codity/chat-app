...
def test_add_reaction(reaction_manager, message_id, user_id, emoji, message_exists, reaction_exists, expected):
    ...
    # Mock MessageReaction.query.filter_by().first
...
def test_remove_reaction(reaction_manager, message_id, user_id, emoji, reaction_exists, expected):
    ...
    with patch('my_module.MessageReaction.query.filter_by', return_value=mock_query):
...
def test_remove_reaction_exception_handling(reaction_manager):
    ...
    with patch('my_module.MessageReaction.query.filter_by', return_value=MagicMock(first=MagicMock(return_value=mock_reaction))):
...
@pytest.mark.parametrize("message_id, reactions", [
    (4, None),  # Error case: None returned from query
])
...
def test_get_reaction_count_invalid_message_id(reaction_manager):
    ...
    MessageReaction.query.filter_by.return_value.count.side_effect = Exception("Invalid message ID")
...
@pytest.mark.parametrize("modify_emojis", [
    (['😢', '😡']),  # Edge case: modifying the returned list
])
...
@pytest.mark.parametrize("expected_type", [
    (list),  # Error case: check return type
])

def test_get_allowed_emojis_error_case_type(reaction_manager, expected_type):
