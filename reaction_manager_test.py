...
@pytest.mark.parametrize("message_id, reactions, expected", [
    ...
    (1, [
        ...
        MagicMock(emoji='😂', user_id=5)
    ], [
        {"emoji": '😀', "count": 2, "users": [1, 2]},
        {"emoji": '😂', "count": 3, "users": [3, 4, 5]}
    ]),
...
def test_toggle_reaction_invalid_emoji(reaction_manager, message_id, user_id, emoji):
    # Assuming ALLOWED_EMOJIS is a class attribute
...
def test_get_allowed_emojis_type(reaction_manager):
    result = reaction_manager.get_allowed_emojis()
    assert isinstance(result, list)  # Happy path: check type


def test_get_allowed_emojis_copy(reaction_manager):
    result = reaction_manager.get_allowed_emojis()
    result.append('😢')
    assert '😢' not in reaction_manager.get_allowed_emojis()  # Edge case: ensure copy