...
def test_add_reaction_db_failure(reaction_manager, message_id, user_id, emoji):
    # Mock message existence
    reaction_manager.db.session.query().get.return_value = MagicMock(id=message_id)
    # Mock no existing reaction
    reaction_manager.db.session.query().filter_by().first.return_value = None
    # Mock DB failure
    reaction_manager.db.session.add.side_effect = Exception("DB Error")
...
@pytest.mark.parametrize("message_id, user_id, emoji, expected", [
    ...
    (1, 1, '❤️', False), # Edge case: user reacted with different emoji
    (1, 3, None, False), # Error case: user did not react to message
    (2, 1, '😂', True),  # Edge case: user reacted to different message
])
