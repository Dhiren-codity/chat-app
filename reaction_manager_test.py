...
@pytest.mark.parametrize("message_id, user_id, emoji", [
    (1, 1, '👍'),  # Simulate database exception
])

def test_remove_reaction_db_exception(reaction_manager, message_id, user_id, emoji):
    with patch('reaction_manager.MessageReaction.query.filter_by') as mock_filter_by:
        mock_query = MagicMock()
        mock_filter_by.return_value = mock_query
...
@pytest.mark.parametrize("message_id, reactions, expected", [
    ...
    (3, [
        ...
        {'emoji': '😂', 'count': 3, 'users': [4, 4, 5]}
    ]),  # Edge case: Duplicate user reactions
])

def test_get_message_reactions(setup_reaction_manager, message_id, reactions, expected):
    # Mock the query to return the provided reactions
    MessageReaction = MagicMock()
    MessageReaction.query.filter_by.return_value.all.return_value = reactions
    result = setup_reaction_manager.get_message_reactions(message_id)
    assert result == expected

def test_get_message_reactions_invalid_message_id(setup_reaction_manager):
    # Error case: Invalid message_id
    MessageReaction = MagicMock()
    MessageReaction.query.filter_by.return_value.all.return_value = None
    result = setup_reaction_manager.get_message_reactions(999)
    assert result == []
...
def test_get_user_reactions_nonexistent_message_id(reaction_manager, mock_query, user_id, message_id):
    mock_query.all.return_value = []
    result = reaction_manager.get_user_reactions(user_id, message_id)
    assert result == []
