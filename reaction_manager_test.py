...
def test_toggle_reaction_happy_path(reaction_manager, existing_reaction, expected_action, expected_status):
    # Mock the query to return existing_reaction
...
def test_get_reaction_count_happy_and_edge_cases(reaction_manager, message_id, mock_count, expected):
...
@pytest.mark.parametrize("message_id, user_id, emoji, expected", [
    (1, 1, None, True),  # Happy path: user reacted to message
    (1, 2, None, False), # Error case: user did not react to message
...
@pytest.mark.parametrize("modify_emoji", [
    (True),  # Edge case: Check if modifying the returned list affects the original
])


def test_get_allowed_emojis_edge_case_modify(reaction_manager, modify_emoji):
