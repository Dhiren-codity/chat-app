...
def test_reaction_manager_init_error_case(mocker):
    # Simulate an error in the default db setup
...
def test_get_user_reactions_happy_path(reaction_manager, mock_query, user_id, message_id, expected):
    mock_query.filter_by.return_value.filter_by.return_value.all.return_value = [
        MagicMock(id=1, message_id=101, emoji="👍", created_at="2023-10-01T12:00:00"),
        MagicMock(id=2, message_id=102, emoji="❤️", created_at="2023-10-02T12:00:00")
    ] if user_id == 1 else []
    result = reaction_manager.get_user_reactions(user_id, message_id)
    assert result == expected

def test_get_user_reactions_no_reactions(reaction_manager, mock_query):
...
@pytest.mark.parametrize("message_id, user_id, emoji, expected", [
    ...
    (2, 1, '❤️', True),  # Edge case: user reacted with a specific emoji
