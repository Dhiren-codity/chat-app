...
def test_remove_reaction_invalid_emoji(reaction_manager, emoji):
    mock_query = MagicMock()
    mock_query.first.return_value = None
    with patch('reaction_manager.MessageReaction.query.filter_by', return_value=mock_query):
        result = reaction_manager.remove_reaction(1, 1, emoji)
        assert result == {"success": False, "message": "Reaction not found"}
...
@pytest.mark.parametrize("expected_copy", [
    (['👍', '❤️', '😂', '😮', '😢', '🎉', '🔥', '👏']),
])

def test_get_allowed_emojis_copy(reaction_manager, expected_copy):
    emojis = reaction_manager.get_allowed_emojis()
    emojis.append('🚀')
    assert reaction_manager.get_allowed_emojis() == expected_copy
