...
@pytest.mark.parametrize("message_id, user_id, emoji, exception_raised", [
    (1, 1, '👍', True),  # Exception during deletion
])

def test_remove_reaction_exception(reaction_manager, message_id, user_id, emoji, exception_raised):
    with patch('reaction_manager.MessageReaction.query.filter_by') as mock_filter_by:
        mock_query = MagicMock()
...
def test_get_user_reactions_invalid_user_id(reaction_manager):
    mock_query = MessageReaction.query()
    mock_query.filter_by.side_effect = Exception("Invalid user ID")
    with pytest.raises(Exception, match="Invalid user ID"):
        reaction_manager.get_user_reactions(-1)
...
def test_get_most_popular_emoji(reaction_manager, message_id, reactions, expected, monkeypatch):
def mock_get_message_reactions(self, message_id):
        return reactions
    monkeypatch.setattr(ReactionManager, "get_message_reactions", mock_get_message_reactions)
