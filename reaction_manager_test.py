...
def setup_reaction_manager(mock_message_reaction):
    ...
    return reaction_manager, mock_message_reaction

@pytest.mark.parametrize("message_id, reactions, expected", [
    (1, [("👍", 1), ("❤️", 2)], [{"emoji": "👍", "count": 1, "users": [1]}, {"emoji": "❤️", "count": 2, "users": [2, 3]}]),
    (2, [], []),
    (3, [("😂", 1), ("😂", 2), ("😂", 3)], [{"emoji": "😂", "count": 3, "users": [1, 2, 3]}]),
])

def test_get_message_reactions(setup_reaction_manager, message_id, reactions, expected):
    reaction_manager, mock_message_reaction = setup_reaction_manager
    reaction_manager.MessageReaction.query.filter_by.return_value.all.return_value = [
...
def test_get_message_reactions_error_case(setup_reaction_manager):
    reaction_manager, _ = setup_reaction_manager
    reaction_manager.MessageReaction.query.filter_by.return_value.all.side_effect = Exception("Database error")
