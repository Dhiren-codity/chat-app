...
class TestReactionManagerInit:

    @pytest.mark.parametrize("db_session, expected_db", [
        (None, 'default_db'),  # Happy path: No db_session provided, should use default
        (Mock(), 'mock_db'),   # Happy path: db_session provided, should use provided session
    ])

    def test_init_with_various_db_sessions(self, db_session, expected_db, monkeypatch):
        # Mock the default db
    ...
    def test_init_with_edge_case_empty_db_session(self):
        # Edge case: Pass an empty db_session
        manager = ReactionManager(db_session=None)
        assert manager.db is not None  # Should default to the default db


    def test_init_with_edge_case_mock_db_session(self):
        # Edge case: Pass a mock db_session
        mock_db_session = Mock(name='mock_db')
        manager = ReactionManager(db_session=mock_db_session)
        assert manager.db == mock_db_session

# Assuming the ReactionManager class is imported from the module where it's defined
...
# Mock classes for Message and MessageReaction
class Message:
    @staticmethod
    def query():
        return MagicMock()
...
class MockDBSession:
    ...
    def rollback(self):
        pass
# Mock ReactionManager class
class ReactionManager:
    ALLOWED_EMOJIS = {":smile:", ":thumbsup:"}
    def __init__(self):
        self.db = MagicMock()
        self.db.session = MockDBSession()
    def add_reaction(self, message_id: int, user_id: int, emoji: str):
        # Method implementation as provided
@pytest.mark.parametrize("message_id, user_id, emoji, message_exists, expected", [
    (1, 1, ":smile:", True, {"success": True, "message": "Reaction added"}),
    (1, 1, ":heart:", True, pytest.raises(ValueError, match="Emoji ':heart:' not allowed")),
    (1, 1, ":smile:", False, pytest.raises(ValueError, match="Message 1 not found")),
    (1, 1, ":smile:", True, {"success": False, "message": "Reaction already exists"})
])

def test_add_reaction(reaction_manager, message_id, user_id, emoji, message_exists, expected):
    ...
    if expected.get("success") is False:
        MessageReaction.query.filter_by().first = MagicMock(return_value=MagicMock(id=123))
    else:
...
def test_remove_reaction(reaction_manager, message_id, user_id, emoji, reaction_exists, expected):
    with patch('my_module.MessageReaction.query.filter_by') as mock_filter_by:
        ...
        mock_filter_by.return_value.first.return_value = MagicMock() if reaction_exists else None
        if isinstance(expected, dict):
...
def test_get_reaction_count_happy_and_edge_cases(reaction_manager, message_id, reaction_count):
