...
def custom_db():
    ...
    (None, "default_db_session"),  # Happy path: No db_session provided, should use default
...
def test_reaction_manager_init_invalid_db():
    with pytest.raises(Exception):  # Replace Exception with specific exception if known
        ReactionManager(db_session="invalid_db_session")
    (None),  # Edge case: None as db_session
    ("custom_db_session"),  # Edge case: Valid custom db_session
])
...
def reaction_manager():
    ...
    (2, [{'emoji': '😂', 'count': 2}, {'emoji': '😮', 'count': 2}], '😂'),  # Edge case: tie, first emoji returned
