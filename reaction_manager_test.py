...
def test_reaction_manager_init(db_session, expected_db, monkeypatch):
    # Mock the default db if not provided
...
def test_reaction_manager_init_with_invalid_db(monkeypatch):
    # Simulate an invalid db session scenario
    monkeypatch.setattr('reaction_manager.db', None)
    with pytest.raises(Exception) as excinfo:
        ReactionManager(db_session="invalid_db")
    assert "Invalid database session" in str(excinfo.value)
