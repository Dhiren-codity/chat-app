...
def test_reaction_manager_init_with_none_session(mocker):
    # Mock the default db
    mocker.patch('reaction_manager.db', "default_db")
...
def mock_query(mocker):
    return mocker.patch('reaction_manager.MessageReaction.query')
...
def test_get_user_reactions_error_case(reaction_manager, mock_query, user_id, message_id):
    with pytest.raises(TypeError):
...
@pytest.mark.parametrize("message_id, expected_count", [
    ...
    (2, 0),  # Edge case: message with no reactions
    (3, 1),  # Edge case: message with a single reaction
])
...
def test_get_allowed_emojis_content(reaction_manager, expected_content):
    assert set(reaction_manager.get_allowed_emojis()) == expected_content
