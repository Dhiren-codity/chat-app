...
def test_reaction_manager_init_with_custom_session(mock_db_session):
    reaction_manager = ReactionManager(db_session=mock_db_session)
    assert reaction_manager.db == mock_db_session

def test_reaction_manager_init_with_default_session(mocker):
    mocker.patch('reaction_manager.db', 'default_db')
    reaction_manager = ReactionManager()
    assert reaction_manager.db == 'default_db'

def test_reaction_manager_init_invalid_session():
    with pytest.raises(TypeError):
        ReactionManager(db_session=123)  # Assuming only valid session objects are allowed
...
def test_get_user_reactions_no_reactions(reaction_manager, mock_query, user_id, message_id):
    mock_query.all.return_value = []
    result = reaction_manager.get_user_reactions(user_id, message_id)
    assert result == []
...
def test_get_reaction_count_happy_path(reaction_manager, message_id, reaction_count):
    ...
    # Assert that the method returns the correct reaction count
...
def test_get_reaction_count_error_case(reaction_manager):
    ...
    with pytest.raises(Exception, match="Database error"):
        reaction_manager.get_reaction_count(999)  # Non-existent message_id
...
@pytest.mark.parametrize("message_id, user_id, emoji, expected", [
    (1, 1, None, True),  # Happy path: user has reacted without specifying emoji
    (1, 1, '👍', True),  # Happy path: user has reacted with specified emoji
    (1, 2, '❤️', False), # Error case: user has not reacted with specified emoji
    (2, 1, None, False), # Edge case: message does not exist
    (1, 1, '🔥', False), # Edge case: emoji not used by user
])
...
def test_has_user_reacted(reaction_manager, message_id, user_id, emoji, expected):
    ...
    # Patching the MessageReaction query
