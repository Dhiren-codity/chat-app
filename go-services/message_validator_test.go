...
func TestExtractURLs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			content:  "Just some random text without URLs.",
			maxURLs:  5,
			expected: []string{},
			wantErr:  false,
		},
		{
			name:     "content with URLs exactly at limit",
			content:  "http://example.com http://example.org",
			maxURLs:  2,
...
func TestCheckSpam(t *testing.T) {
	tests := []struct {
		...
		{"exact caps threshold", "THIS IS A MESSAGE", true, true, "message appears to be spam (excessive caps)"},
	}
