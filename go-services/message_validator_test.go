...
func TestExtractMentions(t *testing.T) {
	tests := []struct {
		...
		{"only special characters", "@@@@", MentionValidator{MaxMentions: 5}, []string{}, false},
		{"mentions with numbers", "Hello @user123", MentionValidator{MaxMentions: 5}, []string{"user123"}, false},
		{"mentions with underscore", "Hello @user_name", MentionValidator{MaxMentions: 5}, []string{"user_name"}, false},
	}
...
func TestAddBannedWord(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mv.AddBannedWord(tt.input) // Adding twice to test duplicates
...
func TestCountWords(t *testing.T) {
	tests := []struct {
		...
		{"valid input", "hello world", 2},
		{"empty input", "", 0},
		{"single word", "hello", 1},
		{"multiple spaces", "   hello   world  ", 2},
		{"new lines and tabs", "hello\nworld\t", 2},
