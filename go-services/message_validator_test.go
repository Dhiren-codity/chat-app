...
func TestCheckSpam(t *testing.T) {
	tests := []struct {
		...
		{"just below caps threshold", "THIS IS A MESSAGE with some caps", false, false, ""},
		{"mixed case with repetition", "Word word WORD word word word word word word word word", true, true, "message appears to be spam (excessive repetition)"},
	}
