...
func TestExtractMentions(t *testing.T) {
	tests := []struct {
		...
		{"exact max mentions", "Hello @user1 @user2", MentionValidator{MaxMentions: 2}, []string{"user1", "user2"}, false},
	}
...
func TestCheckSpam(t *testing.T) {
	tests := []struct {
		...
		{"excessive caps", "THIS IS AN EXCESSIVE CAPS MESSAGE", true, true, "message appears to be spam (excessive caps)"},
		{"mixed case", "This is a MIXED case message", false, false, ""},
		...
		{"exactly at caps threshold", "THIS IS A TEST MESSAGE", true, true, "message appears to be spam (excessive caps)"},
	}
...
func TestAddBannedWord(t *testing.T) {
	tests := []struct {
		...
		{"add another word", "example", []string{"test", "example"}},
		{"add empty string", "", []string{"test", "example", ""}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mv := &MockValidator{}
			for _, word := range tt.expected[:len(tt.expected)-1] {
				mv.AddBannedWord(word)
			}
