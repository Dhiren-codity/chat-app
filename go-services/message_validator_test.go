...
func TestValidateContent(t *testing.T) {
	...
	tests := []struct {
		...
		{"no banned words", "This is a clean message", mv, false, ""},
	}
...
func TestExtractMentions(t *testing.T) {
	tests := []struct {
		...
		{"no mentions", "Hello world", 5, []string{}, false},
		{"single mention", "Hello @user", 5, []string{"user"}, false},
		{"multiple mentions", "Hello @user1 and @user2", 5, []string{"user1", "user2"}, false},
		{"exceed max mentions", "Hello @user1 @user2 @user3", 2, nil, true},
		{"empty input", "", 5, []string{}, false},
		{"only mentions", "@user1 @user2", 5, []string{"user1", "user2"}, false},
		{"mentions with special characters", "Hello @user_1 and @user-2", 5, []string{"user_1", "user"}, false},
		{"mentions with numbers", "Hello @user123", 5, []string{"user123"}, false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mv := MentionValidator{MaxMentions: tt.maxMentions}
			result, err := ExtractMentions(tt.input, mv)
...
func TestExtractURLs(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "content with URLs and text exceeding limit",
			content:  "http://example.com https://example.org http://example.net",
			maxURLs:  2,
			expected: nil,
			wantErr:  true,
		},
...
func TestCheckSpam(t *testing.T) {
	tests := []struct {
		...
		{"mixed case", "This is a SPAM message", false, false, ""},
		...
		{"exact repetition threshold", "word word word word word word word word word word word", true, true, "message appears to be spam (excessive repetition)"},
		{"exact caps threshold", "THIS IS A TEST MESSAGE", true, true, "message appears to be spam (excessive caps)"},
	}
...
func TestSetBannedWords(t *testing.T) {
	tests := []struct {
		...
		{"valid input", []string{"bad", "word"}, []string{"bad", "word"}},
		{"nil input", nil, nil},
		{"empty input", []string{}, []string{}},
		{"single word", []string{"test"}, []string{"test"}},
		{"multiple words", []string{"one", "two", "three"}, []string{"one", "two", "three"}},
	}
