...
@pytest.mark.parametrize("init_params", [
    ...
    (None),  # Edge case: None as parameter
    ({"unexpected_param": 123}),  # Error case: unexpected parameter
])

def test_message_status_manager_init(init_params):
    try:
        manager = MessageStatusManager(**init_params)
        assert isinstance(manager, MessageStatusManager)
    except TypeError as e:
        assert "unexpected_param" in str(e)
...
def test_mark_as_delivered(setup_message, setup_db_session, message_id, user_id, expected):
    ...
    if expected:
        ...
        assert setup_message.delivered_at is not None
...
def test_mark_as_read(message_status_manager, mock_message, message_id, user_id, expected):
    ...
    datetime.utcnow = MagicMock(return_value=fixed_datetime)
    # Run the method
    ...
    if expected is not None:
        assert result['message_id'] == expected['message_id']
        assert result['status'] == expected['status']
        assert result['read_at'] == fixed_datetime.isoformat()
    else:
        assert result is None
    # Verify database commit is called only if message is marked as read
...
def setup_messages(mocker):
    ...
    mock_message1.read_at = None

    mock_message2 = mocker.Mock(spec=Message)
    mock_message2.id = 2
    mock_message2.status = 'delivered'
    mock_message2.sent_at = datetime(2023, 10, 1, 12, 10, 0)
    mock_message2.delivered_at = datetime(2023, 10, 1, 12, 15, 0)
    mock_message2.read_at = datetime(2023, 10, 1, 12, 20, 0)

    return [mock_message1, mock_message2]

@pytest.mark.parametrize("room_id, user_id, expected", [
    (1, 1, [
        {'message_id': 1, 'status': 'sent', 'sent_at': '2023-10-01T12:00:00', 'delivered_at': '2023-10-01T12:05:00', 'read_at': None},
        {'message_id': 2, 'status': 'delivered', 'sent_at': '2023-10-01T12:10:00', 'delivered_at': '2023-10-01T12:15:00', 'read_at': '2023-10-01T12:20:00'}
    ]),
