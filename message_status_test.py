...
class MockMessage:
    def __init__(self, message_id, user_id, status=None, delivered_at=None):
        self.id = message_id
...
@pytest.mark.parametrize("message_id, user_id, message, expected", [
    # Happy path: valid message and user_id
    (1, 2, MockMessage(1, 3), {'message_id': 1, 'status': 'delivered', 'delivered_at': datetime.utcnow().isoformat()}),
    # Error case: message not found
    (2, 2, None, None),
    # Edge case: user_id matches message.user_id
    (3, 3, MockMessage(3, 3), None),
    # Edge case: message_id does not match
    (4, 2, MockMessage(5, 3), None),
])

def test_mark_as_delivered(message_status_manager, message_id, user_id, message, expected):
    # Mocking the Message.query and db.session
    Message = MagicMock()
    Message.query = MockQuery(message)
    db = MagicMock()
    db.session = MockDBSession()
    # Injecting mocks into the method
...
@pytest.mark.parametrize("message_id, user_id, expected", [
    ...
    (2, 2, None),  # Error case: message belongs to the same user
...
@pytest.mark.parametrize("room_id, user_id, messages, expected", [
    ...
    (1, 1, [
        Message(1, 'sent', datetime(2023, 1, 1, 12, 0), datetime(2023, 1, 1, 12, 5), datetime(2023, 1, 1, 12, 10))
    ], [
    ...
    (1, 3, [
        Message(2, 'delivered', datetime(2023, 1, 2, 13, 0), None, None)
    ], [
