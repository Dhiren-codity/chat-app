...
@pytest.mark.parametrize("message_id, user_id, message_user_id, expected_status", [
    (1, 2, 3, 'delivered'),  # Happy path: message exists, user is not the sender
...
@pytest.mark.parametrize("message_id, message, expected", [
    ...
    (5, Message(5, 'sent', None, None, None), {
        'message_id': 5,
        'status': 'sent',
        'sent_at': None,
        'delivered_at': None,
        'read_at': None
    }),  # Edge case: all timestamps are None
])
...
@pytest.mark.parametrize("room_id, user_id, messages, expected", [
    ...
    (2, 1, [], []),
    # Edge case: messages with all timestamps None
    (3, 1, [
        Message(3, 3, 1, 'sent', None, None, None)
    ], [
...
def test_get_room_message_statuses_invalid_input(message_status_manager, room_id, user_id, messages, expected):
    Message.query.filter_by.return_value.all.return_value = messages
    result = message_status_manager.get_room_message_statuses(room_id, user_id)
    assert result == expected
