...
@pytest.mark.parametrize("input_data, expected", [
    ...
    (123, False),  # Edge case: non-string input
])
...
def test_message_status_manager_init(input_data, expected):
    try:
        if input_data is None:
            ...
            manager = MessageStatusManager(input_data)  # Assuming input_data is not used in __init__
...
@pytest.mark.parametrize("message_id, user_id, message_exists, user_matches, expected", [
    ...
    (1, 2, True, False, None),  # Edge case: message already delivered
])
...
def test_mark_as_delivered(message_status_manager, mock_message, message_id, user_id, message_exists, user_matches, expected):
    if message_exists:
        ...
        if not user_matches:
            mock_message.user_id = user_id + 1
    else:
        Message.query.get = MagicMock(return_value=None)
    if expected:
        mock_message.delivered_at = datetime.utcnow()
