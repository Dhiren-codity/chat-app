...
@pytest.mark.parametrize("message_id, user_id, message_user_id, expected_status", [
    (1, 2, 3, 'delivered'),  # Happy path: valid message and user
...
def test_get_room_message_statuses_invalid_user_id(message_status_manager):
    Message.query.filter_by.return_value.all.return_value = []
    result = message_status_manager.get_room_message_statuses(1, 999)
    assert result == []
