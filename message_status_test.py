...
def test_message_status_manager_init_error():
    # Error case: No error expected as __init__ has no parameters
    try:
        instance = MessageStatusManager()
        assert isinstance(instance, MessageStatusManager)
    except Exception as e:
        assert False, f"Unexpected exception during initialization: {e}"
...
@pytest.mark.parametrize("message_id, user_id, expected", [
    ...
    (3, 3, None),  # Edge case: message does not exist
])
...
def test_get_room_message_statuses_invalid_user(message_status_manager):
    Message.query.filter_by.return_value.all.return_value = []
    result = message_status_manager.get_room_message_statuses(1, 999)  # Error case: Invalid user
    assert result == []
