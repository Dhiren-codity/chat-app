...
@pytest.mark.parametrize("message_id, user_id, message_exists, user_matches, expected", [
    (1, 2, True, False, {'message_id': 1, 'status': 'read', 'read_at': datetime.utcnow().isoformat()}),  # Happy path
    (1, 1, True, True, None),  # Error case: user_id matches message.user_id
    (1, 2, False, False, None),  # Edge case: message does not exist
])

def test_mark_as_read(message_status_manager, mock_message, message_id, user_id, message_exists, user_matches, expected):
    # Setup
    if message_exists:
        Message.query.get = MagicMock(return_value=mock_message)
        mock_message.user_id = 1 if user_matches else 2
    else:
