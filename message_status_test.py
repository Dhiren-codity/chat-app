...
@pytest.mark.parametrize("message_id, user_id, message_user_id, expected_status", [
    (1, 2, 3, 'read'),  # Happy path: message exists, user is not the sender
...
@pytest.mark.parametrize("message_id, message, expected", [
    ...
    (3, Message(3, 'read', datetime(2023, 10, 1, 12, 0, 0), datetime(2023, 10, 1, 12, 5, 0), datetime(2023, 10, 1, 12, 10, 0)), {
        'message_id': 3,
        'status': 'read',
        'sent_at': '2023-10-01T12:00:00',
        'delivered_at': '2023-10-01T12:05:00',
    ...
    (4, None, None),  # Error case: message does not exist
])
