...
def test_mark_as_delivered(message_status_manager, message_id, user_id, expected):
    # Setup mock data
    messages = [
...
@pytest.mark.parametrize("message_id, user_id, message_user_id, expected_status", [
    (1, 2, 3, {'message_id': 1, 'status': 'read', 'read_at': datetime.utcnow().isoformat()}),  # Happy path
...
def test_mark_as_read(message_status_manager, message_id, user_id, message_user_id, expected_status):
    ...
    # Verify
    if expected_status:
        assert result['message_id'] == expected_status['message_id']
...
@pytest.mark.parametrize("room_id, user_id, messages, expected", [
    ...
    (1, 1, [
        ...
        Message(3, 1, 1, 'read', datetime(2023, 10, 1, 12, 10, 0), datetime(2023, 10, 1, 12, 15, 0), datetime(2023, 10, 1, 12, 20, 0))
    ], [
        ...
        {'message_id': 3, 'status': 'read', 'sent_at': '2023-10-01T12:10:00', 'delivered_at': '2023-10-01T12:15:00', 'read_at': '2023-10-01T12:20:00'}
    ]),
    # Edge case: no messages
    (1, 2, [], []),
    # Edge case: messages with null timestamps
    (2, 1, [
        Message(4, 2, 1, 'sent', None, None, None)
    ], [
