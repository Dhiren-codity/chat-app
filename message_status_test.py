...
@pytest.mark.parametrize("test_input, expected", [
    ...
    (None, True),  # Edge case: instance without attributes
])
...
@pytest.mark.parametrize("message, user_id, expected", [
    (Message(1, 2), 3, {'message_id': 1, 'status': 'delivered', 'delivered_at': datetime.utcnow().isoformat()}),
    (Message(1, 2), 2, None),  # Error case: user_id is the sender
...
def test_mark_as_delivered(message_status_manager, message, user_id, expected):
    ...
    result = message_status_manager.mark_as_delivered(1, user_id)
    if expected:
...
@pytest.mark.parametrize("message_id, user_id, message_user_id, expected_status", [
    (1, 2, 1, 'read'),  # Happy path: message exists and user is not the sender
...
@pytest.mark.parametrize("room_id, user_id, messages, expected", [
    (1, 1, [
        Message(1, 1, 1, 'sent', None, None, None)
    ], [
        {'message_id': 1, 'status': 'sent', 'sent_at': None, 'delivered_at': None, 'read_at': None}
    ]),
])

def test_get_room_message_statuses_null_dates(message_status_manager, room_id, user_id, messages, expected):
    Message.query.filter_by.return_value.all.return_value = messages
    result = message_status_manager.get_room_message_statuses(room_id, user_id)
    assert result == expected
