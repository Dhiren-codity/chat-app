...
@pytest.mark.parametrize("expected_typing_users", [
    ...
    ({"user1": True}, "edge case: single user typing"),
    ({"user1": True, "user2": False}, "edge case: multiple users with mixed states")
])
...
@pytest.mark.parametrize("room_id, typing_data, expected", [
    ...
    }, [{"user_id": "user1", "username": "Alice"}, {"user_id": "user2", "username": "Bob"}])  # Multiple active users
])
