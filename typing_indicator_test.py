...
def test_typing_indicator_init_edge_case_empty():
    indicator = TypingIndicator()
...
@pytest.mark.parametrize("initial_data, room_id, user_id, expected_data", [
    ...
    ({"room1": {"user1": {"username": "Alice", "started_at": None}}}, "room1", "user2", {"user1": {"username": "Alice", "started_at": None}}),
    ...
    ({"room1": {"user1": {"username": "Alice", "started_at": None}, "user2": {"username": "Bob", "started_at": None}}}, "room1", "user1", {"user2": {"username": "Bob", "started_at": None}})
])
...
@pytest.mark.parametrize("room_id, typing_data, expected", [
    ...
    ("room1", {
        "user1": {"username": "Alice", "started_at": None - timedelta(seconds=3)}
    }, [{"user_id": "user1", "username": "Alice"}]),  # User typing within 5 seconds
    ("room1", {
        "user1": {"username": "Alice", "started_at": None - timedelta(seconds=6)}
    }, []),  # User typing expired
    ("room1", {
        "user1": {"username": "Alice", "started_at": None - timedelta(seconds=3)},
        "user2": {"username": "Bob", "started_at": None - timedelta(seconds=6)}
