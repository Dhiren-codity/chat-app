...
@pytest.mark.parametrize("invalid_attribute", [
    ("non_existent_attribute",),  # Error case: Accessing a non-existent attribute
])

def test_typing_indicator_init_error_case(invalid_attribute):
    indicator = TypingIndicator()
    with pytest.raises(AttributeError):
        _ = getattr(indicator, invalid_attribute)
...
def test_user_started_typing_edge_cases(typing_indicator, room_id, user_id, username):
    ...
    if room_id and user_id and username:
        ...
        assert room_id not in typing_indicator.typing_users or user_id not in typing_indicator.typing_users[room_id]
