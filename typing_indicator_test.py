...
def test_user_started_typing_happy_path(typing_indicator, room_id, user_id, username, expected):
    typing_indicator.user_started_typing(room_id, user_id, username)
    assert typing_indicator.typing_users[room_id][user_id]['username'] == expected[user_id]['username']
...
def test_user_started_typing_new_room(typing_indicator):
    typing_indicator.user_started_typing("new_room", "new_user", "NewUser")
    assert "new_room" in typing_indicator.typing_users
    assert "new_user" in typing_indicator.typing_users["new_room"]

def test_user_started_typing_overwrite_existing_user(typing_indicator):
...
@pytest.mark.parametrize("room_id, user_data, expected", [
    # Happy path: user is typing within 5 seconds
    ...
    ("room2", [], []),
    # Edge case: multiple users, some typing, some not
...
def test_get_typing_users(typing_indicator, room_id, user_data, expected):
    ...
    for user_id, username, seconds_ago in user_data:
        typing_indicator.typing_users.setdefault(room_id, {})[user_id] = {
            'username': username,
            'started_at': current_time - timedelta(seconds=seconds_ago)
        }
    assert typing_indicator.get_typing_users(room_id) == expected
