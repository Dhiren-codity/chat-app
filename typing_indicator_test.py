...
def test_typing_indicator_init_type_error():
    with pytest.raises(TypeError):
        TypingIndicator(123)  # Assuming the constructor should not accept any arguments
...
@pytest.mark.parametrize("room_id, typing_data, expected", [
    ...
    ("room3", {
        "user1": {"username": "Alice", "started_at": datetime.now() - timedelta(seconds=6)},
        "user2": {"username": "Bob", "started_at": datetime.now() - timedelta(seconds=2)}
    }, [{"user_id": "user2", "username": "Bob"}]),  # Mixed active and expired users
])
