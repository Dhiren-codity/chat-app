...
def test_user_started_typing_happy_path(
        typing_indicator, room_id, user_id, username):
...
def test_user_started_typing_existing_user(
        typing_indicator, room_id, user_id, username):
    typing_indicator.user_started_typing(room_id, user_id, username)
...
def test_user_started_typing_edge_case_empty_room_id(
        typing_indicator, room_id, user_id, username):
...
def test_user_started_typing_edge_case_empty_user_id(
        typing_indicator, room_id, user_id, username):
...
def test_user_started_typing_edge_case_empty_username(
        typing_indicator, room_id, user_id, username):
...
@pytest.mark.parametrize("initial_data, room_id, user_id, expected_data", [
    ...
    ({"room1": {"user1": {"username": "Alice", "started_at": None}}}, "room2",
     "user1", {"room1": {"user1": {"username": "Alice", "started_at": None}}}),
    # Error case: user_id does not exist in the room
    ({"room1": {"user1": {"username": "Alice", "started_at": None}}}, "room1",
     "user2", {"room1": {"user1": {"username": "Alice", "started_at": None}}}),
    # Edge case: empty typing_users
    ({}, "room1", "user1", {}),
    # Edge case: multiple users in the room
    ({"room1": {"user1": {"username": "Alice",
                          "started_at": None},
                "user2": {"username": "Bob",
                          "started_at": None}}},
        "room1",
        "user1",
        {"room1": {"user2": {"username": "Bob",
                             "started_at": None}}}),
])
