...
@pytest.mark.parametrize("initial_state, expected", [
    ({}, True),  # Happy path: Check if typing_users is initialized as an empty dictionary
    (None, False),  # Error case: Check if typing_users is not None
    ({'user1': True}, False),  # Edge case: Check if typing_users is not pre-populated
])
