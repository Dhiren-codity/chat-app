...
describe('MessageFormatter', () => {
  ...
  describe('MessageFormatter', () => {
    ...
    describe('methodToTest', () => {
      ...
      it('should return "Just now" for a date very close to the current time', () => {
        const now = new Date();
        const justNow = new Date(now.getTime() - 1000); // 1 second ago
        const result = messageFormatter.methodToTest(justNow);
        expect(result).toBe('Just now');
      });
  
      it('should not return "Just now" for a date in the past', () => {
        const pastDate = new Date('2020-01-01T00:00:00Z');
        const result = messageFormatter.methodToTest(pastDate);
        expect(result).not.toBe('Just now');
      });
  
      it('should not return "Just now" for a future date', () => {
        const futureDate = new Date(new Date().getTime() + 10000); // 10 seconds in the future
  ...
  describe('MessageFormatter', () => {
    ...
    describe('formatMinutesAgo', () => {
      ...
      it('should return "59 minutes ago" for input 59', () => {
        const result = messageFormatter.formatMinutesAgo(59);
        expect(result).toBe('59 minutes ago');
      });
  
      it('should return "60 minutes ago" for input 60', () => {
        const result = messageFormatter.formatMinutesAgo(60);
        expect(result).toBe('60 minutes ago');
      });
  
      it('should handle large numbers correctly', () => {
        const result = messageFormatter.formatMinutesAgo(1000);
  ...
  describe('MessageFormatter', () => {
    ...
    describe('truncateMessage', () => {
      ...
      it('should handle maxLength less than 3 correctly', () => {
        const message = 'HelloWorld!';
        const result = formatter.truncateMessage(message, 2);
        expect(result).toBe('...');
      });
  ...
  describe('MessageFormatter', () => {
    ...
    describe('extractUrls', () => {
      ...
      it('should return an empty array when URLs are malformed', () => {
        const result = messageFormatter.extractUrls('Malformed URLs: htt://bad.com, https//missingcolon.com');
        expect(result).toEqual([]);
      });
    });
  });
  ...
  describe('MessageFormatter', () => {
    ...
    describe('formatMessage', () => {
      ...
      it('should handle message without sender', () => {
        const messageData = {
          content: 'Hello world!',
          timestamp: 1633036800000
        };
      ...
      it('should handle message with multiple mentions and URLs', () => {
        const messageData = {
          content: '@user1 @user2 check this: http://example.com http://another.com',
          timestamp: 1633036800000,
          sender: 'Alice'
        };
        ...
        expect(result).toEqual({
          content: expect.any(String),
          originalContent: '@user1 @user2 check this: http://example.com http://another.com',
          timestamp: expect.any(String),
          sender: 'Alice',
