...
describe('MessageFormatter', () => {
  ...
  describe('MessageFormatter', () => {
    ...
    describe('formatTimeAgo', () => {
      ...
      it('should return "24 hours ago" when diffHours is 24', () => {
        const result = messageFormatter.formatTimeAgo(24);
  ...
  describe('MessageFormatter', () => {
    ...
    describe('extractMentions', () => {
      ...
      it('should return an empty array for a string with only special characters', () => {
        const result = messageFormatter.extractMentions('@@@');
        expect(result).toEqual([]);
      });
  
      it('should return an array with mentions when message contains special characters', () => {
        const result = messageFormatter.extractMentions('Hello @user! How are you @friend?');
        expect(result).toEqual(['user', 'friend']);
      });
  
      it('should return an array with mentions when message contains numbers in mentions', () => {
        const result = messageFormatter.extractMentions('Hello @user123');
  ...
  describe('MessageFormatter', () => {
    ...
    describe('linkifyUrls', () => {
      ...
      it('should not alter text that looks like a URL but is not', () => {
        const message = 'This is not a URL: http://';
        expect(formatter.linkifyUrls(message)).toBe(message);
      });
    });
  });
  ...
  describe('MessageFormatter', () => {
    ...
    describe('method', () => {
      it('should return null if messageData is null', () => {
        const result = messageFormatter.method(null);
        expect(result).toBeNull();
      });
  
      it('should return null if messageData.content is undefined', () => {
        const result = messageFormatter.method({});
        expect(result).toBeNull();
      });
      ...
      it('should format message correctly with valid input', () => {
        ...
        const result = messageFormatter.method(messageData);
      ...
      it('should handle empty content', () => {
        ...
        jest.spyOn(messageFormatter, 'sanitizeMessage').mockReturnValue('');
        jest.spyOn(messageFormatter, 'linkifyUrls').mockReturnValue('');
        jest.spyOn(messageFormatter, 'highlightMentions').mockReturnValue('');
        jest.spyOn(messageFormatter, 'formatTimestamp').mockReturnValue('2021-10-01T00:00:00Z');
        jest.spyOn(messageFormatter, 'extractMentions').mockReturnValue([]);
        jest.spyOn(messageFormatter, 'extractUrls').mockReturnValue([]);
  ...
  describe('MessageFormatter', () => {
    ...
    describe('countWords', () => {
      ...
      it('should handle a mix of spaces, tabs, and newlines', () => {
        expect(formatter.countWords('hello \t\n world')).toBe(2);
      });
    });
  });
