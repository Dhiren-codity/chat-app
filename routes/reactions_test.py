...
def test_toggle_reaction_success(mock_reaction_manager, client):
...
def test_toggle_reaction_missing_data(mock_reaction_manager, client):
    response = client.post('/api/reactions/toggle', headers={'X-User-ID': '123'}, json={})
    assert response.status_code == 400
    assert response.get_json() == {"error": "message_id and emoji are required"}
@patch('routes.reactions.reaction_manager')

def test_toggle_reaction_value_error(mock_reaction_manager, client):
...
def test_toggle_reaction_unexpected_error(mock_reaction_manager, client):
    ...
    ('123', 1, '👍', 200, {'success': True}),
    ('123', None, '👍', 400, {"error": "message_id and emoji are required"}),
    ('123', 1, None, 400, {"error": "message_id and emoji are required"}),
])
@patch('routes.reactions.reaction_manager')

def test_toggle_reaction_various_inputs(mock_reaction_manager, client, user_id, message_id, emoji, expected_status, expected_response):
    if expected_status == 200:
        mock_reaction_manager.toggle_reaction.return_value = {'success': True}
    response = client.post('/api/reactions/toggle', headers={'X-User-ID': user_id}, json={'message_id': message_id, 'emoji': emoji})
...
class TestBulkAddReactions:
    ...
    def test_bulk_add_reactions_success(self, mock_reaction_manager, client):
        ...
        response = client.post(
            '/api/reactions/bulk',
            headers={'X-User-ID': '123'},
            json={"reactions": [{"message_id": 1, "user_id": 123, "emoji": "👍"}]}
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data['success'] is True
