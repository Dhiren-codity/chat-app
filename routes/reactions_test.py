...
class TestGetMessageReactions:
    ...
    def test_get_message_reactions_complex_reaction_data(self, mock_manager):
        ...
        complex_reactions = {
            ...
            "❤️": [
                {"user_id": 3, "username": "user3", "timestamp": "2023-01-01T10:02:00Z", "extra_data": {"role": "admin"}}
            ],
            "🎉": [],
            "custom_emoji": [{"user_id": 4, "username": "user4"}]
        }
    ...
    def test_get_message_reactions_large_dataset(self, mock_manager):
        ...
        for i in range(50):
            emoji = f"emoji_{i}"
            users = [{"user_id": j, "username": f"user_{j}"} for j in range(100)]
            large_reactions[emoji] = users
        ...
        response_data = json.loads(response.data)
        assert response_data["success"] is True
        assert len(response_data["reactions"]) == 50
        assert len(response_data["reactions"]["emoji_0"]) == 100
        mock_manager.get_message_reactions.assert_called_once_with(message_id)
