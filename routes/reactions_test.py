...
class TestRequireAuth:
    ...
    def test_user_id_from_query_param(self, client):
        response = client.get('/test?user_id=5')
        assert response.status_code == 200
        assert response.get_json() == {"user_id": 5}


    def test_user_id_from_query_param_empty(self, client):
        response = client.get('/test?user_id=')
        assert response.status_code == 401
        assert response.get_json() == {"error": "Authentication required"}


    def test_user_id_from_query_param_invalid(self, client):
...
class TestDecoratedFunction:
    ...
    def test_decorated_function(self, mock_request, user_id, expected_status, expected_response, client):
        ...
        if expected_status == 200:
            assert response[0].json == {"result": "success"}
        else:
