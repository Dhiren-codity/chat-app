...
class TestToggleReaction:
    ...
    def test_toggle_reaction_success(self, mock_reaction_manager, client):
        ...
        assert response.status_code == 200
        assert response.get_json() == {'success': True}
        mock_reaction_manager.toggle_reaction.assert_called_once_with(message_id=1, user_id='123', emoji='😀')
    ...
    def test_toggle_reaction_value_error(self, mock_reaction_manager, client):
        ...
        assert response.status_code == 400
        assert response.get_json() == {"error": "Invalid emoji"}
        mock_reaction_manager.toggle_reaction.assert_called_once_with(message_id=1, user_id='123', emoji='invalid_emoji')
...
class TestGetMessageReactions:
    ...
    def test_get_message_reactions_edge_cases(self, mock_get_reactions, client, message_id, expected_status):
        mock_get_reactions.return_value = []
...
class TestBulkAddReactions:
    ...
    @pytest.mark.parametrize("reactions, expected_status, expected_error", [
        ...
        ([{'message_id': 1, 'user_id': 123}], 400, "reactions array is required"),
        ([{'message_id': 1, 'user_id': 123, 'emoji': ''}], 400, "reactions array is required"),
    ])
