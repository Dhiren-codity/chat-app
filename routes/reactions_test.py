...
class TestGetMessageReactions:
    ...
    def test_get_message_reactions_various_ids(self, mock_get_reactions, client, message_id, expected_status):
        # Setup mock
        if message_id == -1:
            mock_get_reactions.side_effect = Exception("Invalid ID")
        else:
...
class TestGetUserReactions:
    ...
    @pytest.mark.parametrize("user_id,expected_status", [
        ('', 401),  # No user ID
        (None, 401),  # None user ID
    ])

    def test_get_user_reactions_missing_user_id(self, client, user_id, expected_status):
        # Make request without user ID
        response = client.get('/api/reactions/user', headers={'X-User-ID': user_id})
