...
class TestToggleReaction:
    ...
    @pytest.mark.parametrize("data,expected_error", [
        ...
        ({'other_field': 'value'}, "message_id and emoji are required"),
    ])
    @patch('routes.reactions.request')
    @patch('routes.reactions.jsonify')

    def test_toggle_reaction_invalid_data_parametrized(self, mock_jsonify, mock_request, data, expected_error):
        # Arrange
        mock_request.get_json.return_value = data
        mock_jsonify.return_value = Mock()
    ...
    def test_toggle_reaction_various_inputs(self, mock_jsonify, mock_manager, mock_request, user_id, message_id, emoji):
        # Arrange
        mock_request.get_json.return_value = {
            'message_id': message_id,
            'emoji': emoji
        }
        mock_manager.toggle_reaction.return_value = {'status': 'toggled'}
        mock_jsonify.return_value = Mock()
        
        # Act
        result = toggle_reaction(user_id)
    ...
    def test_toggle_reaction_manager_returns_none(self, mock_jsonify, mock_manager, mock_request):
        # Arrange
        mock_request.get_json.return_value = {
            'message_id': 123,
            'emoji': '👍'
        }
        mock_manager.toggle_reaction.return_value = None
        mock_jsonify.return_value = Mock()
    ...
    def test_toggle_reaction_manager_returns_empty_dict(self, mock_jsonify, mock_manager, mock_request):
        # Arrange
        mock_request.get_json.return_value = {
            'message_id': 123,
            'emoji': '👍'
        }
        mock_manager.toggle_reaction.return_value = {}
        mock_jsonify.return_value = Mock()
...
class TestBulkAddReactions:
    ...
    def test_bulk_add_reactions_manager_value_error(self, mock_manager, mock_request):
        """Test when reaction_manager raises a ValueError"""
        # Setup
        mock_request.get_json.return_value = {
    ...
    def test_bulk_add_reactions_manager_key_error(self, mock_manager, mock_request):
        """Test when reaction_manager raises a KeyError"""
        # Setup
        mock_request.get_json.return_value = {
    ...
    def test_bulk_add_reactions_large_batch(self, mock_manager, mock_request):
        """Test with large batch of reactions"""
        # Setup
        reactions = [
            {'message_id': i, 'user_id': 1, 'emoji': '👍'} 
            for i in range(100)
        ]
        mock_request.get_json.return_value = {'reactions': reactions}
        mock_manager.bulk_add_reactions.return_value = {'added': 100, 'failed': 0}
        ...
        response_data = json.loads(response.data)
        assert response_data['success'] is True
        assert response_data['result'] == {'added': 100, 'failed': 0}
        mock_manager.bulk_add_reactions.assert_called_once_with(reactions)

    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_bulk_add_reactions_partial_success(self, mock_manager, mock_request):
        """Test when some reactions succeed and some fail"""
        # Setup
        mock_request.get_json.return_value = {
            'reactions': [
                {'message_id': 1, 'user_id': 1, 'emoji': '👍'},
        ...
        mock_manager.bulk_add_reactions.return_value = {'added': 2, 'failed': 1}
        ...
        response_data = json.loads(response.data)
        assert response_data['success'] is True
        assert response_data['result'] == {'added': 2, 'failed': 1}

    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_bulk_add_reactions_complex_result(self, mock_manager, mock_request):
        """Test with complex result structure from manager"""
        # Setup
        mock_request.get_json.return_value = {
            'reactions': [{'message_id': 1, 'user_id': 1, 'emoji': '👍'}]
        }
        ...
        mock_manager.bulk_add_reactions.return_value = complex_result
        ...
        response_data = json.loads(response.data)
        assert response_data['success'] is True
        assert response_data['result'] == complex_result
