...
class TestAddReaction:
    ...
    def test_add_reaction_with_special_characters_emoji(self, mock_manager, mock_request, success_response):
        """Test adding reaction with special character emojis."""
        request_data = {"message_id": 123, "emoji": "🤷‍♀️"}
        mock_request.get_json.return_value = request_data
        mock_manager.add_reaction.return_value = success_response
    ...
    def test_add_reaction_with_text_emoji(self, mock_manager, mock_request, success_response):
        """Test adding reaction with text-based emoji."""
        request_data = {"message_id": 123, "emoji": ":thumbsup:"}
        mock_request.get_json.return_value = request_data
        mock_manager.add_reaction.return_value = success_response
    ...
    def test_add_reaction_with_extra_fields(self, mock_manager, mock_request, success_response):
        """Test adding reaction with extra fields in request data."""
        request_data = {
            "message_id": 123,
            "emoji": "👍",
            "extra_field": "ignored",
...
class TestBulkAddReactions:
    ...
    def test_user_id_parameter_not_used_in_logic(self, mock_manager, mock_request, valid_reactions_data):
        """Test that user_id parameter doesn't affect the function logic."""
        mock_request.get_json.return_value = valid_reactions_data
        expected_result = {"added": 3, "failed": 0}
        mock_manager.bulk_add_reactions.return_value = expected_result
        ...
        assert json.loads(response1.data) == json.loads(response2.data) == json.loads(response3.data)
    ...
    def test_unicode_emoji_handling(self, mock_manager, mock_request):
        ...
        expected_result = {"added": 4, "failed": 0}
        ...
        response_data = json.loads(response.data)
        assert response_data["success"] is True
        assert response_data["result"]["added"] == 4

    @patch('routes.reactions.request')

    def test_request_get_json_called_correctly(self, mock_request):
        """Test that request.get_json() is called properly."""
        mock_request.get_json.return_value = None
        
        bulk_add_reactions(123)
        
        mock_request.get_json.assert_called_once_with()
