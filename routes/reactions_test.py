...
class TestAddReaction:
    ...
    def test_add_reaction_success(self, mock_manager, mock_request, valid_request_data, success_response):
        ...
        result, status_code = add_reaction(user_id=789)
    ...
    def test_add_reaction_boundary_values(self, mock_manager, mock_request, user_id, message_id, emoji, success_response):
        """Test boundary values for user_id and message_id."""
        request_data = {"message_id": message_id, "emoji": emoji}
        mock_request.get_json.return_value = request_data
        mock_manager.add_reaction.return_value = success_response
        
        result, status_code = add_reaction(user_id=user_id)
    ...
    def test_add_reaction_various_emojis(self, mock_manager, mock_request, emoji, success_response):
    ...
    @pytest.mark.parametrize("exception_type,error_message,expected_status", [
        (ValueError, "Invalid message ID", 400),
        (ValueError, "Invalid emoji format", 400),
        (ValueError, "User not authorized", 400),
    ])
    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_add_reaction_value_error_handling(self, mock_manager, mock_request, exception_type, error_message, expected_status, valid_request_data):
        """Test ValueError exception handling."""
        mock_request.get_json.return_value = valid_request_data
        mock_manager.add_reaction.side_effect = exception_type(error_message)
    ...
    def test_add_reaction_general_exception_handling(self, mock_manager, mock_request, exception_type, error_message, valid_request_data):
    ...
    def test_add_reaction_invalid_user_id_types(self, mock_manager, mock_request, user_id, valid_request_data, success_response):
        """Test various invalid user_id types."""
        mock_request.get_json.return_value = valid_request_data
        mock_manager.add_reaction.return_value = success_response
        
        result, status_code = add_reaction(user_id=user_id)
        
        # Function should still call reaction_manager with whatever user_id is passed
        assert status_code == 200
        mock_manager.add_reaction.assert_called_once_with(
            message_id=123,
            user_id=user_id,
    ...
    def test_add_reaction_invalid_data_types(self, mock_manager, mock_request, message_id, emoji, success_response):
        ...
        # Function passes data as-is to reaction_manager
        assert status_code == 200
        mock_manager.add_reaction.assert_called_once_with(
            message_id=message_id,
            user_id=789,
    ...
    def test_add_reaction_extra_fields_ignored(self, mock_manager, mock_request, success_response):
    ...
    def test_add_reaction_request_get_json_exception(self, mock_request):
        """Test exception when request.get_json() fails."""
        mock_request.get_json.side_effect = Exception("JSON parsing failed")
        
        result, status_code = add_reaction(user_id=789)
        
        assert status_code == 500
        response_data = json.loads(result.data)
        assert "Failed to add reaction: JSON parsing failed" in response_data["error"]
...
class TestToggleReaction:
    ...
    def test_toggle_reaction_success(self, mock_manager, mock_request, valid_request_data, mock_reaction_result):
        ...
        assert response.get_json() == mock_reaction_result
    ...
    def test_toggle_reaction_missing_required_fields(self, mock_request, request_data, expected_error):
        """Test reaction toggle with missing required fields."""
        # Arrange
        user_id = 456
        mock_request.get_json.return_value = request_data
    ...
    def test_toggle_reaction_boundary_values(self, mock_manager, mock_request, message_id, emoji, user_id):
        """Test reaction toggle with boundary values."""
        # Arrange
        request_data = {"message_id": message_id, "emoji": emoji}
        mock_request.get_json.return_value = request_data
    ...
    def test_toggle_reaction_value_error(self, mock_manager, mock_request, valid_request_data, error_message):
        """Test reaction toggle with ValueError from reaction manager."""
        # Arrange
        user_id = 456
        mock_request.get_json.return_value = valid_request_data
    ...
    def test_toggle_reaction_general_exceptions(self, mock_manager, mock_request, valid_request_data, exception_type, exception_message):
        """Test reaction toggle with general exceptions."""
        # Arrange
        user_id = 456
        mock_request.get_json.return_value = valid_request_data
        mock_manager.toggle_reaction.side_effect = exception_type(exception_message)
        ...
        # Assert
        assert status_code == 500
        expected_error = f"Failed to toggle reaction: {exception_message}"
        assert response.get_json()["error"] == expected_error
    ...
    def test_toggle_reaction_various_user_ids(self, mock_manager, mock_request, valid_request_data, user_id):
        """Test reaction toggle with various user IDs."""
        # Arrange
        mock_request.get_json.return_value = valid_request_data
    ...
    def test_toggle_reaction_unicode_emoji(self, mock_manager, mock_request):
        """Test reaction toggle with various Unicode emojis."""
        # Arrange
        user_id = 456
        unicode_emojis = ["🚀", "🌟", "💯", "🔥", "👨‍💻", "🏳️‍🌈"]
        ...
        for emoji in unicode_emojis:
            request_data = {"message_id": 123, "emoji": emoji}
            mock_request.get_json.return_value = request_data
            mock_result = {"message_id": 123, "user_id": user_id, "emoji": emoji, "action": "added"}
            mock_manager.toggle_reaction.return_value = mock_result
    ...
    def test_toggle_reaction_manager_called_with_correct_params(self, mock_manager, mock_request, valid_request_data):
        """Test that reaction manager is called with correct parameters."""
        # Arrange
        user_id = 789
        mock_request.get_json.return_value = valid_request_data
...
class TestGetMessageReactions:
    ...
    def test_get_message_reactions_exception_with_special_characters(self, mock_manager):
        ...
        error_message = "Error with special chars: àáâãäå ñ 中文 🚨"
    ...
    def test_get_message_reactions_large_dataset(self, mock_manager):
        """Test function with large reaction dataset."""
        # Arrange
        message_id = "msg_large"
        large_reactions = {}
        
        # Create large dataset with multiple emojis and users
        emojis = ["👍", "👎", "❤️", "😂", "😮", "😢", "😡"]
        for emoji in emojis:
            large_reactions[emoji] = [
                {"user_id": i, "username": f"user{i}"} 
                for i in range(1, 101)  # 100 users per emoji
            ]
        ...
        assert response_data["success"] is True
        assert response_data["message_id"] == message_id
        assert len(response_data["reactions"]) == 7
        for emoji in emojis:
            assert len(response_data["reactions"][emoji]) == 100
...
class TestGetUserReactions:
    ...
    def test_get_user_reactions_success_with_message_filter(self, mock_manager, mock_request):
        """Test successful retrieval of user reactions filtered by message."""
        # Arrange
        user_id = 123
        message_id = 100
        filtered_reactions = [
    ...
    def test_get_user_reactions_empty_results(self, mock_manager, mock_request):
        """Test successful retrieval with empty reaction list."""
        # Arrange
        user_id = 123
        empty_reactions = []
        mock_request.args.get.return_value = None
        mock_manager.get_user_reactions.return_value = empty_reactions
        
        # Act
        with patch('routes.reactions.jsonify') as mock_jsonify:
            mock_jsonify.return_value = ({"success": True, "user_id": user_id, "reactions": empty_reactions}, 200)
            response, status_code = get_user_reactions(user_id)
    ...
    def test_get_user_reactions_boundary_values(self, mock_manager, mock_request, user_id, message_id, sample_reactions):
    ...
    def test_get_user_reactions_reaction_manager_exception(self, mock_manager, mock_request):
    ...
    def test_get_user_reactions_generic_exception(self, mock_manager, mock_request):
        """Test exception handling with generic exception."""
        # Arrange
        user_id = 123
        mock_request.args.get.return_value = None
        mock_manager.get_user_reactions.side_effect = RuntimeError("Unexpected error")
        
        # Act
        with patch('routes.reactions.jsonify') as mock_jsonify:
            mock_jsonify.return_value = ({"error": "Failed to get user reactions: Unexpected error"}, 500)
            response, status_code = get_user_reactions(user_id)
        
        # Assert
        mock_jsonify.assert_called_once_with({
            "error": "Failed to get user reactions: Unexpected error"
        })
        assert status_code == 500
    ...
    def test_get_user_reactions_request_args_parsing(self, mock_manager, mock_request, sample_reactions):
        """Test that request args are parsed correctly with type conversion."""
        # Arrange
        user_id = 123
        message_id = 100
        mock_request.args.get.return_value = message_id
    ...
    def test_get_user_reactions_exception_during_request_parsing(self, mock_manager, mock_request):
        """Test exception handling when request parsing fails."""
        # Arrange
        user_id = 123
        mock_request.args.get.side_effect = Exception("Request parsing failed")
        
        # Act
        with patch('routes.reactions.jsonify') as mock_jsonify:
            mock_jsonify.return_value = ({"error": "Failed to get user reactions: Request parsing failed"}, 500)
            response, status_code = get_user_reactions(user_id)
        
        # Assert
        mock_jsonify.assert_called_once_with({
            "error": "Failed to get user reactions: Request parsing failed"
        })
        assert status_code == 500
    ...
    def test_get_user_reactions_none_user_id(self, mock_manager, mock_request, sample_reactions):
...
class TestGetReactionCount:
    ...
    def test_reaction_manager_returns_negative_count(self, mock_jsonify, mock_manager):
        """Test handling when reaction manager returns negative count."""
        # Arrange
    ...
    def test_reaction_manager_returns_zero_count(self, mock_jsonify, mock_manager):
        """Test handling when reaction manager returns zero count."""
        # Arrange
        message_id = "msg_zero"
        zero_count = 0
        mock_manager.get_reaction_count.return_value = zero_count
        mock_jsonify.return_value = Mock()
    ...
    def test_jsonify_exception_handling(self, mock_jsonify, mock_manager):
        """Test exception handling when jsonify itself raises an exception."""
        # Arrange
        message_id = "msg_jsonify_error"
        mock_manager.get_reaction_count.return_value = 5
        ...
        # Assert
        mock_manager.get_reaction_count.assert_called_once_with(message_id)
        # jsonify will be called twice - once for success, once for error
        assert mock_jsonify.call_count == 2
        assert status_code == 500

    @patch('routes.reactions.reaction_manager')
    @patch('routes.reactions.jsonify')

    def test_large_message_id_string(self, mock_jsonify, mock_manager):
        """Test handling of very large message ID strings."""
        # Arrange
        large_message_id = "msg_" + "x" * 10000
        expected_count = 42
        mock_manager.get_reaction_count.return_value = expected_count
        mock_jsonify.return_value = Mock()
        
        # Act
        result, status_code = get_reaction_count(large_message_id)
        
        # Assert
        mock_manager.get_reaction_count.assert_called_once_with(large_message_id)
        mock_jsonify.assert_called_once_with({
            "success": True,
    ...
    def test_unicode_message_id(self, mock_jsonify, mock_manager):
        """Test handling of Unicode characters in message ID."""
        # Arrange
        unicode_message_id = "msg_🚀_测试_🎉"
        expected_count = 7
        mock_manager.get_reaction_count.return_value = expected_count
        mock_jsonify.return_value = Mock()
        
        # Act
        result, status_code = get_reaction_count(unicode_message_id)
        
        # Assert
        mock_manager.get_reaction_count.assert_called_once_with(unicode_message_id)
        mock_jsonify.assert_called_once_with({
            "success": True,
            "message_id": unicode_message_id,
            "count": expected_count
        })
        assert status_code == 200
...
class TestGetMostPopular:
    ...
    def test_exception_handling(self, mock_jsonify, mock_manager, exception_type, exception_message):
        ...
        # Arrange
        message_id = "msg_error"
        mock_manager.get_most_popular_emoji.side_effect = exception_type(exception_message)
        mock_jsonify.return_value = Mock()
    ...
    def test_exception_with_empty_message(self, mock_jsonify, mock_manager):
        ...
        mock_manager.get_most_popular_emoji.side_effect = Exception("")
        mock_jsonify.return_value = Mock()
    ...
    def test_unicode_emoji_handling(self, mock_jsonify, mock_manager):
        ...
        unicode_emoji = "🌟✨🎊🎈🎁"
        mock_manager.get_most_popular_emoji.return_value = unicode_emoji
        mock_jsonify.return_value = Mock()
        ...
        mock_jsonify.assert_called_once_with({
            "success": True,
            "message_id": message_id,
            "most_popular_emoji": unicode_emoji
        })
        assert status_code == 200
    ...
    def test_empty_string_emoji(self, mock_jsonify, mock_manager):
    ...
    def test_reaction_manager_called_with_correct_parameter(self, mock_jsonify, mock_manager):
        """Test that reaction manager is called with the exact parameter passed."""
        # Arrange
        message_id = "specific_test_id_789"
        mock_manager.get_most_popular_emoji.return_value = "🎯"
        mock_jsonify.return_value = Mock()
        ...
        assert mock_manager.get_most_popular_emoji.call_count == 1

    @patch('routes.reactions.reaction_manager')
    @patch('routes.reactions.jsonify')

    def test_jsonify_response_structure(self, mock_jsonify, mock_manager):
        """Test that jsonify is called with correct response structure."""
        # Arrange
        message_id = "structure_test"
        emoji = "🏗️"
...
class TestGetAllowedEmojis:
    ...
    def test_get_allowed_emojis_unicode_emojis(self, mock_jsonify, mock_reaction_manager_class):
        ...
        unicode_emojis = [
            ...
            '👨‍👩‍👧‍👦',  # Family with ZWJ sequence
            '🤷‍♀️',  # Person shrugging with gender modifier
            '🎉',  # Party popper
...
class TestBulkAddReactions:
    ...
    def test_invalid_request_data(self, mock_request, request_data, expected_error):
        """Test various invalid request data scenarios."""
        mock_request.get_json.return_value = request_data
    ...
    def test_reaction_manager_exception(self, mock_manager, mock_request, valid_reactions_data):
        """Test handling of reaction manager exceptions."""
        mock_request.get_json.return_value = valid_reactions_data
        mock_manager.bulk_add_reactions.side_effect = Exception("Database connection failed")
        
        response, status_code = bulk_add_reactions(123)
        
        assert status_code == 500
        response_data = json.loads(response.data)
        assert response_data["error"] == "Failed to bulk add reactions: Database connection failed"

    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_reaction_manager_value_error(self, mock_manager, mock_request, valid_reactions_data):
        """Test handling of specific ValueError from reaction manager."""
        mock_request.get_json.return_value = valid_reactions_data
        mock_manager.bulk_add_reactions.side_effect = ValueError("Invalid emoji format")
        
        response, status_code = bulk_add_reactions(123)
        
        assert status_code == 500
        response_data = json.loads(response.data)
        assert response_data["error"] == "Failed to bulk add reactions: Invalid emoji format"

    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_single_reaction_bulk_add(self, mock_manager, mock_request):
    ...
    def test_large_batch_reactions(self, mock_manager, mock_request):
        """Test bulk add with large batch of reactions."""
        large_batch_data = {
            "reactions": [
                {"message_id": i, "user_id": 123, "emoji": "👍"}
                for i in range(1, 101)  # 100 reactions
            ]
        }
        expected_result = {
            "added": 100,
            "failed": 0,
            "details": [{"message_id": i, "status": "success"} for i in range(1, 101)]
        }
        
        mock_request.get_json.return_value = large_batch_data
        mock_manager.bulk_add_reactions.return_value = expected_result
    ...
    def test_request_get_json_exception(self, mock_request):
        """Test handling of request.get_json() exception."""
        ...
        assert status_code == 500
        response_data = json.loads(response.data)
        assert "Failed to bulk add reactions: Invalid JSON" in response_data["error"]

    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_user_id_parameter_ignored(self, mock_manager, mock_request, valid_reactions_data, bulk_add_result):
        """Test that user_id parameter is ignored (reactions contain their own user_ids)."""
        mock_request.get_json.return_value = valid_reactions_data
        mock_manager.bulk_add_reactions.return_value = bulk_add_result
        
        # Call with different user_id than in reactions data
        response, status_code = bulk_add_reactions(999)
        
        assert status_code == 200
        # Verify that the reactions data passed to manager contains original user_ids
        mock_manager.bulk_add_reactions.assert_called_once_with(valid_reactions_data["reactions"])
        
        # Verify all reactions still have user_id 123, not 999
        called_reactions = mock_manager.bulk_add_reactions.call_args[0][0]
        for reaction in called_reactions:
...
class TestDecoratedFunction:
    ...
    def test_function_args_and_kwargs_preservation(self, mock_jsonify, mock_request, mock_function):
        """Test that original function args and kwargs are preserved."""
        # Arrange
    ...
    def test_function_return_value_passthrough(self, mock_jsonify, mock_request, mock_function):
        """Test that decorated function return value is passed through unchanged."""
        # Arrange
    ...
    def test_user_id_conversion_error_handling(self, mock_jsonify, mock_request, mock_function):
        """Test handling of invalid user_id values that cannot be converted to int."""
        # Arrange
        mock_request.headers.get.return_value = "invalid_number"
        mock_request.args.get.return_value = None
    ...
    def test_no_args_no_kwargs(self, mock_jsonify, mock_request, mock_function):
        """Test decorated function with no additional arguments."""
        # Arrange
    ...
    def test_user_id_in_kwargs_override(self, mock_jsonify, mock_request, mock_function):
        """Test that user_id from authentication overrides any existing user_id in kwargs."""
        # Arrange
        mock_request.headers.get.return_value = "300"
        mock_request.args.get.return_value = None
        
        # Act
        result = decorated_function(mock_function, user_id=999, other_param="test")
        
        # Assert
        mock_function.assert_called_once_with(user_id=300, other_param="test")
        assert result == "success"
