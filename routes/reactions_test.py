...
class TestAddReaction:
    ...
    def test_exception_handling(self, mock_reaction_manager, client):
        ...
        assert json.loads(response.data) == {"error": "Failed to add reaction: Some unexpected error"}
...
class TestRemoveReaction:
    ...
    def test_remove_reaction(self, mock_reaction_manager, client, input_data, expected_status, expected_response):
        if expected_status == 200:
            ...
            mock_reaction_manager.remove_reaction.side_effect = Exception("Some error occurred")
    ...
    def test_exception_handling(self, mock_reaction_manager, client):
        ...
        response = client.post('/api/reactions/remove', data=json.dumps({"message_id": 1, "emoji": "👍"}), content_type='application/json')
...
class TestDecoratedFunction:
    ...
    def test_decorated_function(self, mock_request, user_id, expected_status, expected_response, client):
        ...
        if expected_status == 200:
            response = decorated_function(mock_f)
    ...
    def test_exception_handling(self, client):
        @patch('routes.reactions.request')
        def mock_request_for_exception(mock_request):
            mock_request.headers = {'X-User-ID': '1'}
            mock_f = Mock(side_effect=Exception("Some error"))
            with pytest.raises(Exception):
                decorated_function(mock_f)

        mock_request_for_exception()