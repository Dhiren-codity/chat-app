...
class TestRequireAuth:
    ...
    def test_function_with_no_args_or_kwargs(self, mock_request, mock_function):
        ...
        # Assert
        mock_function.assert_called_once_with(user_id=123)
        assert result == {'success': True}
    ...
    def test_function_with_mixed_args_and_kwargs(self, mock_request, mock_function):
        ...
        # Assert
        mock_function.assert_called_once_with('pos1', 'pos2', user_id=123, kw1='val1', kw2='val2')
        assert result == {'success': True}

    @patch('routes.reactions.request')

    def test_user_id_override_in_kwargs(self, mock_request, mock_function):
        """Test that decorator adds user_id even if it exists in kwargs."""
        # Setup
        mock_request.headers.get.return_value = '123'
        mock_request.args.get.return_value = None
        ...
        # Execute - pass user_id in kwargs (should be overridden)
        result = decorated_func(user_id=999, other_param='value')
        
        # Assert - decorator's user_id should override the passed one
        mock_function.assert_called_once_with(user_id=123, other_param='value')
        assert result == {'success': True}

    @patch('routes.reactions.request')

    def test_invalid_user_id_raises_value_error(self, mock_request, mock_function):
        """Test that invalid user ID string raises ValueError."""
        # Setup
...
class TestAddReaction:
    ...
    @pytest.mark.parametrize("exception_type,error_message", [
        ...
        (ValueError, "Invalid emoji format"),
        (ValueError, "User not authorized"),
    ])
    @patch('routes.reactions.request')
    @patch('routes.reactions.reaction_manager')

    def test_add_reaction_value_error_handling(self, mock_manager, mock_request, exception_type, error_message, valid_request_data):
    ...
    def test_add_reaction_with_none_user_id(self, mock_manager, mock_request, valid_request_data, success_response):
        """Test reaction addition with None user_id."""
        mock_request.get_json.return_value = valid_request_data
        mock_manager.add_reaction.return_value = success_response
        
        result, status_code = add_reaction(user_id=None)
    ...
    def test_add_reaction_manager_called_with_correct_params(self, mock_manager, mock_request, success_response):
        """Test that reaction manager is called with correct parameters."""
        request_data = {"message_id": 456, "emoji": "🎉"}
        mock_request.get_json.return_value = request_data
        mock_manager.add_reaction.return_value = success_response
        
        add_reaction(user_id=123)
    ...
    def test_add_reaction_request_get_json_called(self, mock_request, valid_request_data):
        """Test that request.get_json() is called."""
        mock_request.get_json.return_value = valid_request_data
        
        with patch('routes.reactions.reaction_manager') as mock_manager:
            mock_manager.add_reaction.return_value = {"success": True}
            add_reaction(user_id=789)
...
class TestGetReactionCount:
    ...
    def test_reaction_manager_returns_none(self, mock_jsonify, mock_manager):
        """Test handling when reaction manager returns None."""
        # Arrange
        message_id = "msg_none_count"
        mock_manager.get_reaction_count.return_value = None
        mock_jsonify.return_value = Mock()
    ...
    def test_empty_string_exception_message(self, mock_jsonify, mock_manager):
    ...
    def test_very_long_message_id(self, mock_jsonify, mock_manager):
        """Test handling of very long message ID."""
        # Arrange
        message_id = "msg_" + "x" * 1000  # Very long message ID
        expected_count = 7
        mock_manager.get_reaction_count.return_value = expected_count
        mock_jsonify.return_value = Mock()
...
class TestGetMostPopular:
    ...
    def test_jsonify_exception_handling(self, mock_jsonify, mock_manager):
        """Test when jsonify itself raises an exception."""
        # Arrange
        message_id = "msg_jsonify_error"
        expected_emoji = "👍"
        mock_manager.get_most_popular_emoji.return_value = expected_emoji
    ...
    def test_none_message_id(self, mock_jsonify, mock_manager):
        """Test function behavior with None message_id."""
        # Arrange
        message_id = None
        expected_emoji = "🤔"
        mock_manager.get_most_popular_emoji.return_value = expected_emoji
        ...
        # Assert
        mock_manager.get_most_popular_emoji.assert_called_once_with(None)
        mock_jsonify.assert_called_once_with({
            "success": True,
            "message_id": None,
            "most_popular_emoji": expected_emoji
...
class TestGetAllowedEmojis:
    ...
    def test_get_allowed_emojis_unicode_emojis(self, mock_jsonify, mock_reaction_manager_class):
        ...
        unicode_emojis = [
            ...
            '🇺🇸',  # Flag emoji with regional indicators
        ]
