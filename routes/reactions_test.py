...
class TestGetAllowedEmojis:
    ...
    def test_get_allowed_emojis_empty_list(self, mock_reaction_manager, client):
        # Setup mock
...
class TestBulkAddReactions:
    ...
    def test_bulk_add_reactions_various_inputs(self, mock_reaction_manager, client, reactions, expected_status, expected_error):
        mock_reaction_manager.bulk_add_reactions.return_value = {"added": len(reactions) if reactions else 0}
...
class TestDecoratedFunction:
    ...
    def test_error_no_user_id(self, client):
        response = client.get('/test-endpoint')
        assert response.status_code == 401
        data = response.get_json()
        assert data['error'] == 'Authentication required'

    @pytest.mark.parametrize("header,query_param,expected_status", [
        (None, None, 401),
        ('', '', 401),
        ('123', None, 200),
        (None, '123', 200),
    ...
    def test_edge_cases(self, client, header, query_param, expected_status):
