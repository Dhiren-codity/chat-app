...
class TestRemoveReaction:
    ...
    def test_remove_reaction_missing_data(self, client, json_data, expected_status, expected_response):
        # Make request
        response = client.post('/api/reactions/remove', headers={'X-User-ID': '123'}, json=json_data)
...
class TestToggleReaction:
    ...
    def test_toggle_reaction_invalid_input(self, mock_reaction_manager, client, json_data, expected_status, expected_response):
...
class TestGetUserReactions:
    ...
    def test_get_user_reactions_various_cases(self, mock_reaction_manager, client, user_id, message_id, expected_status):
        ...
        query_params = f"?message_id={message_id}" if message_id is not None else ""
        headers = {'X-User-ID': user_id} if user_id else {}

        # Make request
        response = client.get(f'/api/reactions/user{query_params}', headers=headers)
...
class TestGetMostPopular:
    ...
    def test_get_most_popular_various_emojis(self, mock_reaction_manager, client, message_id, expected_emoji):
        # Setup mock
...
class TestDecoratedFunction:
    ...
    def test_authentication_with_header(self, client):
        with patch('routes.reactions.f', return_value=jsonify({"success": True})) as mock_f:
    ...
    def test_authentication_with_query_param(self, client):
        with patch('routes.reactions.f', return_value=jsonify({"success": True})) as mock_f:
            response = client.get('/test-endpoint?user_id=456')
            assert response.status_code == 200
            data = response.get_json()
            assert data['success'] is True
    ...
    def test_authentication_priority(self, client, header_value, query_value, expected_user_id):
