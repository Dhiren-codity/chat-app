...
class TestGetMostPopular:
    ...
    def test_none_message_id_parameter(self, mock_jsonify, mock_manager):
    ...
    def test_jsonify_exception_handling(self, mock_jsonify, mock_manager):
        ...
        # Assert
        mock_manager.get_most_popular_emoji.assert_called_once_with(message_id)
        # First call should be the success case, second call should be the error case
        assert mock_jsonify.call_count == 2
        error_call = mock_jsonify.call_args_list[1]
        assert "Failed to get popular emoji: JSON serialization error" in str(error_call)
        assert status_code == 500
