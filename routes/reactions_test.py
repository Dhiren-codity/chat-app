...
class TestGetMessageReactions:
    ...
    @pytest.mark.parametrize("message_id, expected_status", [
        ...
        (-1, 500),  # Assuming negative ID might cause an error
    ])
    @patch('routes.reactions.reaction_manager')
...
class TestGetUserReactions:
    ...
    def test_get_user_reactions_authentication(self, mock_reaction_manager, client, user_id, expected_status):
        # Setup mock
        mock_reaction_manager.get_user_reactions.return_value = [{'reaction': '👍'}]

        # Make request
        headers = {'X-User-ID': user_id} if user_id else {}
        response = client.get('/api/reactions/user', headers=headers)
...
class TestDecoratedFunction:
    ...
    @pytest.mark.parametrize("header_value,query_param,expected_user_id", [
        ('456', None, 456),
        (None, '789', 789),
        ('101', '202', 101),  # Header takes precedence over query param
    ])

    def test_various_authentication_methods(self, client, header_value, query_param, expected_user_id):
