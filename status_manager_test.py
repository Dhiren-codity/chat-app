...
@pytest.mark.parametrize("time_difference", [
    ...
    timedelta(seconds=1),  # Happy path: Slight delay
])

def test_set_offline_last_seen_time(time_difference):
    user_status = UserStatus(user_id=1)
    user_status.set_offline()
    first_last_seen = user_status.last_seen
    user_status.set_offline()
    second_last_seen = user_status.last_seen
    assert second_last_seen >= first_last_seen + time_difference
...
@pytest.mark.parametrize("user_id, is_online, last_seen, expected", [
    ...
    # Edge case: User with a future last seen time
