...
def test_get_status(user_id, is_online, last_seen, mocker):
    # Mock datetime to control the last_seen value
    mocker.patch('status_manager.datetime', autospec=True)
    status_datetime = datetime(2023, 10, 1, 12, 0, 0)
    status_manager.datetime.now.return_value = status_datetime
    user_status = UserStatus(user_id)
    user_status.is_online = is_online
    user_status.last_seen = last_seen if last_seen else status_datetime
    expected_status = {
        'user_id': user_id,
        'is_online': is_online,
        'last_seen': last_seen if last_seen else status_datetime
    }
...
def test_get_status_after_set_online(mocker):
    mocker.patch('status_manager.datetime', autospec=True)
    status_datetime = datetime(2023, 10, 1, 14, 0, 0)
    status_manager.datetime.now.return_value = status_datetime
    ...
    expected_status = {
        'user_id': 6,
        'is_online': True,
        'last_seen': status_datetime
    }
    assert user_status.get_status() == expected_status

def test_get_status_after_set_offline(mocker):
    mocker.patch('status_manager.datetime', autospec=True)
    status_datetime = datetime(2023, 10, 1, 15, 0, 0)
    status_manager.datetime.now.return_value = status_datetime
    user_status = UserStatus(7)
    user_status.set_offline()
    expected_status = {
        'user_id': 7,
        'is_online': False,
...
@pytest.mark.parametrize("user_id, is_online, expected_online", [
    ...
    (3, False, False), # Edge case: same user goes offline
])
...
@pytest.mark.parametrize("member_ids, expected_statuses", [
    # Happy path: All members are online
    ([1, 2, 3], [
    ...
    ([], []),
    # Edge case: Some members are offline
    ([1, 2, 3], [
        {'user_id': 1, 'is_online': True, 'last_seen': None},
        {'user_id': 2, 'is_online': False, 'last_seen': None},
        {'user_id': 3, 'is_online': True, 'last_seen': None}
    ]),
    # Error case: Non-existent member
    ([4], [
