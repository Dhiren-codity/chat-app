...
def test_get_user_status(status_manager, user_id, expected_status):
    # Setup for user 2 and 3
    status_manager.update_user_status(2, True)
    status_manager.update_user_status(3, False)
    # Get user status
    result = status_manager.get_user_status(user_id)
    # Adjust expected last_seen to match the result for comparison
    if result['last_seen']:
        expected_status['last_seen'] = result['last_seen']
...
def test_get_user_status_edge_cases(status_manager, user_id):
    result = status_manager.get_user_status(user_id)
    assert result == {'user_id': user_id, 'is_online': False, 'last_seen': None}
