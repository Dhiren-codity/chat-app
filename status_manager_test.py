...
def test_set_online_changes_status(initial_online_status, expected_online_status):
...
def test_set_online_with_no_prior_last_seen():
    user_status = UserStatus(user_id=1)
...
def test_get_group_members_status(status_manager, member_ids, expected_statuses):
    ...
    for res, exp in zip(result, expected_statuses):
        ...
        # We don't check 'last_seen' for exact match due to time variability
        assert (res['last_seen'] is None) == (exp['last_seen'] is None)
