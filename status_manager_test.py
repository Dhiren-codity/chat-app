...
def test_get_user_status(status_manager, user_id, is_online, expected_status):
    ...
    if expected_status['last_seen'] is datetime:
        ...
        assert status['last_seen'] == expected_status['last_seen']
...
def test_get_group_members_status(status_manager, member_ids, expected_statuses):
    ...
    # Adjust last_seen for comparison
    for status in statuses:
        if status['last_seen']:
            status['last_seen'] = status['last_seen'].replace(microsecond=0)
    for expected_status in expected_statuses:
        if expected_status['last_seen']:
            expected_status['last_seen'] = expected_status['last_seen'].replace(microsecond=0)
