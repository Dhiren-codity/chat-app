...
@pytest.mark.parametrize("initial_last_seen", [
    ...
    (None),                               # Edge case: last seen was never set
])
...
@pytest.mark.parametrize("initial_last_seen", [
    (datetime(2023, 9, 30, 12, 0, 0)),  # Edge case: Last seen is in the past
...
def test_status_manager_init_edge_case():
    ...
    assert len(sm.user_statuses) == 0, "edge case: user_statuses should be empty upon initialization"
