...
@pytest.mark.parametrize("user_id, is_online, last_seen", [
    ...
    # Edge case: user online but no last seen
    (4, True, None),
])
...
def test_update_user_status_happy_path(
        status_manager,
        user_id,
        is_online,
        expected_online):
...
def test_get_user_status_user_not_found(
        status_manager, user_id, expected_status):
...
def test_get_user_status_edge_cases(
        status_manager,
        user_id,
        initial_status,
        expected_status):
...
def test_get_group_members_status(
        status_manager,
        member_ids,
        expected_statuses):
    ...
    for status, expected in zip(statuses, expected_statuses):
        ...
        if expected['last_seen']:
            assert abs((status['last_seen'] -
                        expected['last_seen']).total_seconds()) < 1
        else:
