...
def test_set_offline_last_seen(user_status):
    with patch('datetime.datetime') as mock_datetime:
        ...
        user_status.set_offline()
        assert user_status.last_seen == datetime(2023, 10, 1, 12, 0, 0)
@pytest.mark.parametrize("mock_now, expected_last_seen", [
    (datetime(2023, 10, 1, 12, 0, 0), datetime(2023, 10, 1, 12, 0, 0)),  # Happy path
    (datetime(2023, 10, 1, 12, 0, 0) + timedelta(days=1), datetime(2023, 10, 2, 12, 0, 0)),  # Edge case: Future date
])

def test_set_offline_last_seen_edge_cases(user_status, mock_now, expected_last_seen):
    with patch('datetime.datetime') as mock_datetime:
...
@pytest.mark.parametrize("expected_statuses", [
    ...
    ({'user1': 'active'}, "edge case: pre-populated status"),  # Hypothetical edge case
])
