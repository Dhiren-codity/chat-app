...
def test_set_offline_last_seen_edge_cases(time_difference):
    user_status = UserStatus()
    initial_time = datetime(2023, 1, 1, 12, 0, 0)
    with patch('datetime.datetime') as mock_datetime:
        mock_datetime.now.return_value = initial_time
        user_status.set_offline()
        assert user_status.last_seen == initial_time
        mock_datetime.now.return_value = initial_time + time_difference
        user_status.set_offline()
        assert user_status.last_seen == initial_time + time_difference
...
@pytest.mark.parametrize("expected_statuses", [
    ...
    ({'user1': None}, "edge case: user with no status"),
])
...
@pytest.mark.parametrize("user_id, expected", [
    ...
    (3, {'user_id': 3, 'is_online': False, 'last_seen': None}),    # User not in list
