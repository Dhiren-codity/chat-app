...
@pytest.mark.parametrize("user_id, display_name, expected", [
    ...
    (3, "Nonexistent User", None),  # Error case: user does not exist
    (2, "", {'user_id': 2, 'display_name': ""}),  # Edge case: empty display name
])

def test_update_display_name(profile_manager, mock_db, user_id, display_name, expected):
