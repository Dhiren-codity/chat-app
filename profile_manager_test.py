...
@pytest.mark.parametrize("expected_folder", [
    ...
    ('uploads//profiles'),  # Edge case: double slashes
])
...
@pytest.mark.parametrize("invalid_folder", [
    (None),  # Error case: None as folder
...
@pytest.mark.parametrize("user_id, user_data, expected", [
    ...
    (2, None, None),
    (3, User(3, "janedoe", "Jane Doe", "jane@example.com", None, None, None, None), {
        'user_id': 3,
        'username': "janedoe",
...
def test_get_profile(profile_manager, user_id, user_data, expected):
    User.query.get = MagicMock(return_value=user_data)
