...
def test_init_upload_folder_error():
    with pytest.raises(AttributeError):
        profile_manager = ProfileManager()
        profile_manager.upload_folder = None  # Error case: setting to None
...
def test_update_status_message(profile_manager, mock_user, user_id, status_message, expected):
    # Mock User.query.get
    User.query.get = MagicMock(side_effect=lambda uid: mock_user if uid == 1 else None)
